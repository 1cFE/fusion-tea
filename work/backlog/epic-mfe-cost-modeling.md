...
Completed 2026-09-05 — closed and archived to `work/completed/20260905_WI-041_source-anchored-wall-load-fence/` at the close of goal `wall-and-heating` (`trail.md` § Goal close — 2026-09-05); the spec path below resolves there. Built under goal `wall-and-heating` round 2 (`source-anchored-wall-fence`), task T-002. The wall-load check compares a circular-torus **average** operand to the source's printed **peak** limit; this item makes it compare like with like — a computed peak, the model's average times a calibration computed in the model from the source's own printed peak (4.05 MW/m² at 2700 MW, Table 2 image) and the model's own reference area at the source's geometry — and moves the CAS72 lifetime operand to the peak, because the source sets first-wall lifetime by the peak damage (Table 6). Expected and disclosed: the baseline flips to `wall_load_ok` violated (≈ 4.087 against 4.05) and CAS72 rises about 37 %; neither is tuned. Basis: `work/orchestration/goals/wall-and-heating/evidence/round2_T-001_source_basis.md`. Spec: `work/active/WI-041_source-anchored-wall-load-fence/spec.md`. Home of discovery rows `20260903-priced-levers#1` and `20260903-wall-and-heating#1`, `#3` (routed to round 2 at the round-1 close).

### Item WI-042: Sourced Helium-Ash Profile (minted 2026-09-05)

Completed 2026-09-06 — closed and archived to `work/completed/20260906_WI-042_sourced-helium-ash-profile/` at the close of goal `stored-energy-basis` (its `trail.md` § Goal close — 2026-09-06); paths under `work/active/` below resolve there. Built under goal `stored-energy-basis` round 2 (`ash-profile-from-the-rule`), task T-001, on the owner's § Answered when (c) ruling (`work/orchestration/goals/stored-energy-basis/trail.md` § Owner ruling — 2026-09-05: *"we should fix the ash profile (and make sure this scales up for larger stellarators)"* `[OWNER-VERBATIM]`). The model gives the helium ash the fuel's profile exponent in the pressure integral while the source it cites for the ash amount (Stellaris Appendix A, A.5–A.6) also states the ash shape — "helium ash profiles are obtained using a fixed ratio of particle-to-energy confinement time" (p. 9). This item makes the ash profile follow that rule pointwise (n_He(ρ) ∝ n_D n_T ⟨σv⟩(T_i(ρ)), τ* uniform in ρ), closes the electrons by quasi-neutrality, gives W and β one pressure integral, and computes the shape from the rule at every lever point so it holds for a larger machine — never a point-A exponent. The W integral form, the fuel and temperature exponents and the peaks are reserved and do not change; nothing is fitted toward the printed 504.65 MJ. Expected and disclosed at the baseline (oracle-side prototype): W 551.4 → about 519.9 MJ; required heating 90.6 → about 49.1 MW against 50 (sustainment satisfied by 0.9 MW, inside the source's own 2.7 % residual — on the boundary); the wall peak 4.088 → about 3.979 against 4.05; LCOE 313.5 → about 322.3; neither flip tuned. Every committed study is restated (no multiplier re-reads a sustainment-dependent column; the `20260904-wall-and-heating` window is re-executed by the round's study). Basis: `work/orchestration/goals/stored-energy-basis/learnings.md` L-002, L-003; `evidence/w_counterfactual/attribution_sourced_definition.json`. Spec: `work/active/WI-042_sourced-helium-ash-profile/spec.md`. Home of discovery row `20260904-wall-and-heating#1` (its 2026-09-05 `stored-energy-basis` row: `model fix`, open, not minted, owner-gated — discharged when this item lands and the restating study reads it).

### Item WI-043: Burn-Control Lever or Second Sustainment Inequality (minted 2026-09-06)

Backlog, unstarted; captured at the close of goal `stored-energy-basis` `[OWNER-VERBATIM 2026-09-06: "carry all three, but make sure any relevant work items are captured in the backlog"]`. Home of discovery rows `20260904-wall-and-heating#4` and `20260905-stored-energy-basis#1`. The sustainment fence `sustainment_ok` is one-sided: it asserts `p_aux_required ≤ installed coupled heating` and so passes points whose computed requirement is negative — a plasma that would self-heat beyond control, for which the model has no steady state at any non-negative heating and no burn-control lever. At the WI-042 pin this decides every count on the package: 3,654 of the 7,712 points of study `20260905-stored-energy-basis` read a negative requirement, 510 of the 681 driven points of the committed `20260904-wall-and-heating` record ignite at the rule, and the cheapest "feasible" points at both heating levels are ignited unless the driven set is read separately. Candidate forms: a second inequality `p_aux_required ≥ 0` in `mfe_viability.sysml` 'Sustainment Limit', or a burn-control lever the model does not have; which is right is a modelling judgement, and the round-2 review of `stored-energy-basis` recommends grounding it as a goal before the form is chosen. Every feasibility claim on the package is made on the driven set until this lands. Sources: `exploration/stellarator_e2e/studies/20260905-stored-energy-basis/record.md` §§ 3, 4, 15 #1; `work/orchestration/goals/stored-energy-basis/learnings.md` L-005.

### Item WI-044: Minor-Radius Bound and Wall-Peak Calibration Re-Anchoring (minted 2026-09-06)

Backlog, unstarted; captured at the close of goal `stored-energy-basis` `[OWNER 2026-09-06]`. Home of discovery rows `20260904-wall-and-heating#3` and `20260905-stored-energy-basis#2`. Nothing in the model bounds the minor radius `a` from above and the magnet account does not price it (the winding length is `k_coil × R0` with a held casing mass — `work/completed/20260903_WI-036_winding-pack-sizing/design.md` D3), so every study optimum sits at the window's `a`-edge (2.2 m at both heating levels in `20260904-wall-and-heating` and again at the WI-042 pin), and ignition grows along `a` (41 → 460 ignited points over 1.3 → 2.2 m at the rule). The wall-peak calibration (WI-041) is anchored at the source's design geometry and carried unchanged across the sweep; the round-2 review of `wall-and-heating` named a re-anchoring rule beside the bound. Candidate forms: a bound on `a` from a sourced constraint (a coil-bore or radial-build term — the WI-036 seam) and a re-anchoring rule for the calibration; the geometry question underneath ("is the minor radius the model favours a machine?") is a goal of its own if pursued. Related and not a work item: the research request on the aspect-ratio scaling of τ*/τ_E, helium suppression and the ISS04 iota (`20260904-wall-and-heating#2`, `20260905-stored-energy-basis#8`), carried in the discovery log, not minted. Sources: `exploration/stellarator_e2e/studies/20260904-wall-and-heating/record.md` § 15 #3; `20260905-stored-energy-basis/record.md` § 6 `a`, § 15 #2; `work/orchestration/goals/wall-and-heating/learnings.md` L-009, L-010.
...
- **Item 1 / WI-009** — **CLOSED AS SUPERSEDED ([OWNER] 2026-07-25, demo epic Board housekeeping).** Substance built (`mfe_power_core.sysml` CAS22.1.3/.4/.8 + `mfe_divergent`, `mfe_power_balance.sysml`, `mfe_viability.sysml`, plus later additions `mfe_magnet_cost`, `mfe_cryo_plant`, `mfe_account_costs`; re-sourced from 1costingFE per the PROTOCOL §5 mitigation) and refined/audited through the WI-019–028 corrective run, whose records are the effective completion evidence. No close-out audit vs own spec, no re-scope; disposition note in the item record.
- **Item 2 / WI-010** — **CLOSED AS SUPERSEDED ([OWNER] 2026-07-25, same ruling).** `mfe_plant.sysml` is the wiring backbone, refined/audited via WI-019–028. Decision record: its "LCOE within a credible MFE range at defaults" bar was never formally assessed; the ruling accepts that gap rather than opening a close-out audit.
...
- **WI-018** — **CLOSED AS SUPERSEDED ([OWNER] 2026-07-25, same ruling).** The instance it set out to build exists and carries the demo headline; the item was spec-only, overtaken by the corrective run. Disposition note in the item record.
...
**Last Updated**: 2026-09-06 (WI-042 completed; WI-043 and WI-044 captured at the close of goal `stored-energy-basis`)
**Next Action**: none scheduled within this epic. Demo work — including the account-scope/handshake-gap items and the hold-out comparison — is governed by `.project/concepts/stellarator-mbse-demo.md` and tracks in the dedicated demo epic (`.project/backlog/epic_stellarator_mbse_demo.md`), not here. The Item 1/2/WI-018 disposition was ruled 2026-07-25 (closed as superseded — see Item Status Assessment). This epic retains only its own residual, non-demo goals: the tokamak instantiation (Item 3's other half — a demo Non-Goal, so it waits on its own merits) and the viability sweep (Item 4). Both are owner sequencing calls, likely post-reveal.
