...
**MR-WI027-8 (Constraint, HARD).** Canonical `models/` semantics SHALL NOT be edited by this item. Regeneration consumes the canonical asserts as-is. If design surfaces a blocking incompatibility between the canonical model and constraint lowering, that is a **surface-to-orchestrator** event, not a silent edit.
