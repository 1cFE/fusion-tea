...
- [x] **Commit B**: the regenerated package, the impl, the oracle, the pin files, the fixtures, this plan — `cb355321` (first made as `9ebf9f08` without a pathspec, which swept in four of the owner's staged `.project` files; amended the same minute to drop them and their exact staged blobs restored to the index, working tree untouched)
...
**The study battery of record (after commit B `cb355321` and the `test_verify.py` restatement): **359 passed / 1 skipped** — the recorded baseline exactly; the run of record, after commit B and the one test restatement (`test_verify.py`'s planted mismatch reversed in direction because the store now records `wall_load_ok` violated), invoked as the plan says with no `PYTHONPATH` export; 456.8 s.** With `tests/models` 48 passed / 13 skipped, both batteries are green with every delta explained and no expectation patched to match. Phase 6 complete; the item's close (through the modelling PM, after the round's study) is owner-held.
