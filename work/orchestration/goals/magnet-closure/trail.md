...
### Round 2 review — 2026-09-01

**Reviewer:** a fresh session — it authored no part of round 1, round 2, or the round-1 review (`GOAL_RUNBOOK.md` § What "fresh" means); it picked up from the § Stop handoff and the commits. **Verdict: `FINDINGS`.** Four findings. None changes the scores; two change what the owner should be told before closing. The round stays closed and is not resumed.

**What I re-derived myself, from the committed artifacts rather than from the trail.**

- **The grader's study numbers.** Recounted straight from `20260830-stress-fence/results/points.csv`: 660 `wp_stress_ok` violations on the grid arm; the transect flips between 0.28 m (835.7 MPa, violated) and 0.30 m (780.0 MPa, satisfied); `magnet_capital` and `lcoe` bit-identical at all 16 transect points (finding `#2` is visible in the column, not inferred); the rollup identity `magnet_capital == winding_pack + magnet_structure` holds at all 1,634 rows. Every number the two cell records lean on reproduces.
- **The 1,617-vs-1,618 offset is a stated convention, not a defect.** The grid arm carries 1,618 rows while every § 4 tally reports 1,617 (my recount: 660 violated + 958 satisfied). The record states it — "Grid = 1,617 points ...; baseline appended once" (`record.md:34`) — and `synthesis.md:42` reconciles 1,634 = 1,617 + 1 + 16 explicitly. Recorded here so no later reader re-chases it.
- **Every line citation in `grading-r3-regrade.md` resolves and says what the cell claims.** Spot-checked across both cells: `mfe_magnet_field.sysml:45` (the B_axis expression) and `:81` (sigma_wp), `mfe_viability.sysml:145` (the limit inequality), `mfe_plant.sysml:55` (the EXPOSE), `:891-894` (the assert), `:909-910` (the two comparison/cryo channels), the instance bindings at `stellarator_plant.sysml:174,195,205`, and `VALIDATION_MATRIX.md:64-66`. The grader read what it cited.
- **The round honoured its own "no model increment" declaration exactly.** Six files changed since the round-1 close commit `591d3c3a`, all of them records: `CURRENT_WORK.md`, the two rubric-directory deposits, `DISCOVERY_LOG.md`, `learnings.md`, `trail.md`. `git diff 04b258d4..HEAD` over `models/`, the generated package, the model twins, `tests/`, and `scripts/` is empty. Nothing under measurement moved while it was being measured.
- **The battery at round 2's own sha.** The 398-passed run stands from round-1 HEAD and no code moved after it, so it carries. What round 2 could have broken is the record layer, and that is green at HEAD: `tests/orchestration` + `tests/study/test_records.py`, 39 passed — the goal-contract and findings-join tests included.
- **The round-1 reviewer's one warning was honoured.** It asked that the re-grade weigh SV-040's cryoplant conjunct directly rather than inherit the `passing` flag the WI-035 author set. The grader did, in its own "weighed directly, not inherited" section, and argued the conjunct from the anchor's wording rather than from the flag.

**Scope, strategy, and disposition fidelity.**

- **Strategy fidelity: yes.** The declared strategy was one measurement and nothing else; the round ran one task, deposited two cell records, and touched no model. No abandonment condition fired, and no rubric anchor was contested — reserved gate 4 was never approached.
- **Scope drift: none.** T-001's authorized set was the evidence map, the grader's deposit, trail entries, and the disposition rows. That is exactly what changed.
- **Retries: none to classify.**
- **Dispositions: landed.** Three rows appended under existing ids (`20260830-stress-fence#1`, `#2`, `20260823-magnet-technology-ab#4`), plus the round-1 amendment's `20260821-power-cycle-ab#1` row in the round-open commit. No touched row returns `unrouted`; `#1`/`#2` stand as *proposals* pending the owner, which is the honest state and not a `unrouted` return. `#4` is unchanged and still behind the Rung C gate, correctly.
- **Cited artifacts: unmoved.** The rubric's last touching commit is still `dc0f0b6d`, so `rubric.md@dc0f0b6d` is the same yardstick both gradings cite — the frozen-yardstick claim holds mechanically, not just by assertion.

**Findings.**

1. **The grader's independence was asserted, not recorded.** The goal's close condition rests on one line inside the grading deposit: "fresh re-grade session (non-author) ... received the pointer-only evidence map". Nothing on disk recorded how that session was obtained or what it was asked, so a leading prompt would have been undetectable. Round 1's administrator has the same shape and the round-1 review accepted the precedent, so this is the precedent's cost arriving at the moment it matters most, not a new defect. **Discharged today** by the amendment below: the spawn prompt is recovered verbatim from the round-2 session transcript and deposited at `evidence/round2_T-001_grader_prompt.md`. It reads as a non-leading prompt — the three cruxes are handed over as open questions ("Decide whether the conjunction is met as written"), the prior scores are given with "NOT to anchor your score", and no target is stated anywhere.
2. **"Affected studies" is three studies, not one.** The close proposal put to the owner frames the non-reproducibility wrinkle as the magnet A/B alone. Design D8 (`work/active/WI-035_magnet-closure/design.md:76-80`) names three, in its own words: `20260823-magnet-technology-ab` **not reproducible as written** (its 74-value B axis retired); `20260829-p-pump-fence` **not reproducible as written** (its R sweep held B = 9.0, so post-inversion its committed points mean something else); `20260821-power-cycle-ab` — axes untouched, replays up to the 1-ulp field shift. The third is the one study that *could* have been re-run at the new pin and was not. What stands in its place is the 99/99 per-R fence agreement against p-pump-fence, honestly scoped to the field-independent fences. **Discharged today** by the second amendment, which records the owner's ruling against all three studies by name rather than against one.
3. **The goal's own question names cryoplant, and cryoplant was exposed rather than newly sized.** § Question asks for cost "split into separately sized winding-pack / structure / cryoplant accounts". What D7 landed is a split of the pre-existing aux-cooling account into its `aux_cost` / `cryo_cost` outputs, with `cryo_cost` exposed as its own channel — same expression, bit-identical value. The grader weighed this against the S3 anchor and accepted it with an argument I agree with (the anchor tests separately sized sub-accounts, not a CAS22.1.3 home; power supplies, which the same anchor names, never lived there either; the anchor tests model state, not the diff). The finding is not the ruling, it is that the close summary never surfaced it: the owner should close knowing that one of the three accounts the question names was exposed, not built.
4. **The aggregate shape of R3.S is stated nowhere, only its pieces.** Of the four sub-accounts the S3 anchor names, exactly one responds to a magnet design lever. Winding pack follows `I_coil` (through a held 25 m winding length). Structure is held per-casing mass times coil count. Cryoplant is `cryo_base·(p_cryo/30)^0.7` over a chain whose two drivers, `q_nuc` and `vol_cold`, are both held (`mfe_cryo_plant.sysml:48`; `vol_cold_cryo` held under the standing WI-032 R3 ruling). Power supplies is `base·(p_et/1100)^0.7` in plant gross electric, carrying no magnet quantity at all (`mfe_account_costs.sysml:140-165`). Each piece is disclosed in its own place and the grader's cell cites the drivers accurately; the sum of them is written down nowhere. It is the honest one-line summary of what S3 currently buys, and it belongs in `learnings.md` — appended there as a reviewer-added entry.

Not findings, but worth carrying:

- **What the S2→S3 delta actually rests on.** Power supplies and the cryoplant account both predate WI-035, so the measured structural delta is carried entirely by the winding-pack / casing-structure split. That is real and it is what the round claimed; it is worth saying plainly so the delta is not read as four accounts appearing at once.
- **SV-040's wording is close to the anchor's, and its author is the WI-035 author.** The circularity is inert here because the grader argued the conjunct from the anchor and cited the flag only alongside the model evidence. It would not stay inert if a future re-grade leaned on the flag.

**Learning delta: item 1 accepted as proposed, unchanged; one reviewer-added entry.** Item 1 is supported by what I checked above — the rubric's last touching commit is still `dc0f0b6d`, both gradings cite the same sha, and no anchor was contested, so "the yardstick held still while the model moved" is mechanically true and not a claim about intent. The reviewer-added entry is finding 4's aggregate, marked as the reviewer's own so its provenance is not confused with the round's.

**Recommendation.** Land the two amendments, then recommend the owner-held close: § Answered when's measurement is taken at target, its battery conjunct is green, and its study conjunct is satisfied on the reading recorded below. Mint the `#1`/`#2` routing as one item — with one scoping caveat for its spec: `c_coil` = 25 m is the printed *typical* circumference of non-planar stellarator coils, with per-coil circumferences unprinted (`stellarator_plant.sysml:191`), so deriving winding length from bore geometry will need a coil-shape factor. The likely outcome is trading one held constant for a smaller one, plus the genuinely bigger half — `wp_side` gaining a cost consequence. Nobody should expect S4 from that item, and its spec should say so up front rather than discover it at design.

### Amendment 2026-09-01 — amends "T-001 return — 2026-08-30" and "Round 2 result — 2026-08-30" (grader provenance)

Per Round 2 review finding 1: both entries rest the re-grade's authority on the grader's own self-attestation inside `grading-r3-regrade.md`, with nothing on disk recording how the grader was obtained or what it was asked. Discharged today. The spawn is recovered verbatim from the round-2 session transcript and deposited at `work/orchestration/goals/magnet-closure/evidence/round2_T-001_grader_prompt.md`: one `Task` spawn, `subagent_type: general-purpose`, description "Fresh Row-3 re-grade", the `prompt` field byte-for-byte, with the transcript path and tool_use id named in the file's header. Both entries' evidence refs are to be read as including that file. The prompt supplies the frozen rubric, the pointer-only evidence map, and the prior scores marked "NOT to anchor your score", and hands the three cruxes over as open questions with no target score stated — the reviewer's reading is that it does not lead. Going forward, a goal round that obtains a grader, an administrator, or any other clean-context judge deposits the spawn prompt in `evidence/` at the time, rather than leaving the independence claim to be recovered later.

### Amendment 2026-09-01 — amends "T-001 return — 2026-08-30" (the outstanding owner reading) and "Round 2 result — 2026-08-30" (stop reason)

Per Round 2 review finding 2: the T-001 return states the outstanding "affected studies re-run clean" reading against one study (the retired B axis of `20260823-magnet-technology-ab`). The affected set is three, per the design's own D8 (`work/active/WI-035_magnet-closure/design.md:76-80`), and the owner's ruling of 2026-09-01 (recorded at § Goal close below) is to be read against all three:

- `20260823-magnet-technology-ab` — **not reproducible as written**; its 74-value `B` axis retired with the inversion the owner ruled on 2026-08-30. Not re-run. A translated re-run onto `I_coil` was considered and declined: it would be a new study answering a question nobody has open.
- `20260829-p-pump-fence` — **not reproducible as written**; its (R, a) sweep held B = 9.0, so post-inversion an R sweep at held current moves the field and its committed points mean something else. Not re-run. What stands in its place is the point-by-point comparand check the round-1 study performed and the round-1 review verified from outside the record: over the 33 shared R values, the three field-independent fences (`recirc_ok`, `wall_load_ok`, `net_positive`) each agree at every one — 99/99, zero mismatches. The field-side fences are not comparable across the re-baseline and were not compared, which is the honest scoping.
- `20260821-power-cycle-ab` — axes untouched; the only one of the three that **can** replay at the new pin. Not re-run. Its open row `#1` was re-confirmed at the new pin by re-declaring and re-tracing `availability` against the regenerated package (still `no_constraint_response`), not by replaying the study.

The § Answered when conjunct is therefore satisfied by the new study running clean at the new pin plus the recorded restatement, on the intent the goal wrote down for the word: "re-run without crashes or unexplained constraint-verdict flips". No verdict flipped unexplained anywhere the two states are comparable. What is **not** claimed: that any of the three committed records replays. Two of them cannot, by the design consequence the owner ruled and the round restated in writing before regeneration. A replay of `20260821-power-cycle-ab` at the new pin remains available to a future round as a regression check; it is not a condition of this close.

## Goal close — 2026-09-01

### Owner ruling — 2026-09-01

Ruled by the owner in session, on the round 2 review's recommendation. Verbatim: "a / close / mint". `[OWNER 2026-09-01]`

- **(a) The "affected studies re-run clean" conjunct.** Reading (a) accepted: the new study at the new pin plus the recorded restatement satisfies the conjunct; no translated re-run of the retired B axis is required. The reading is recorded against all three affected studies by name in the amendment immediately above, not against one.
- **Close.** § Answered when is met in the contract's own terms: a fresh non-author re-grade against `rubric.md@dc0f0b6d` reads **R3.P = 3** (was 1) and **R3.S = 3** (was 2) with the anchors' full conjunctions evidenced; the validation battery is green (398 passed / 14 skipped at the round-1 sha, no code moved after it; the record layer re-run green at the round-2 sha); the study conjunct stands on reading (a). Each of the three was checked independently by the fresh review rather than taken from the trail. The goal is **closed by the owner** per § Close rule.
- **Mint.** No round 3. Sightings `20260830-stress-fence#1` (winding length held at `c_coil`) and `#2` (`wp_side` costless) are one gap and are minted as **WI-036** ("Winding-pack sizing chain: winding length from coil geometry, wp_side into the sizing and cost chain", standard, P1) under the MFE Cost Modeling — Tokamak & Stellarator epic, through the modeling PM's own `pm add-item`. Two final rows appended under the existing ids; the work item's own record carries them from here. Minting creates the backlog item only — it starts no work and opens no goal.
- **Carried into WI-036's spec, not decided here:** `c_coil` = 25 m is the printed *typical* circumference of non-planar stellarator coils and the per-coil circumferences are unprinted (`stellarator_plant.sysml:191`), so a geometry-derived winding length will need a coil-shape factor. Expect one held constant traded for a smaller one, plus the larger half — `wp_side` gaining a cost consequence. The item is not a route to S4.
- **Disclosed at the close, per review findings 3 and 4:** of the three accounts § Question names, the cryoplant account was **exposed** (D7 split an existing expression into its own output and channel, bit-identical), not newly sized; and of the four sub-accounts the S3 anchor names, only the winding pack responds to a magnet design lever. The grader weighed both against the written anchor and the review agreed with its rulings. The owner closes knowing this.

**Effect:** goal `magnet-closure` is closed, answered at target. The first measured maturation delta of the demo epic's Item 10 chain is on the record: Row 3 (1, 2) → (3, 3) at a yardstick that did not move. WI-035's item close, merge, and push remain owner-held and are untouched by this ruling.

### Amendment 2026-09-01 — amends every entry citing `work/active/WI-035_magnet-closure/` (path redirect on item close)

WI-035 was closed by the owner on 2026-09-01 through the modeling PM's own operation (`pm close-item WI-035`), which archived the item to `work/completed/20260901_WI-035_magnet-closure/` and set its backlog row to `completed: 2026-09-01`. The spec/design/plan citations throughout this trail, the round-2 evidence map, and `grading-r3-regrade.md` were written against the live path and are to be read at the archive path; the three files moved unchanged. Nothing is edited in place to chase the move — this entry is the redirect. `tests/models` re-run after the archive: 48 passed / 13 skipped, unchanged from the item's own implementation record.

**One consequence, flagged and deliberately not fixed.** The `'Magnet Capital'` calc def carries `**Source**: work/active/WI-035_magnet-closure/design.md` in its doc comment (`models/library/analyses/mfe_magnet_cost.sysml:146`, its twin, and the three generated files and the snapshot that echo it). That citation no longer resolves, which is an MR-4 defect. Correcting it means editing `models/`, which moves the semantic fingerprint and invalidates pin `287479a3…` — a regeneration and a fresh integrate hop for a doc-comment path. It rides the next regeneration that happens for a real reason (WI-036 is the obvious carrier) rather than forcing one. Recorded here and in `.project/CURRENT_WORK.md` so it is not rediscovered as a surprise.
