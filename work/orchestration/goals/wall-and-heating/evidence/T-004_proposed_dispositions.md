...
## Notes applied after C-001.r2 (`PASS`) — 2026-09-04

Two non-blocking notes from the r2 verdict, applied before the rows were appended; neither changes a reading. **Row 2:** the heating-cost product at `860ce7d1` is `ecrh_per_mw * p_ecrh_in` in `models/library/analyses/mfe_account_costs.sysml:221-223` (the plant instance at `stellarator_plant.sysml:842-845` binds the rate and `p_ecrh`); the row's citation now names both. **Row 14:** the optimised-wall statement reads *clears below a net of 1.042 (the crossover, 4.05 / 3.886) and fails above it; at the optimised range's ends the operand reads 3.342 (0.86) and 4.170 (1.073)*.
