...
### Amendment 2026-09-05 (b) — amends § Reserved gates (the W form and exponents gate) and § Invariants

**§ Answered when (c) ruled** `[OWNER-VERBATIM 2026-09-05]` *"ok I agree with next steps. we should fix the ash profile (and make sure this scales up for larger stellarators). and I don't want to add the footnote."* — trail § Owner ruling — 2026-09-05. The reserved gate on changing the profile family in `models/` is resolved in one respect only: **the helium-ash profile shape in the pressure integral changes to the paper's own rule, with the electrons closed by quasi-neutrality, as a minted work item through the modelling PM.** The W integral form, the fuel and temperature exponents, and the peak fixed point are not released by this ruling; W is still never tuned, and the printed 504.65 MJ is still not a target (`learnings.md` L-002).

**§ Invariants gains one line** `[OWNER-VERBATIM 2026-09-05: "make sure this scales up for larger stellarators"]`: the ash profile is computed from the rule at any geometry — the fusion-rate profile times the particle-to-energy confinement ratio — never bound as an exponent digitized at point A; the item's design and its restating study show the shape and its effect on W across the geometry window the committed studies sweep.

Nothing else in this file changes.

### Amendment 2026-09-05 (c) — amends § Invariants (the Package line)

**Trigger.** The Package line says nothing is written under `models/`, the generated package or the manifest by this goal, and that the pin stays `c1b0f0d1…`. Amendment (b) records the owner's (c) ruling: the ash-profile change lands as a minted work item through the modelling PM under this goal, and the owner's new scaling line asks for the item's restating study — which needs a promoted pin. Following the Package line as written would work against the ruling. The conflict is surfaced here (capture-fidelity § Surfacing), not resolved silently.

**Reading** `[AGENT] — the consequence of the owner's (c) ruling, written by the round-2 agent; awaiting ratification`. The Package line binds this goal's own pen. A `pm add-item`, a spec through the owning PM, a regeneration through the pinned codegen and a pin through the integrate seam are those workflows' writes, not the goal layer's (`GOAL_RUNBOOK.md` § The discovery log, § The native seams). From round 2: the model change lands only through the modelling PM's stages; the generated package and the manifest change only inside that item, through the pinned codegen and `scripts/integrate.py`; round 2 promotes at most one pin, a new identity the trail cites; and `c1b0f0d1…` stays the pin of record for every round-1 number and for every comparison the restating study makes against the committed `20260904-wall-and-heating` record. The baseline of record for round 1 stays LCOE 313.513 with `sustainment_ok` and `wall_load_ok` violated. The second bullet of § Invariants ("labelled oracle-side … until a model change lands through the modelling PM and a study runs at a new pin") already anticipated this. Nothing else in this file changes.

### Amendment 2026-09-06 — amends § Status (closed), § Invariants (Amendment (c) ratified) and § Reserved gates

**Status: `closed`, 2026-09-06**, on § Answered when, by owner ruling `[OWNER-VERBATIM 2026-09-06: "close it"]` on the round-2 fresh review's recommendation (trail § Round 2 review; `evidence/round2_review.md` § Recommendation): (a) met at grounding and ratified; (b) met in round 1 (the written attribution) and corroborated in the package (SV-053); (c) ruled on 2026-09-05 and executed — WI-042 minted, landed, pinned (`ec984adc1572…`), the `20260904-wall-and-heating` window restated by re-execution, the scaling line of Amendment (b) met by construction and demonstrated. Round 2 of 6; no round 3 under this contract.

**§ Amendment (c) ratified as written** `[OWNER-VERBATIM 2026-09-06: "ratify as written"]`: its reading of the Package line — the invariant binds the goal's own pen; writes through the modelling PM inside a minted item are that workflow's; one new pin per round; `c1b0f0d1…` the pin of record for every round-1 number and comparison — is owner-grade from this date.

**§ Reserved gates:** the first bullet (ratification of the wording) closes with the goal as worded; the W form / exponents gate stands resolved in the one respect of Amendment (b) and untouched otherwise; the re-grade of the inherited invariants is executed as added, dated update notes beneath the original lines `[OWNER 2026-09-06]` (trail § Goal close, ruling 5); the `wall-and-heating` fifth condition recorded the same way (ruling 5); the rubric Row 1 consequence is not ruled and is carried with WI-043.

The rulings, the executed admin and what is carried: trail § Goal close — 2026-09-06. Nothing else in this file changes.
