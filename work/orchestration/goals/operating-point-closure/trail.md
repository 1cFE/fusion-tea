...
### T-006 return — 2026-09-01 (round 2)

- **Outcome:** `COMPLETE`. The fresh non-author re-grade exists: `.project/active/demo-depth-rubric/grading-r1-regrade.md` — **R1.P = 3 (was 2), Row 1 at its declared target (P3 / S not_applicable reconfirmed)** — same rubric sha `dc0f0b6d`, model `fbf70093`, executed claims at package `41e06ecb…`/c0000; a valid progress comparison per protocol §7. Grader prompt deposited at `evidence/round2_T-006_grader_prompt.md`; the grader's independence statement is in the file's header.
- **Evidence:** the re-grade file; SV-041/042/043 flipped to `passing` via `pm update-validation` on the now-existing evidence (impl spot-check + oracle parity + committed study).
- **Reading.** The grader's anchor reading: the L3 anchor's operative verbs are *links* and *pushes back*, both satisfied — ISS04 ties the computed field, density, and temperature into one executable balance whose computed operand meets an engineering limit that demonstrably structures the feasible set, and the committed study's constrained optimum leaves the beta floor. Three things the grader routed onward, carried to the close packet: (1) **a contestable-anchor note** — the row *preamble* says "determine" where the anchor table says "links"; if the owner meant the solver reading, that is an owner-gated rubric revision (new version, both model states re-scored), never a grading override; (2) **Row 4's target revisit is now owed** (its P3 was conditioned on Row 1; owner-gated target-setting, recorded not acted); (3) five evidence-integrity findings attached to the cell, none a level (oracle-derived-both-sides operand values; the disclosed never-tuned baseline violation; stale prototype-vintage digits in the instance retirement notes; the study record's Addendum-corrected defects; the SV rows' pending status — the last now discharged by the flips above).
- **Decision:** trigger — SV-041/042/043 read `pending` with their evidence in the repo (grader EI finding). Decision and reason — flip to `passing` through the native op; the evidence is the T-003 spot check, the T-004 oracle parity at rel 0.0, and the committed study's fence anatomy. Tier — execution detail. Who decided — the round agent, 2026-09-01. What changed — `modeling_project/VALIDATION_MATRIX.md` (three statuses, op-written).

## Round 2 result — 2026-09-01

- **Intent:** **met.** The round set out to land the forward-sustainment closure and measure it; Row 1 grades at target by a fresh non-author re-grade at the frozen rubric.
- **Task sequence:** T-001 (spec/SV amendment + `#4` routing row) `COMPLETE` · T-002 (design) `COMPLETE` · T-003 (implement, both trees, tests green) `COMPLETE` · T-004 (integrate `CANDIDATE` pin `35e922c5…`) `COMPLETE` · T-005 (committed study `20260901-sustainment-fence` + fresh synthesis) `COMPLETE` · C-001.r1 `REVISE` → C-001.r2 `PASS` · T-006 (fresh re-grade) `COMPLETE`.
- **Last semantic outcome:** `COMPLETE` (T-006) with a valid study reading behind it (T-005 + checkpointed dispositions).
- **Stop reason:** last outcome `COMPLETE` + a valid study reading in hand + the goal's § Answered when measured → round closes on trigger 1 (valid study reading; the answered-when assessment goes to the owner through the close rule).
- **Evidence refs:** `work/active/WI-037_operating-point-closure/` (amended spec, design, plan with implementation record and MR-WI037-7 restatement); `evidence/T-004_integration_return.json` (pin); `exploration/stellarator_e2e/studies/20260901-sustainment-fence/` at `1d28454f`+`62a1fa7b` (record, synthesis, addendum); `evidence/T-005_precritique.md`, `evidence/T-005_proposed_dispositions.md` (+ r2), `evidence/round2_T-006_grader_prompt.md`; `.project/active/demo-depth-rubric/grading-r1-regrade.md`; battery at result: `tests/models` 48 passed / 13 skipped; `tests/study` 348+2 passed in the seam environment (T-004 note on the env-artifact failure).
- **Learning delta (proposed):**
  - L-004: The Row-1 P3 anchor is satisfiable by forward sustainment — "links … and pushes back" — without a solved operating point; the fresh grader's reading concurs with the strategy's, and the residual "links vs determine" tension is a rubric-wording question for the owner (contestable-anchor note), not a modeling gap.
  - L-005: With the sustainment fence active, the machine's honest state is: no feasible operating point at the printed 50 MW installed anywhere in the swept space (the ISS04-relief-vs-conductor-ceiling deadlock); feasibility returns at ≥ ~91 MW installed, and the constrained optimum (LCOE 293.468, beta 0.0311) sits off the beta floor, bounded by sustainment below and the conductor ceiling above — field is rewarded, and the binding technology is the magnet, not plasma stability.
  - L-006: The committed-study fixture chain (known-answer expectations, suite constants, manifests) carries the operating-point-holds assumption widely; retiring held plasma inputs cost ~30 re-derivation sites, all re-derived from live evidence, never patched to match — budget for this in any future entry-point retirement.
- **Finding dispositions:** all five checkpointed dispositions landed — `20260823-magnet-technology-ab#4` **discharged** (class-bearing row appended, not-final caution carried); `20260901-sustainment-fence#1` and `#4` routed `model fix` as close proposals (class-bearing rows appended, minting reserved to the owner/next selection act); `#2` standing WI-034 route reaffirmed; `#3` closed as declared seam for this goal (ANNEX names `sustain__*`; upstream limitation stands under `20260821-power-cycle-ab#5`). No touched row returns unrouted.

### Round 2 review — 2026-09-01

- **Reviewer:** fresh non-author session, spawned from the deposited prompt (`evidence/round2_review_prompt.md`); no context inherited. Full review deposited at `evidence/round2_review.md` — the review of record; this entry summarizes.
- **Verdict:** `FINDINGS` — the round's substance stands in full; § Answered when genuinely met pending the owner's two conjunct readings; close recommended over a round 3.
- **Checks (all performed):** every cited ref opened; strategy fidelity confirmed (amend-before-implement discharged first; the grader's owner-routing of the anchor tension is the strategy's own path, not an abandonment); all six task scopes walked commit-by-commit — two scope notes named (T-004's tests/study fixture surface was outside the authorized list as written: disclosed drift, required by the seam's own gates, no rework owed; T-005's study-layer writes are the invoked runbook's own steps, authorized); zero-retries classification verified against the runbook definition; the checkpoint sequence verified append-only with real reviewer pushback; all five discovery dispositions landed as class-bearing rows with honest movement claims; external-mutation walk clean (nothing cited moved outside its task); crux numbers independently recounted (334/9 feasible/293.46793/beta 0.031091/all eight violation counts to the digit); battery re-run by the reviewer (models 48/13; study suite zero failures).
- **Findings:** five, all records-hygiene: (1) integrate scopes must name the fixture/suite surface henceforth; (2) the C-001 checkpoint spawn prompt was undeposited — discharged by dated addition at `evidence/round2_C-001_checkpoint_prompt.md`; (3) design D7 "three"-vs-four prose slip (dated note when next touched; the plan restatement is authoritative and correct); (4) SV-041..043 Requirement/evidence columns empty (fill when next touched); (5) grader-carried items filed at source.
- **Learning delta:** L-004 accepted; L-005 accepted with the precision correction (the ~91 MW threshold is oracle-derived; the committed resolution is one grid step, flip between 90 and 100 MW); L-006 accepted. Appended to `learnings.md` with this entry.
- **Constraints carried forward:** seven, listed in `evidence/round2_review.md` § 4.
- **Next:** the reviewer recommends the **owner-held close** (round 2 of 6; § Answered when's measured conjunct met at the frozen yardstick). The close packet — the delta, the five owner rulings it needs (the "affected studies re-run clean" reading; the contestable-anchor note; Row 4's target revisit; the two `model fix` close-proposal routes; WI-037 close/merge/push) — is in `evidence/round2_review.md` § 5 and is put to the owner by the round agent.

## Goal close — 2026-09-02

Owner present in session; the five close-packet rulings, each `[OWNER 2026-09-02]`, taken one at a time through /_my_ask_me:

1. **The anchor stands as written — "links" is the operative test.** Row 1's P3 grade holds; no rubric revision is triggered by the preamble's "determine" wording. The solver ambition stays recorded at the P4 rung and in L-002.
2. **The "affected studies re-run clean" conjunct is satisfied on the offered reading** (the magnet-closure (a) analogue): the new study ran clean at the new pin, the MR-WI037-7 restatement names all four non-replayable studies, and the one baseline verdict change is disclosed and explained. No replay directed.
3. **Both evidenced escape routes minted as backlog items, neither grounded:** `20260901-sustainment-fence#1` → **WI-038** (Conductor-Grade Lever: B_max Consequence Chain); `#4` → **WI-039** (Heating System Structure, rubric Row 4). Log rows appended; the next-goal selection is a future owner-present act on the full gap picture.
4. **Row 4's written target stays P2 for now.** The deferral condition ("rides on row 1") is on record as fired; the raise rides the next deliberate rubric version (batched with the stale Rung C text, pending the owner's OQ6 policy ruling).
5. **Admin executed by the round agent, merge/push owner-held:** WI-037 closed and archived to `work/completed/20260902_WI-037_operating-point-closure/` (`pm close-item`; `tests/models` re-run green after the move); this close entry and the goal.md amendment written; CURRENT_WORK updated.

§ Answered when is met in the affirmative direction: a fresh non-author re-grade at `rubric.md@dc0f0b6d` scores R1.P = 3 with the anchor's full conjunction evidenced, the validation battery green (re-run by the round-2 reviewer and again at this close), and the affected-studies conjunct satisfied per ruling 2. Closed per § Close rule on the round-2 review's recommendation. Trail citations to `work/active/WI-037_operating-point-closure/` now resolve at the archived path above; per the magnet-closure precedent this amendment redirects them — no trail entry is edited.

**The goal's answer, for the record:** yes — the plasma operating point is now checked against the machine instead of merely typed in. A confinement relation (ISS04, executable at every point) links the computed field and installed heating to the density and temperature choice; a power limit with a fully computed operand pushes back, demonstrably structuring feasibility in a committed study; the fuel and ash densities are computed, not cited; and the one pathology the question grew from — field never rewarded, optimum pinned to the beta floor — is discharged with the optimum measured off the floor, bounded by sustainment and conductor technology instead.
