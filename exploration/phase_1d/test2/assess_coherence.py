...
def assess_one(
    concept: dict, template: str, model: str | None = None
) -> dict:
    ...
    return {
        "id": concept["id"],
        "concept": {col: concept[col] for col in DIFF_COLUMNS},
...
def main():
    parser = argparse.ArgumentParser(
        description="Assess coherence of random fusion concepts"
    )
    parser.add_argument(
        "--input", type=Path, default=DEFAULT_INPUT, help="Input JSON from generate step"
    )
    parser.add_argument(
        "--output", type=Path, default=DEFAULT_OUTPUT, help="Output assessments JSON"
    )
    parser.add_argument(
        "--prompt", type=Path, default=DEFAULT_PROMPT, help="Prompt template file"
    )
    parser.add_argument(
        "--model", type=str, default=None, help="Claude model to use (e.g. sonnet, haiku)"
    )
    parser.add_argument(
        "--start", type=int, default=1, help="Start from concept ID (for resuming)"
    )
    parser.add_argument(
        "--limit", type=int, default=None, help="Max concepts to assess (default: all)"
    )
    ...
    for i, concept in enumerate(concepts):
        ...
        print(f"[{seq}/{total - skipped}] Assessing concept #{concept['id']}...", end=" ", flush=True)
        ...
        try:
            ...
            print(f"coherent={coherent}, plausible={plausible}")
        ...
        # Write incrementally after each assessment (crash safety)
        output_data = {
            "model": args.model,
