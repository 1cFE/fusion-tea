# Integration Pass: Batch {{batch_number}} — {{batch_name}}
...
Also read:
- **Schema**: `{{schema_path}}`
- **Current table** (if exists): `{{table_path}}`
- **Current citations** (if exists): `{{citations_path}}`
...
Write the table to `{{table_path}}` as CSV. The columns should be:
...
For each (concept, column) pair, extract the citation and confidence level. Write to `{{citations_path}}` as a long-form CSV with one row per (concept, column) pair:
...
Write a checkpoint report to `{{checkpoint_path}}` with the following structure:

```markdown
# Checkpoint {{batch_number}}: {{batch_name}}
