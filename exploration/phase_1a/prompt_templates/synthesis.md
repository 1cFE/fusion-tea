# Dossier Synthesis: {{concept_name}}
...
1. **Current dossier**: `{{dossier_path}}`
   {{#if first_iteration}}(Does not exist yet — you will create it from scratch.){{/if}}
   {{#unless first_iteration}}(Read this file. It contains the current best knowledge.){{/unless}}

2. **New research**: `{{output_path}}`
   (Read this file. It contains findings from the latest research iteration.)

3. **Schema**: `{{schema_path}}`
...
Write the complete dossier to `{{dossier_path}}` using the structure below. This OVERWRITES the previous dossier — include all information, not just changes.
...
**Last updated**: {{date}}
**Iterations completed**: {{iteration_number}}
...
Append the following to `{{changelog_path}}`:
...
- **Published Machine/Plant?**: {{published_machine}}
