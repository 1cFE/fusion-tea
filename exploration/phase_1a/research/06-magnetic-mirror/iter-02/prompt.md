# Fusion Concept Research: Magnetic Mirror (p-B11)
...
1. **Start broad**: Search the web for "Pale Blue fusion technology" and similar queries. Look for the company's website, Wikipedia page, Fusion Industry Association profile, press releases, and investor presentations.
...
- **Value**: RF (ICRH)
- **Confidence**: low
- **Citation**: Inferred from alpha channeling mechanism and published physics (Kolmes et al. 2022 "hybrid fast and thermal proton scheme")
- **Notes**: The primary energy input is RF waves for alpha channeling — waves extract energy from fusion-born alpha particles and channel it into protons, maintaining a nonthermal distribution. The specific RF wave type/frequency is not publicly specified. Ion cyclotron range waves are the most likely mode based on historical alpha channeling physics, but lower hybrid or other modes are possible. NBI is not clearly part of the concept. Schema note: "RF (alpha channeling)" would be a more accurate descriptor for this concept.

### Energy Capture
- **Value**: Direct (charged particle)
- **Confidence**: medium
- **Citation**: Patent US20230298771 "Direct Energy Converter for Axisymmetric Mirror Fusion Reactor" (2023); ARPA-E presentation
- **Notes**: p-B11 fusion produces only charged particles (3 alpha particles per reaction, no neutrons). A Standing Wave Direct Energy Converter (SWDEC) has been patented that converts alpha particle kinetic energy directly to electricity via an RF device. Power from SWDEC is recirculated to maintain the radial electric field driving plasma rotation. The SWDEC patent (US20230298771) is from a potentially different inventor group — whether Pale Blue adopts SWDEC or develops their own DEC is not confirmed. The physics strongly favors direct conversion for p-B11.

### Plasma State
- **Value**: Confined
- **Confidence**: medium
- **Citation**: ARPA-E presentation; Princeton press release describing theoretical/early-stage research
- **Notes**: The concept targets a sustained, nonthermal plasma state — energetic protons maintained by alpha channeling while electrons and boron are kept cooler to minimize bremsstrahlung. This is neither a conventional burning plasma nor a simple confined plasma. `Sustained` could also fit given the active RF wave maintenance of the plasma, but `Confined` is more appropriate given the early theoretical stage and Q~1 target range. The concept is still working toward demonstrating Q > 0.

### Magnet Type
- **Value**: TBD
- **Confidence**: low
- **Citation**: No public specification from Pale Blue/Fisch group
- **Notes**: Mirror machines use solenoidal (axisymmetric) coils — simple wound geometry, not complex 3D shapes. The related CMFX experiment at UMD uses LTS superconducting magnets (3 T throat, 0.3 T midplane). For a reactor-scale device, HTS wound coils would be the likely choice given current industry trends, but the Pale Blue group has not disclosed magnet technology. Their publications focus on plasma physics, not engineering subsystems.
...
- **Notes**: The distinguishing technology bet combines: (1) centrifugal confinement via E×B plasma rotation providing species-selective confinement exploiting mass disparity, (2) alpha channeling using RF waves to extract energy from fusion-born helium and channel it into fuel protons, (3) ponderomotive barriers using static field perturbations for species-selective ion traffic control between chambers, and (4) multi-chamber architecture (fusion + heat exchange + plug) enabling spatial separation of reactants and products. Patents filed March 2025.

## Remaining Gaps

### Primary Heating (low confidence)
- **Searched**: ARPA-E presentation, published papers (Kolmes et al. 2022), Princeton press releases, patent filings
- **What would resolve it**: A detailed technical paper specifying the RF wave scheme (ICRH, lower hybrid, or other modes) for alpha channeling
- **Another iteration likely to help?**: Possibly — deeper search of Fisch group's RF physics papers could narrow the wave type

### Magnet Type (TBD)
- **Searched**: ARPA-E presentation, Princeton press releases, CMFX documentation
...
- **Another iteration likely to help?**: Unlikely — the group's publications focus on plasma physics, not engineering subsystems. May need to wait for company maturation.

### Plasma State (medium confidence)
- **Searched**: ARPA-E presentation, published physics papers
- **What would resolve it**: A published target Q value or explicit statement about plasma regime (burning vs. sustained vs. confined)
- **Another iteration likely to help?**: Possibly — a deeper dive into breakeven analysis papers might clarify the target operating point

### Energy Capture (medium confidence)
- **Searched**: Patent US20230298771, ARPA-E presentation, web searches
- **What would resolve it**: Confirmation that Pale Blue adopts the SWDEC approach (or discloses their own DEC design)
- **Another iteration likely to help?**: Unlikely — this is a company disclosure issue

### Company Status
- **Searched**: FIA membership, Wikipedia, Crunchbase, Tracxn, general web
- **What would resolve it**: Company website launch, FIA listing, or funding announcement
- **Another iteration likely to help?**: Possibly — a targeted search for recent (late 2025/early 2026) news

## Key Sources

1. **ARPA-E presentation** (July 2025): https://arpa-e.energy.gov/sites/default/files/2025-08/Day2_08_Fisch.pdf — Primary source for CHARM concept architecture, advantages, and status
...
4. **Princeton breakthrough article** (2025): https://www.princeton.edu/news/2025/12/17/search-next-big-breakthrough-princeton
5. **Patent US20230298771**: "Direct Energy Converter for Axisymmetric Mirror Fusion Reactor" (SWDEC concept, 2023)
6. **arXiv:2505.23047**: CMFX fusion yield measurements (2025)
7. **arXiv:2502.13300**: Ochs, Kolmes, Fisch — "Preventing ash from poisoning proton-boron 11 fusion plasmas"
8. **arXiv:2502.02008**: "Ponderomotive barriers in rotating mirror devices using static fields"
9. **arXiv:2504.18634**: Kolmes, Ochs, Fisch — "Ion Mix Can Invert Centrifugal Confinement"
10. **arXiv:2302.12346**: Ochs, Munirov, Fisch — "Confinement time and ambipolar potential in a relativistic mirror-confined plasma"
11. **CMFX at UMD**: https://ireap.umd.edu/research/centrifugal-mirror-fusion-experiment
