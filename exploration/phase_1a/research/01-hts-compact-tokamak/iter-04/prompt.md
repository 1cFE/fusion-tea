# Fusion Concept Research: HTS Compact Tokamak (D-T)
...
1. **Start broad**: Search the web for "Commonwealth Fusion Systems fusion technology" and similar queries. Look for the company's website, Wikipedia page, Fusion Industry Association profile, press releases, and investor presentations.
...
- **Notes**: The ARC baseline design uses a Rankine (steam) cycle at ~30% efficiency. sCO2 Brayton and combined cycles have been studied and could offer higher efficiency (up to 40%+). CFS has not publicly committed to a specific cycle for the commercial plant, but the published ARC paper specifies Rankine.
...
- **Value**: `Quasi-steady`
- **Confidence**: medium
- **Citation**: Sorbom et al. 2015; CFS blog on pulsed magnets ("ARC operations will pulse for longer, perhaps for tens of minutes at a time")
- **Notes**: The original ARC paper (2015) targets full steady-state via 25 MW LHCD + bootstrap current. More recent CFS communications describe ARC as pulsing "for tens of minutes at a time." `Quasi-steady` matches both the concept description and recent CFS statements. The steady-state aspiration may represent a future upgrade path. SPARC itself is clearly pulsed (10-second flat-top).
...
- **Notes**: Baseline CSV said "No" — corrected. SPARC is under construction; ARC is a published conceptual power plant design with an announced Virginia site.
...
CFS has not publicly committed to Rankine vs. Brayton for the commercial ARC plant. The 2015 paper uses Rankine as baseline, but sCO2 Brayton has been studied. A CFS statement on the ARC power conversion cycle would raise this to high. Searched: CFS website (JS-blocked), Wikipedia, Sorbom et al. 2015, ScienceDirect thermodynamic cycles paper.

### Operation Mode (medium confidence)
Tension between the 2015 paper's steady-state target and recent CFS statements about "pulsing for tens of minutes." A definitive CFS engineering statement on ARC's operational profile would clarify. `Quasi-steady` is the safe answer covering both interpretations. Searched: CFS blog, Sorbom et al. 2015, SPARC overview papers.

Both gaps are unlikely to be resolved by further web research — they require either a new CFS publication or a direct engineering disclosure. Another iteration is not recommended unless new CFS publications become available.
...
5. [SPARC (tokamak) — Wikipedia](https://en.wikipedia.org/wiki/SPARC_(tokamak))
6. [ARC fusion reactor — Grokipedia](https://grokipedia.com/page/ARC_fusion_reactor) — most detailed specifications source
7. [CFS Blog — SPARC Assembly](https://blog.cfs.energy/cfs-takes-its-next-step-toward-fusion-energy-assembling-the-sparc-tokamak/)
8. [CFS Blog — $863M Funding](https://blog.cfs.energy/how-863m-in-new-funding-fast-tracks-commercial-fusion-power/)
9. [CFS Blog — PIT VIPER Pulsed Power Magnets](https://cfs.energy/news-and-media/commonwealth-fusion-systems-second-breakthrough-superconducting-technology-handles-mammoth-pulses-of-power/)
10. [IEEE Spectrum — "This Fusion Reactor Is Held Together With Tape"](https://spectrum.ieee.org/fusion-2662267312)
11. [World Nuclear News — SPARC Assembly](https://www.world-nuclear-news.org/articles/assembly-starts-of-sparc-as-iter-cryopumps-completed)
12. [CFS Virginia Plant Announcement](https://fusionenergyinsights.com/blog/post/commonwealth-fusion-systems-to-build-world-s-first-commercial-fusion-power-plant-in-virginia)
