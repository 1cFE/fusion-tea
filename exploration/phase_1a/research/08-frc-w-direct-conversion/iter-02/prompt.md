# Fusion Concept Research: FRC w/ Direct Conversion (D-He3)
...
- **Name**: FRC w/ Direct Conversion (D-He3)
- **Company**: Helion Energy
- **Confinement approach**: Magnetized Target Fusion
- **Description**: Two FRC plasmoids collide and compress. Magnetic flux change in external coils enables direct electricity capture without thermal conversion. D-He3 fuel for near-aneutronic operation.
...
1. **Start broad**: Search the web for "Helion Energy fusion technology" and similar queries. Look for the company's website, Wikipedia page, Fusion Industry Association profile, press releases, and investor presentations.
...
- **Notes**: Combines magnetic confinement (FRC topology) with inertial compression (plasmoid collision and magnetic compression). Schema explicitly classifies pulsed FRC compression as MIF.
...
- **Notes**: Two FRC plasmoids formed, accelerated, collide, merge, then magnetically compressed. Distinct from TAE's beam-driven steady-state FRC. Helion's proprietary term is "Fusion Engine."

### Fuel
- **Value**: D-He3
- **Confidence**: high
- **Citation**: https://www.helionenergy.com/articles/explaining-helions-fusion-fuel-choice-d-he-3/
- **Notes**: Target commercial fuel is D-He3 (18.3 MeV per reaction: 3.6 MeV alpha + 14.7 MeV proton). Polaris will progress through D-D, D-T, and D-He3 during testing. He3 is self-bred from DD side reactions (50% direct He3, 50% tritium decaying to He3 with 12.3-year half-life). Helion states D-He3 releases "only 5% of its energy in the form of fast neutrons."

### Primary Heating
- **Value**: Magnetic compression
- **Confidence**: high
- **Citation**: https://www.helionenergy.com/technology/; DocsLib ARPA-E presentation
- **Notes**: Two-stage heating: (1) kinetic energy from FRC collision converts to ion thermal energy during merging, (2) magnetic compression via pulsed EM coils raises temperature to fusion conditions. Capacitor banks discharge through electromagnetic coils. No auxiliary RF or NBI heating.
...
- **Notes**: FRC plasma exists for ~milliseconds during each pulse cycle (formation, acceleration, collision, compression, fusion, expansion, energy recovery). Helion does not aim for ignition or self-sustaining burn — energy is recovered from each individual pulse.
...
- **Notes**: Aluminum coils pulsed with capacitor banks (>50 MJ, tens of kV). Not superconducting, no cryogenics. Compression fields >10 T demonstrated (Trenta), 15 T+ target (Polaris), 40 T reactor target. Cables use copper, aluminum, and custom alloys.
...
- **Notes**: Trenta operated at ~1 pulse per 10 minutes. Polaris targets 1 Hz. ARPA-E presentation shows 50 MW at 2 Hz as a design point. Long-term commercial targets may be higher (10 Hz or 60 Hz mentioned speculatively). Value of `~1 Hz` captures current prototype/near-term commercial target.
...
- **Notes**: System is fundamentally an RLC circuit. Capacitor banks (>50 MJ, tens of kV) discharge through aluminum electromagnetic coils to form, accelerate, and compress FRC plasmoids. Coils also serve as energy recovery system — expanding plasma induces current back into capacitors. Key specs: aluminum coils, >10 T (Trenta), 15 T+ (Polaris), 40 T reactor target.

## Remaining Gaps

All 12 differentiation columns are filled at high confidence. Minor areas where further detail could refine values:

- **Repetition Rate (commercial target)**: Near-term target (1 Hz Polaris) is well-supported, but commercial-scale target is less certain (2 Hz from ARPA-E presentation, speculative 10-60 Hz long-term). Another iteration could search for updated Polaris test results or Orion plant specifications.
- **Neutron energy fraction**: Helion claims 5%; schema default for D-He3 is ~10%. The difference depends on D-He3/D-D reaction ratio at operating temperature. A physics paper on D-He3 side reaction fractions at ~200 million C could clarify, but this does not affect the schema vocabulary value.
- **Published Machine/Plant?**: No detailed reactor engineering design (like ARIES or ARC) is public. Helion has announced Orion (50 MWe, target 2028) and a 500 MWe Nucor partnership, but without published engineering design documents. Could change if Helion publishes Orion specifications.
