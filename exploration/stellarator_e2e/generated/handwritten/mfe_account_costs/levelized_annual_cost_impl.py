...
Returns:
    tuple[float, ...]: (crf, levelized)

Example:
    >>> inputs = Levelized_Annual_CostInput(...)
    >>> crf, levelized = run_levelized_annual_cost(inputs)
    """
    a1 = (inputs.annual_cost * ((1.0 + inputs.inflation_rate) ** inputs.project_time))
    disc_pow_n = ((1.0 + inputs.interest_rate) ** inputs.operational_years)
    crf = ((inputs.interest_rate * disc_pow_n) / (disc_pow_n - 1.0))
    pv = ((a1 * (1.0 - (((1.0 + inputs.inflation_rate) / (1.0 + inputs.interest_rate)) ** inputs.operational_years))) / (inputs.interest_rate - inputs.inflation_rate))
    levelized = (crf * pv)
    return (
        crf,
        levelized,
