...
Returns:
    tuple[float, ...]: (wall_area, shield_vol, structure_vol, r_coil, blanket_vol, vessel_vol)

Example:
    >>> inputs = MFE_Radial_BuildInput(...)
    >>> wall_area, shield_vol, structure_vol, r_coil, blanket_vol, vessel_vol = run_mfe_radial_build(inputs)
    """
    vacuum_or = (inputs.a_in + inputs.vacuum_t_in)
    firstwall_or = (vacuum_or + inputs.firstwall_t_in)
    blanket_or = (firstwall_or + inputs.blanket_t_in)
    reflector_or = (blanket_or + inputs.reflector_t_in)
    ht_shield_or = (reflector_or + inputs.ht_shield_t_in)
    C = (((inputs.kappa_in * 2.0) * (inputs.pi ** 2)) * inputs.R_in)
    ht_shield_vol = (C * ((ht_shield_or ** 2) - (reflector_or ** 2)))
    blanket_layer_vol = (C * ((blanket_or ** 2) - (firstwall_or ** 2)))
    firstwall_vol = (C * ((firstwall_or ** 2) - (vacuum_or ** 2)))
    reflector_vol = (C * ((reflector_or ** 2) - (blanket_or ** 2)))
    structure_or = (ht_shield_or + inputs.structure_t_in)
    gap1_or = (structure_or + inputs.gap1_t_in)
    vessel_or = (gap1_or + inputs.vessel_t_in)
    coil_or = (vessel_or + inputs.coil_t_in)
    gap2_or = (coil_or + inputs.gap2_t_in)
    lt_shield_or = (gap2_or + inputs.lt_shield_t_in)
    lt_shield_vol = (C * ((lt_shield_or ** 2) - (gap2_or ** 2)))
    return (
        ((((inputs.kappa_in * 4.0) * (inputs.pi ** 2)) * inputs.R_in) * vacuum_or),  # wall_area
