...
class StellarisBetaOkConstraintModule(ModuleBase[StellarisBetaOkConstraintInput, StellarisBetaOkConstraintOutput]):
    ...
    def run(self, beta: float, beta_limit: float) -> ModuleResult[StellarisBetaOkConstraintOutput]:
        StellarisBetaOkConstraintInput(beta=beta, beta_limit=beta_limit)  # validate every resolved formal
        body = constraint_pred_definition_mfe_viability__beta_limit(beta=beta, beta_limit=beta_limit)
        ...
        return ModuleResult(
            data=StellarisBetaOkConstraintOutput(
                evaluation=ConstraintEvaluation(
                    ...
                    observed={"beta": float(beta), "beta_limit": float(beta_limit)},
                )
            )
        )
