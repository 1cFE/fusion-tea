...
*Basis**: Volume-based vessel-shell cost with gross-electric power law
    ...
    def validate_and_fill_default(
        self, unit_cost: float, vessel_vol: float, p_et: float, p_et_ref: float, alpha: float    ) -> Vessel_CostInput:
        ...
        """
        return Vessel_CostInput(unit_cost=unit_cost, vessel_vol=vessel_vol, p_et=p_et, p_et_ref=p_et_ref, alpha=alpha)

    def run(
        self, unit_cost: float, vessel_vol: float, p_et: float, p_et_ref: float, alpha: float    ) -> ModuleResult[Float]:
        """Execute calculation.
        ...
        """
        # Validate inputs
        validated_inputs = self.validate_and_fill_default(unit_cost, vessel_vol, p_et, p_et_ref, alpha)
