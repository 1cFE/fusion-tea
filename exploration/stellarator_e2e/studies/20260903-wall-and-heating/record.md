...
Written by the executor (the resuming round-1 session) after the fresh administrator's read (`synthesis.md`, which stamps `snapshot.json` digest `547958ab…`, unchanged). Every correction below was re-derived by the executor from `results/points.csv`, `results/oracle_operands.csv` and `results/window_scan.json` before it was written. No value in `results/`, `snapshot.json`, `indicators.json`, `axes.json`, `study.py` or `scan.py` changed. Per step 15, the sections above are not rewritten; this addendum supersedes the statements it names. The administrator's recount agrees with every § 3 headline, every § 4 count, every § 15 finding, the baseline row, both transects, the optimum and the shadow-column survivors (`synthesis.md` § 3); the five items below are prose slips.

1. **§ 6 `I_coil` — LCOE at 14.75 MA.** The fixed-(T, n) current series at eta 0.60, T 16 keV, n 1.0× prints "279.5" for 14.75 MA. The value is **279.127**; the 279.5 was an interpolation written where a data value belonged. The series reads 261.460 (14.0, infeasible) / 267.159 / 273.046 / 279.127 / 285.408 / 291.897 over 14.0 → 15.25 MA, and the claim — LCOE rises with current at fixed (T, n) — stands.
2. **§ 6 `n_e0` — the 1.2× column at 100 MW.** "Wall-blocked at 44 of 60 points" is wrong: `wall_load_ok` is violated at **57 of 60** points in the 1.2× column and at 45 of 60 in the 1.1× column. The 44 was a mis-summed subset of verdict combinations. The reading — density carries the machine into the wall-limited region — stands and is stronger.
3. **§ 4 and § 11 — how often `wp_stress_ok` fired in the scan.** "280 times of 6160 candidates" repeats the critique's F8 figure; `results/window_scan.json` shows **560** (280 at each wall-plug level, all at I 18 MA), never alone. The executed count (0 of 639) and the reading (the executed windows do not reach it) are unchanged.
4. **§ 6 `I_coil` — "I 14–15.4 MA is sustainment-blocked everywhere" at 100 MW.** 134 of the 144 points in that band fail `sustainment_ok`; the 10 exceptions are exactly the 10 wall-alone points the same sentence names. A wording slip, not a count.
5. **§ 6 `I_coil` — the 14.0 MA point in the current series.** The series labels the 14.0 MA value (261.460, `c0534`) "wall-blocked"; it violates `sustainment_ok` **and** `wall_load_ok`, as § 6 `I_coil`'s first paragraph and § 17 already state. The label is corrected to "infeasible (sustainment and wall)".

**What this addendum does not change:** every § 15 finding id, kind, disposition and home; every § 4 violation and alone count; the feasible counts; the constrained optimum and the shadow-bound survivor counts; the transect values; the § 12 identity of the 36 shared points; the snapshot and every digest in it.

## Addendum — 2026-09-04 (second) — stale prose in the digested `axes.json` (evidence untouched)

Written by the executor after checkpoint C-001.r1 of goal `wall-and-heating` round 1 found it. The `p_wallplug_heat` group note in `axes.json` — the declaration file preflight gated and `snapshot.json` digests — still reads "arm-couple-110 additionally sweeps it as 110/eta_source": the first design's arm name and coupled level, written before the first execution showed 110 MW sits below the anchor's own requirement (§ 15 #6). The executed arm is `arm-couple-132` at 132 MW coupled, as `study.py`, § 11 and § 15 record. The declaration's keys, provenance and constraint reach are unaffected; only the prose is stale. `axes.json` is left as digested and gated rather than edited after the fact, on the `20260821-power-cycle-ab#9` precedent; this addendum is the correction.
