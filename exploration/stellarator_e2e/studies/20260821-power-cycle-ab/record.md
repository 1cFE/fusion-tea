...
Written by the executor's reviewing session (RUN-STUDY Item 6, Phase 2 certification read, 2026-08-23). Nothing above this line, and nothing in `snapshot.json`, `indicators.json`, or `results/`, is changed. Two findings are added; no statement is corrected.

| Id | Kind | Finding | Disposition | Home |
|---|---|---|---|---|
| `20260821-power-cycle-ab#10` | process | The study's governing quantity, the recirculating fraction, has no per-point evidence on the package side (finding `#5`), and the record's one interaction result — the η gain is largest where recirculation is heaviest — rests on five oracle-scan points (`results/oracle_scan.json`). The executor had the oracle and could have emitted an oracle-derived, labelled operand column for all 3,792 points before verification, turning an argument into data. | Not added here (results are frozen). From the next study on: when the store does not record a predicate operand, the executor emits it from the oracle as a separate artifact, named as oracle-derived, before step 10; the runbook gains that sentence at Item 6 close (design I1 bars the edit inside the item). | runbook step 9; applied as executor practice in study 1 |
| `20260821-power-cycle-ab#11` | process | The study store (`_work/`) lives inside the record directory. It is gitignored and the administrator reported not opening it, but the record contract says the administrator reads "the record directory and nothing else", and the store was inside it. The committed directory and the directory on disk were not the same thing. | Not moved (the store path is in `snapshot.json` `stores[].path` and the verification summary's digest; moving it would orphan both). From the next study on: `study.py` `run()` places `_work/` beside the record directory, not inside it, and `stores[].path` records that. | study definition convention (the study definition file beside each record); `studies/.gitignore` already covers `**/_work/` |
