...
## Addendum 2026-08-23 (certification)

Written by the reviewing session (RUN-STUDY Item 6, Phase 3 certification read). Nothing above this line, and nothing in `snapshot.json`, `indicators.json`, or `results/`, is changed. One statement is qualified and one finding is added.

**Qualification of § 6 (`arm-nb3sn` has no feasible point).** The closing node, (4.69 T, 0.50×), sits 0.016 above the recirculation threshold (rec_frac 0.516) with the two cryogenic assumptions held at the values least favourable to Nb3Sn inside their sourced ranges: `f_carnot_cryo` 0.20 (DI-009's band is 0.22–0.30) and `vol_cold_cryo` 390 m³ (the top of the derived 285–390 m³ range at the ceiling field). Re-evaluating that node through the package-owned oracle seam (`oracle_entry._compute`, the same arithmetic `verify.py` checks the package against) across every sourced combination gives rec_frac 0.516 / 0.511 / 0.511 / 0.508 / 0.507 / 0.504 for (`f_carnot`, `vol_cold`) = (0.20, 390) / (0.20, 285) / (0.24, 390) / (0.24, 285) / (0.30, 390) / (0.30, 285). **The arm stays empty at grid resolution in every case**, so the headline does not depend on the held values; but the margin narrows to 0.004, and under the most favourable sourced values a finer density grid between 0.500× and 0.505× could admit a single feasible sliver. The statement that the two fences cross inside one grid step stands; "no feasible point" is a statement at this grid and these held values, not a boundary claim between nodes. These numbers are oracle-side, not package evidence (no package point was run for them).

| Id | Kind | Finding | Disposition | Home |
|---|---|---|---|---|
| `20260823-magnet-technology-ab#11` | process | The headline of an A/B can rest on a knife-edge against a held assumption (here 0.004–0.016 of recirculating fraction against the cryogenic fraction-of-Carnot and cold volume). The record disclosed the held values and their ranges but did not quantify the headline's sensitivity to them; the certification read did, after the fact, with the oracle. | Quantified above. From the next study on: when a headline verdict sits within one grid step of a fence, the executor re-evaluates that node across the sourced range of every held value the fence depends on, and states the margin, before committing. | runbook step 11 (executor practice; sentence at Item 6 close) |
