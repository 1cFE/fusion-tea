"""Apply canonical blanket energy multiplication (mn) to D-T concept model_setup.py files.

Reads Fuel from table.csv, looks up the canonical mn from
lib.canonical_params.canonical_mn(), and updates the corresponding mn / MN
value in each D-T concept's model_setup.py.

Usage:
    uv run python exploration/concept_analysis/scripts/standardize_mn.py            # dry-run
    uv run python exploration/concept_analysis/scripts/standardize_mn.py --apply    # apply edits

Behavior:
- D-T only — non-D-T fuels have no canonical mn (skipped with note)
- Skips lines containing "# DEVIATION:" (Tier-A retains per scoring_framework.md
  §"Blanket energy multiplication" — sourced HCPB+Be, FLiBe, physics-coupling
  arguments, etc.)
...
Mirror of standardize_availability.py / standardize_eta_th.py.
"""
...
from lib.canonical_params import canonical_mn  # noqa: E402
...
#   mn=1.1,
#   mn: float = 1.1
# Does NOT match: mn=MN (RHS identifier), positional tuple elements, or any
# token where "mn" is part of a longer identifier (e.g. "umn", "mn_max").
MN_PATTERN = re.compile(
    r"(?P<prefix>\s*(?<![A-Za-z0-9_])(?:mn|MN)\b"
    r"(?:\s*:\s*[A-Za-z][\w\[\], ]*)?\s*=\s*)"
    r"(?P<value>\d+\.?\d*)"
    r"(?P<suffix>[,\s]*)"
    r"(?P<comment>(?:#[^\n]*)?)"
...
def find_mn_lines(model_path: Path) -> list[tuple[int, str, float]]:
    """Return list of (line_number, line_text, value) for each non-DEVIATION
    mn assignment line. Filters to range 0.9–2.0 to avoid false-positive matches
    on unrelated parameters that happen to share the prefix.
    """
    results = []
...
def update_model_file(model_path: Path, new_value: float) -> int:
    """In-place update mn assignments to new_value (excluding DEVIATION lines
    and lines already at canonical). Returns count of lines modified.
    """
    text = model_path.read_text(encoding="utf-8")
    new_lines = []
    modified = 0
    ...
    for line in text.splitlines(keepends=True):
        ...
        replaced = (
            ...
            + "(Blanket energy multiplication: D-T canonical)"
            + line[m.end():]
        )
...
def main() -> None:
    ...
    for cid, fuel in sorted(fuels.items()):
        ...
        if not model_path.exists():
            ...
            continue
        try:
            canonical = canonical_mn(fuel)
        except ValueError:
            rows.append((cid, fuel, None, None, None, "non-D-T (no canonical mn)"))
            continue

        lines = find_mn_lines(model_path)
        if not lines:
            rows.append((cid, fuel, canonical, None, None,
                         "no mn line found (or all DEVIATION-marked)"))
            continue
