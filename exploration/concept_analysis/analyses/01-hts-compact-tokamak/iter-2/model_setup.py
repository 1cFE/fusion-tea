...
overrides = [
    ...
    {"account": "C220108", "value": 17.5 * 1.33, "enabled": False,
     "provenance": "derived", "source": "arc-reactor-specifications.md §6.3",
     "rationale": (
         "DISABLED (F-1): the 2015 design defers the divertor ('left as an open "
         "question') and gives only a rough estimate: a 2 cm tungsten divertor over "
         "~20% of first-wall area, ~$500k materials -> ~$17.5M fabricated (FY2014), "
         "x 1.33 = $23.3M. Enabling this *replaced* the library default ($56.3M) with "
         "a LOWER number ($23.3M) — i.e. it departed from the library in the "
         "non-conservative direction on the basis of an explicitly incomplete "
         "placeholder, while the narrative rates ARC's divertor difficulty as 'between "
         "ITER and reactor designs' (likely MORE expensive than a generic scaling "
         "default, not less). An override should be better-grounded than the default "
         "it replaces; this one is not. Disabled so the library default stands and the "
         "divertor is carried as the Section-6 data gap it already is (Data Gap #2). "
         "Entry retained (disabled) to document the rejected figure.")},
]
