...
**6. Divertor is undesigned (Impact: Moderate).** The 2015 paper explicitly defers the divertor ("the physical divertor design was left for later study"), pricing only a $17.5M rough tungsten placeholder [§5.1, §6.3]. Heat-flux scaling places ARC's divertor difficulty "between ITER and reactor designs" — i.e. likely *more* expensive than a generic scaling default, not less. The $17.5M placeholder is therefore **not** used to override the C220108 account (doing so would lower the account below the library default on an admittedly-incomplete number); the library default stands and the divertor is carried as a data gap, to be revisited as an upward sensitivity (see Data Gap #2).
...
Per-account walkthrough result. The ARC 2015 paper publishes a subsystem cost table in FY2014 USD; this is the rare case where the design-point source grounds several accounts directly. All escalations use **CPI-U FY2014→2024 ≈ 1.33** (236.7 → 313.7). **Three accounts are enabled as overrides** (C220103, C220101, CAS27); a fourth (C220108 divertor) is recorded as a *disabled* candidate because the only ARC-grounded figure is an incomplete placeholder that would move cost the wrong way (see its entry below and Data Gap #2). The remaining canonical accounts (C220102 shield, C220104 heating, C220105 primary structure, C220106 vacuum, C220107 power supplies, C220110 remote handling, C220111 installation, CAS21/23/24/26, CAS70/80) are **not** overridden — either the paper gives no separable figure, the figure is small enough to sit within the library default, or the account is library-owned. Account count (3 enabled) is within the High archetype-fit band (0–4).
...
overrides:
  ...
  - account: C220108
    ...
    rationale: |
      DISABLED — kept as a documented candidate, not an active override. The 2015 design
      defers the divertor ("left as an open question") and gives only a rough estimate: a
      2 cm tungsten divertor over ~20% of first-wall area, ~$500k materials -> ~$17.5M
      fabricated (FY2014). x 1.33 = $23.3M. Enabling this would *replace the library
      default ($56.3M native) with a lower number ($23.3M)* — i.e. depart from the
      library in the non-conservative direction on the basis of an explicitly incomplete
      placeholder for an undesigned subsystem. That violates override discipline: a
      departure should be better-grounded than the default it replaces, not less complete,
      and ARC's divertor difficulty is rated "between ITER and reactor designs" (likely
      *more* expensive than a generic scaling default, not less). The library default
      therefore stands, and the divertor is carried as Data Gap #2 — to be revisited as an
      *upward* sensitivity toward ITER-class divertor cost once an ARC divertor concept is
      published, never as this downward point override.
```
...
| 2 | Divertor undesigned — only a $17.5M placeholder exists; library C220108 default ($56.3M native) stands, override disabled | S2, S3, S5b | not-yet-sourced / truly-unknown | important | Apply ITER/SPARC divertor cost scaling once an ARC divertor concept is published; model as an *upward* sensitivity toward ITER-class cost, not a downward point override |
...
**Reconciling the compactness advantage with a high modeled $/kWe.** The "ARC advantage" above is a *fusion-power-density* advantage versus the large low-field path (ITER's ~$24B at 5.3 T), and it must not be read as a low cost of electricity. When the structure-dominated magnet override and ARC's modest engineering gain are honestly costed, the model lands ARC's 1 GWe cross-concept point at **≈ 544 $/MWh and an overnight cost of ≈ 51,900 $/kW (~$51.9/W)** (native design point: ≈ 199 $/MWh at 16,092 $/kW) — roughly 10–20× the comparator point designs cited here (ARIES-AT ≈ 50 $/MWh; ALPHA re-costing ≈ 2.4 $/W). This is the *expected* consequence of the well-grounded C220103 override, **not** a discrepancy to be deflated. Two effects convert the compactness win into a high $/kWe: (1) the magnet account is dominated by the $4.6B structural-steel cage reacting the 23 T peak field (§2), so the compact machine is not a cheap machine; and (2) the advantage is per-unit-*fusion*-power, eroded per-unit-*electric* by ARC's low engineering gain (Qe = 3.5) and by the modular, non-scaling penalty of stacking ~4.3 × 233 MWe units to reach the 1 GWe comparison basis (no economy-of-scale credit across units). The compactness benefit is therefore real against the low-field path on a $/W-of-*fusion* basis, but it does not survive translation to $/kWe — and the model correctly reflects that rather than contradicting the narrative.
