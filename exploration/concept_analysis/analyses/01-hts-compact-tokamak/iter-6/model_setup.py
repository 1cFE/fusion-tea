...
Modeling approach:
    ...
    Indirect cost framing (iter-6 update):
    ...
    FOAK scenario (iter-6 update):
    The NOAK central estimate (noak=True, C220103 at paper value) is paired with a
    discrete FOAK scenario that applies a 2× multiplier to the nuclear island
    magnets cost and sets noak=False. CATF IWG methodology (arxiv-2602-19389 §2.1.5)
    cites FOAK fusion plants at 150–200 $/MWh vs. 60–100 $/MWh NOAK — this is a
...
# Source: arc-reactor-specifications.md §4.1, §6; sciencedirect-science-article-pii-s2772830725000390.md §Introduction
...
# (no published estimate exists; analysis.md §6, gap #15 — truly-unknown additive line).
...
result = model.forward(
    ...
    lifetime_yr=30,               # Standard 30-yr plant lifetime
                                  # NOTE: TF coil fluence limit = 9 FPY; coil replacement
                                  # required at ~9 FPY — not yet costed. arc-reactor-specifications.md §5

    n_mod=1,                      # Single-module plant
    construction_time_yr=5.0,     # DEFAULT — compact; shorter than large LTS tokamak (6 yr default)
    interest_rate=0.07,
...
6. Indirect cost structure: framework defaults encode ~29% of TCC for indirect/owner/
   ...
   Reference LCOE benchmark: 55.1 $/MWh at 636.75 MWe, 0.9 availability, 30-yr life
   (FECONS illustrative DT tokamak; arxiv-2601-21724 §8.3 Table 3).
...
7. FLiBe special materials: $146 M$ (~2025 USD).
   ...
   NOTE: FLiBe chemistry plant + tritium extraction system capital cost is NOT included
   (no published estimate; analysis.md §6, gap #15 — truly-unknown additive BOP line).
