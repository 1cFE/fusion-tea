...
The **primary sensitivity axes** in order of leverage are:
1. **3D coil cost multiplier** (CAS21): Range 1.5–5× wound-coil tokamak magnet cost; SMC demo data (2027) is the first real data point. This single parameter dominates the competitiveness question.
2. **Capacity factor** (availability): Steady-state, disruption-free operation is the primary potential offset to higher CAPEX. Range 85–95% [analogue: Helios targets 88%; W7-X demonstrated >97% run-time availability in experimental context, though plant availability will be lower due to blanket/divertor maintenance].
...
**H2 (Capacity Factor Advantage):** Stellaris achieves a capacity factor of ≥ 88% (≥ 3–5 percentage points above a disruption-limited tokamak reference of ~83–85%), sufficient to offset the low-beta CAPEX penalty at a coil cost multiplier of ≤ 1.5×. The 88% target is anchored in the Helios design [helios-stellarator-comparison.md §2] and the disruption-free physics argument; however, blanket and divertor scheduled maintenance sets the practical ceiling, and no published availability model exists for Stellaris. If capacity factor advantage is ≤ 2 percentage points, it does not compensate for the coil cost premium even at the optimistic end.
...
- **Demonstrated**: W7-X island divertor demonstrated steady-state operation with strong detachment at low power density, showing large wetted areas and reduced heat flux concentration. Tungsten divertor target fabrication at W7-X scale.
...
- **Missing at scale**: Long-term tungsten erosion and redeposition behavior in an island divertor under fusion-relevant neutron flux and plasma heat flux. Divertor replacement scheme within the constrained geometry of the Stellaris remote maintenance architecture [stellaris-design-details.md §2.11].
...
| **Capacity factor** | **85–95%** | **[analogue: helios-stellarator-comparison.md §2; disruption-free steady-state argument]** | **medium** | **Helios design targets 88%; W7-X demonstrated >97% experimental run-time; plant availability floor set by blanket/divertor scheduled maintenance. Disruption-free operation removes a major tokamak availability-limiting event.** |
...
|---|-----------------|---------|----------|-------------|----------------------|
| 1 | Capital cost estimate for Stellaris plant | S1, S5 | proprietary | blocking | Proxima internal; ARIES-CS LTS stellarator studies as floor analogue; Brown (2018) for relative account weights |
...
|-------------|-----------|-----------|-------|-------|
| **CAS21: Magnet System** | **Large +** | **1.5–5×** | Brown (2018); no commercial HTS stellarator precedent | 3D non-planar coil geometry vs. D-shaped wound coils; primary LCOE uncertainty. Includes higher cryogenic load (111 MW conduction). |
| **CAS22: Heating & Current Drive** | **Large −** | **−50% to −80%** | helios-stellarator-comparison.md §3.1 (1 MW ignited) | ECRH only; no NBI, no ICRF, no CS; no current drive needed. Conditional on H4: if 50 MW sustained, advantage largely disappears. No CS cost is a firm saving. |
| **CAS23: First Wall / Blanket** | **Small +** | **5–15%** | stellaris-design-details.md §2.7, §2.8 | 3D curved tungsten tile fabrication premium over flat tokamak tiles; WCLL complexity adapted to helical geometry. WCLL vs. FLiBe: WCLL likely somewhat simpler than ARC-class FLiBe (lower temperature, water coolant). Net: small positive delta due to geometry, partially offset by blanket simplicity. |
...
| **CAS25: Power Conversion** | **Small −** | **~−5–10%** | [inferred: ~32% vs. potential 38–40% for sCO₂ | Paradoxically negative at the system level: EUROFER97 temperature limit constrains steam to ~500°C, producing ~32% gross thermal efficiency vs. potential 38–40% for sCO₂. Lower efficiency means less gross power from the same thermal plant, reducing BOP capital cost per MWe of thermal input but reducing net output. Steam Rankine is a well-understood, lower-capital technology. |
...
| **CAS20: Land / Site** | **0 (neutral)** | — | Proxima targeting Gundremmingen (former nuclear site) | Decommissioned nuclear site reuse may reduce site prep cost; not unique to stellarator. |
| **Net directional assessment** | **Uncertain** | Dominated by CAS21 vs. CAS22 | The competitiveness case depends entirely on whether the 3D coil manufacturing premium (large +) is more than offset by: the H&CD saving (large −, conditional on ignition) + the capacity factor advantage (not in table — affects LCOE denominator, not capital cost). |
...
8. **dossier.md** — Phase 1a research summary: 2 iterations, 12 differentiation columns, overall confidence medium. 8 high-confidence values, 4 medium-confidence values. Summary statistics: peak fusion power 2.7 GW, net electrical ~1 GW, W7-X heritage confirmed, HTS REBCO magnets at 20 T, WCLL TBR 1.07, steady-state disruption-free operation. Location: `exploration/phase_1a/research/09-qi-stellarator-hts/dossier.md`.
