...
Modeling approach:
    ...
    (H2a) Higher-beta QI design: follow-on 4% beta scenario per CIEMAT-QI4X finding
        (arXiv:2512.08825, December 2025). At 4% beta vs. 2.76%, plasma volume at fixed
        fusion power density scales down by ~31%, reducing first wall area, blanket mass,
        and vacuum vessel scale proportionally. H4-true ECRH assumption used. This is a
        scenario branch for next-generation QI designs, NOT a Stellaris v1 variant.
    ...
    iter-6 updates vs. iter-5:
    1. Replacement-inclusive LCOE surfaced as the primary headline figure (not appended
       as a footnote sweep). The raw initial-build LCOE is demoted to a secondary line;
       replacement cost is computed and displayed inline with each scenario.
       Basis: analysis.md §7 Cross-Concept Notes (final footnote): "The raw $106/MWh is a
       stellarator-specific lower bound. Omitting it biases the cross-concept comparison."
    2. CAS account structure note added to docstring and output (matching analysis.md §7
       clarification: CAS21 = Buildings, not Magnets; coils live at C220103 within CAS22).
    3. Replacement cost calculation generalized across coil-cost multiplier sweep to
       produce replacement-inclusive LCOE for each multiplier point.
...
_R0_H2A  = 11.6   # m — UNCERTAIN: H2a higher-beta scenario
...
_AVAILABILITY      =  0.88      # capacity factor (matching _SHARED below)
_ANNUAL_ENERGY_GWH = _P_NET_MWE * _AVAILABILITY * 8760.0 / 1e3  # GWh/yr


def _repl_lcoe(c220103_m: float) -> float:
    ...
    return annual_repl_cost_m * 1e6 / (_ANNUAL_ENERGY_GWH * 1e3)         # $/MWh
...
print(f"\nScenario H2a: Higher-beta QI design (4% beta, R0≈{_R0_H2A} m, smaller machine)")
...
# iter-6 addition: replacement-inclusive LCOE reported for each multiplier point,
# making the total lifecycle cost visible for cross-concept comparison.
...
11. CAS-level directional deltas vs. 01-hts-compact-tokamak reference (analysis.md §7):
    ...
    Net directional outcome: UNCERTAIN. Dominated by C220103 coil cost vs. C220104 saving.
...
12. Scenario H2a: Higher-beta QI design (4% beta) — iter-5 addition retained
    CIEMAT-QI4X (arXiv:2512.08825) demonstrated a QI configuration maintaining island
    divertor compatibility, small neoclassical transport, and good alpha confinement at
    beta up to 4%. This is a SCENARIO BRANCH for next-generation QI designs, not a Stellaris
    variant. At 4% vs. 2.76% beta, plasma volume scales down by ~31% (linear with beta at
    fixed fusion power and power density), reducing first wall area, blanket mass, and vacuum
    vessel scale proportionally and partially closing the gap to compact tokamak power density.
    The 3D coil manufacturing premium (C220103) remains as the primary cost differentiator.
    Source: analysis.md §2, H2a; arxiv-2512-08825.md (CIEMAT-QI4X, December 2025).
""")
