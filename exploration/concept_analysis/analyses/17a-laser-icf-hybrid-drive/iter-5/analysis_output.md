...
- **C220108** (Target factory — IFE/MIF target manufacturing): Xcimer targets use liquid DT + plastic ablator (no gold hohlraum, no cryogenic ice layer). The whitepaper states the lower rep rate "significantly reduces capsule fabrication costs" but provides no target cost per unit, no factory cost, and no manufacturing process details. The dossier comparison table notes "not publicly specified" for Xcimer target costs. **Derived override included (enabled, low confidence):** the structural difference — ~22M targets/yr at 0.7 Hz vs. ~315M targets/yr for a 10 Hz comparable, plus simpler target construction — grounds a 0.60× analyst estimate. No company cost figure; the multiplier is a sensitivity parameter.
...
**Override count:** 7 enabled overrides. The archetype-fit is Low, expecting 6–12 enabled overrides. The count of 7 falls within the expected band. Of these, only C220104 and C220107 have direct provenance; the remaining 5 are derived with analyst estimates, reflecting the dossier's rich qualitative descriptions but sparse cost quantification. CAS70 (O&M) is omitted from the registry because it is not currently overridable in 1costingFE — overrides on CAS70 are silently dropped by the framework.
