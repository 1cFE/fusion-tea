# 1costingfe Model Setup: Electrostatic Hybrid (Orbitron)

You are generating a runnable 1costingfe model setup script for **Electrostatic Hybrid (Orbitron)**
(Avalanche Energy). The script must run via `uv run python model_setup.py` and emit an
LCOE estimate.
...
**Start here.** Open `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\13-electrostatic-hybrid\analysis.md` and read its `## Design Point` block and
...
3. **`spec` carries only design-point inputs.** Do NOT put the uniform
   ...
   **Archetype-specific spec key blocklist (workarounds for known library
   ...
   - **DIPOLE** (`ORBITRON == "DIPOLE"`): do **NOT** pass
   ...
   - **MIF concepts** (`ORBITRON ∈ {MAG_TARGET, MAGLIF,
...
## Output
Write the script to: `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\13-electrostatic-hybrid\iter-1\model_setup.py`
