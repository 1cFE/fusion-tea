...
Wired as a true hybrid: η_th = 0.44 (thermal channel), η_DEC = 0.44 (alpha DEC channel),
f_dec = 0.30 (charged-particle fraction); pulsed_thermal_forward handles both natively.

Modeling approach:
  - Native design point: ~2800 MWe net at 10 Hz (BLF paper Table 2 states 2.8 GWe)
  - pulsed_conversion = THERMAL; hybrid via f_dec=0.30, eta_de=0.44 (pulsed_thermal_forward
    lines 591-604); both channels are live auto-diff inputs
...
Key deviations from defaults:
  - eta_th = 0.44 (vs RANKINE default 0.40): He Brayton + Li-breeding boost (thermal channel)
  - eta_de = 0.44: alpha DEC channel efficiency (Rax et al. 2025 theory, TRL 1–2)
  - f_dec = 0.30: charged-particle fraction of P_fus routed to DEC; 70% goes thermal
...
_SHARED_KWARGS = dict(
    ...
    # pulsed_conversion = THERMAL uses pulsed_thermal_forward, which natively supports
    # partial charged-particle direct capture via f_dec and eta_de (lines 591-604).
    # This replaces the former ETA_TH_COMBINED hand-fold: both channels are live inputs.
    pulsed_conversion=PulsedConversion.THERMAL,
    eta_th=ETA_TH,                   # 0.44; He Brayton + Li-breeding thermal channel
    eta_de=ETA_DEC,                  # 0.44; alpha DEC channel (Rax et al. 2025, TRL 1–2)
    f_dec=F_CHARGED,                 # 0.30; charged-particle fraction → DEC channel
...
print(f"  η_th (thermal)    = {ETA_TH:.0%}       MEDIUM — He Brayton + Li-breeding thermal channel")
print(f"  η_DEC (direct)    = {ETA_DEC:.0%}       LOW    — alpha DEC efficiency; Rax et al. 2025, TRL 1–2")
print(f"  f_dec (DEC frac)  = {F_CHARGED:.0%}       DEC channel share (30% charged / 70% thermal)")
...
# DEC elasticity sweep: eta_de is a live auto-diff input via pulsed_thermal_forward.
# Thermal channel (η_th=0.44, f_neutron=0.70) is held fixed; only eta_de varies.
# Non-zero LCOE variation across the sweep confirms the DEC channel is wired and live.
print("\nDEC Efficiency Sensitivity (η_th=0.44 fixed; direct eta_de sweep via pulsed_thermal_forward):")
print(f"  {'η_DEC':>6} {'LCOE $/MWh':>11} {'Δ LCOE $/MWh':>14}")
print("  " + "-" * 34)
_ETA_DEC_SWEEP = [0.20, 0.30, 0.35, 0.40, 0.44, 0.50, 0.55]
for _eta_dec_val in _ETA_DEC_SWEEP:
    _kw_dec = dict(_SHARED_KWARGS)
    _kw_dec["eta_de"] = _eta_dec_val
    ...
    print(f"  {_eta_dec_val:>6.2f} {_lcoe_dec:>11.1f} {_delta:>+14.1f}{_marker}")
print("  DEC handles 30% of P_fus (charged-particle channel, f_dec=0.30).")
print("  Non-zero variation confirms eta_de is a live input in the framework-native hybrid wiring.")
...
def _p_net_same_laser(G, f_rep,
                      e_uv_mj=5.0, eta_pin=ETA_PIN, eta_th=ETA_TH, mn=MN,
    ...
    p_gross = p_fus * eta_th * mn  # thermal channel only (≈ hybrid at ETA_TH = ETA_DEC)
