...
| Blanket thermal efficiency (η_th; model: `eta_th`) | 44% (0.44) | optics-express-2025-paper.md §Table 2 | medium | Thermal-channel-only: η_th = 0.40 (He Brayton) + 0.04 (exothermic Li-breeding boost); handles 70% of P_fus via neutron energy absorbed in LiPb blanket. Sandia VHTR analog (Wright et al. SAND2006-4147) reports 42.8% simple recuperated and 45.8% first-IHC at 1190 K, bracketing 44% as near-simple-cycle design [osti-servlets-purl-1323907.md §6] |
| Direct energy conversion efficiency (η_de; model: `eta_de`) | 44% (0.44) | optics-express-2025-paper.md §Table 2 | low | Separate direct-channel parameter; handles 30% of P_fus (alpha particles + plasma exhaust magnetically guided to DEC electrodes). Described as "conservative"; theoretical basis (Rax et al., 2025); never demonstrated at any scale |
| Combined plant efficiency (η_e; derived) | 44% (0.44) | optics-express-2025-paper.md §Table 2 | medium | Derived: η_e = f_neutron × η_th + f_charged × η_de = 0.70×0.44 + 0.30×0.44 = 0.44. Both channels are separate native framework inputs (`eta_th`, `eta_de`, `f_dec`); they coincidentally share the same efficiency value at this design point. If η_de falls, net output falls proportionally — the 30% DEC channel represents ~840 MWe at the 10 Hz design point |
...
| Neutron energy to blanket fraction (model: `1 − f_dec`) | ~70% | optics-express-2025-paper.md §Table 2 | high | Author explicitly states 70% neutron fraction directed to thermal blanket (f_neutron = 1 − f_dec = 0.70) |
| Charged-particle energy fraction (model: `f_dec`) | ~30% | optics-express-2025-paper.md §Table 2 | high | Alpha particles + plasma exhaust directed to DEC via magnetic guide; separates the DEC channel from the thermal channel in the framework's native hybrid formula |
