# 1costingfe Model Setup: Projectile ICF (First Light Fusion)

You are generating a runnable 1costingfe model setup script for **Projectile ICF (First Light Fusion)**
(First Light Fusion). The script must run via `uv run python model_setup.py` and emit an
LCOE estimate.
...
**Start here.** Open `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\22-projectile-icf\analysis.md` and read its `## Design Point` block and
...
```python
"""1costingfe model: Projectile ICF (First Light Fusion) (First Light Fusion).
...
3. **`spec` carries only design-point inputs.** Do NOT put the uniform
   ...
   **Archetype-specific spec key blocklist (workarounds for known library
   ...
   - **DIPOLE** (`HEAVY_ION == "DIPOLE"`): do **NOT** pass
   ...
   - **MIF concepts** (`HEAVY_ION ∈ {MAG_TARGET, MAGLIF,
...
## Output
Write the script to: `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\22-projectile-icf\iter-1\model_setup.py`
