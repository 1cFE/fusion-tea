...
**C220101 — First wall, blanket & neutron multiplier**: The liquid lithium curtain design is distinctive — 1-meter-thick flowing lithium serving simultaneously as blanket, shield, and first wall. No dollar figure is published for this subsystem. The concept eliminates solid first-wall replacement (claimed "lifetime-of-plant vessel") which would reduce this account relative to solid-wall IFE designs. However, no company-grounded quantity or unit cost justifies a specific override value. **No override.**

**C220102 — Radiation shield**: The lithium curtain itself performs the shielding function; no separate radiation shield structure is described. This suggests this account could be near-zero (shielding integrated into C220101). However, no published cost figure exists. **No override** (insufficient quantitative basis).
...
```yaml
  - account: C220108
    ...
    rationale: |
      ...
      Library default (high-rep-rate IFE target factory) is inapplicable at 1M/year volume.
```

**C220110 — Remote handling & maintenance**: First Light claims the vessel never needs internal replacement due to lithium shielding. If true, remote handling requirements are substantially reduced compared to tokamaks or dry-wall IFE. However, target loading and projectile gun maintenance would still require access systems. No published cost. **No override.**
...
Enabled overrides: 4 (C220104, C220107, C220108, CAS27).

Expected band for Low archetype-fit: 6–12 enabled overrides.

**Discrepancy note**: The count of 4 falls below the expected band of 6–12. This reflects the severe data limitation for this concept — First Light published almost no subsystem-level cost data before pivoting away from projectile ICF. The available overrides represent the only accounts where company-grounded quantitative evidence exists. Padding to 6 would require inventing values not supported by the dossier, which violates the anti-hallucination rules. The low override count is an honest reflection of the "Limited" data availability rating and "low" grounding confidence.
...
| 5 | Target manufacturing cost at volume | S4, S5b | truly-unknown | important | No published estimate; Hawker (2020) provides parametric framework |
...
**Contextual positioning**: While no formal comparable is assigned, the concept can be informally positioned relative to other IFE approaches in the landscape:

- **vs. Laser ICF concepts (Xcimer, Inertia, Focused Energy)**: Projectile ICF replaces the laser driver (at $60–$700/J) with an electromagnetic gun ($1.7–$2/J). This is a >30× reduction in driver cost per joule — the fundamental economic thesis. The tradeoff is that the projectile approach requires extremely high target gain (>200) to compensate for the low repetition rate, whereas laser IFE can achieve economic power output at gain 30–100 with 1–10 Hz rep rates.
...
- **Unique features with cost implications**:
  - Sub-Hz operation (0.033 Hz) — drastically reduces target factory requirements but demands very high yield per shot
  - Claimed "lifetime-of-plant vessel" from lithium shielding — eliminates first-wall replacement cost if validated
