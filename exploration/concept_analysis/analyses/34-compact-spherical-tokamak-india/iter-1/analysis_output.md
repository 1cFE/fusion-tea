...
**Peer-reviewed publications**: None found for Pranos Fusion specifically.
...
**Phase 1a dossier completeness**: High confidence on confinement family, confinement concept (spherical tokamak), fuel (D-T), and operation mode (steady-state). Low confidence on magnet type, primary heating, plasma state, and tritium breeding approach. Driver technology is unknown. Most gaps are structural — the company is in the computational design phase and has published nothing technical beyond the IAEA FUSE profile and web-facing descriptions.

**Key data gaps limiting this analysis**:
1. No plasma parameters published — Q, fusion power, plasma current, and temperature are all unknown
2. Machine geometry (R, A, B_T) not disclosed — prevents any physics-based scaling
...
**Magnet System (TF/PF/CS Coils) — TRL 2–3 (Pranos-specific)**

- **Demonstrated**: TF coil stress analysis and CAD completed, per IAEA FUSE Portal. This represents computational design work, not hardware fabrication. Broader context: HTS spherical tokamak magnets at pilot scale have been demonstrated by Tokamak Energy (Demo4, complete 14 TF + 2 PF coil set, 11.8 T, November 2025 [referenced in 21-spherical-tokamak-hts §Section 3]), providing technology existence proof for the HTS route — but Pranos has not confirmed they are using HTS.
...
**Plasma Physics Experimental Basis — TRL 1–2 (Pranos-specific)**

- **Demonstrated**: "Working with plasma in a glass globe" per company description — indicates glow discharge or low-temperature plasma diagnostic capability, not tokamak-relevant plasma confinement. Three experimental configurations (Ragya, Pragya, PraniQ) are named but not described. No published plasma parameters, density, temperature, or confinement time data exist for any Pranos device.
- **On paper only**: The Jenga digital twin platform (MHD, transport, neutronics, thermal-structural, PMI, plant-level integration) — this is the most substantive technical output the company has described. If functional, it represents meaningful systems-level modeling capability. However, no validation data or published outputs exist.
- **Missing at scale**: A functioning tokamak at any scale. The progression from glass globe plasma to even a small tokamak discharge (e.g., equivalent to India's SS-ST or MAST at its inception) requires superconducting or resistive magnet fabrication, vacuum vessel construction, plasma current initiation, and feedback control — none of which has been demonstrated.
...
| Specific capital cost (analogue, ST at 50 MW scale) | [analogue] $10,000–$30,000/kWe | [analogue: ARIES-ST cost scaling applied to 50 MWe; 2.5–4× penalty vs. 500 MWe ST; reference from Brown 2018 and ARIES-ST] | low | Purely analogue estimate; Pranos design parameters unknown; very wide uncertainty |

### Missing Parameters

| Parameter | Gap Type | Criticality | Notes |
|-----------|----------|-------------|-------|
| Major radius (R), aspect ratio (A) | proprietary / not-yet-published | blocking | No physics design point can be established without machine geometry |
| On-axis toroidal field (B_T) | proprietary / not-yet-published | blocking | Field determines beta, fusion power density, and magnet cost |
...
**9. Stewart, R. and Shirvan, K. (2022) — Fusion regulatory cost scenario**
