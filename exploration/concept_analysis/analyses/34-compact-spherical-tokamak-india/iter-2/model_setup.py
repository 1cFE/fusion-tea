...
Modeling approach:
    ...
    The arXiv preprint arxiv-2603-11549 (March 2026) describes PRAGYA — the
    company's first experimental low-aspect-ratio tokamak (R₀=0.4m, A≈2.2,
    B_T=0.1T, Ip≤25kA, final engineering design phase). PRAGYA confirms the
    ST family classification and provides the first published machine geometry,
    but its parameters are far below any commercial operating point. The
    commercial 50 MWe concept parameters remain undisclosed.

    This model uses the 0D physics model (use_0d_model=True) to route the
    magnetic field B → plasma current I_p → density n_e → fusion reactivity
    → p_fus → LCOE. This gives non-zero elasticity for B, q95, and f_GW —
    the three parameters that most directly represent the unknown commercial
    design point. T_e is a derived output (found by bisection to achieve
    p_net_target); it is not a free input in inverse mode. All commercial
    plasma parameters carry an UNCERTAIN flag.

    Nearest-neighbor: Tokamak Energy ST-HTS (concept 21), A≈2.3. PRAGYA's
    A≈2.2 is consistent with the model's A≈2.0–2.2 analogue range.
...
Concept choice rationale:
    ...
    for a ~50 MWe spherical tokamak. PRAGYA (R₀=0.4m, A≈2.2) is the
    experimental precursor; commercial dimensions remain undisclosed.

Key deviations from framework defaults:
    - use_0d_model=True: physics-based 0D model; B drives I_p → n_e → p_fus
    - B=4.0 T: commercial HTS analogue (far above PRAGYA 0.1T; undisclosed)
...
_SHARED_KWARGS = dict(
    ...
    # ── Economic / Financial ────────────────────────────────────────────────
    availability=0.80,              # UNCERTAIN: fleet vision implies ~87%
                                    # (analysis.md §5: 3,000 GWh/day ÷ 125 GW × 24 h)
                                    # but 0.80 applied as conservative analogue;
                                    # no maintenance scheme disclosed for Pranos
                                    # Ref: Araiinejad & Shirvan (2025) DT MCF: 75–90%
    ...
    p_cryo=0.5,                     # DEFAULT: cryogenic power (consistent with HTS assumed)
)
...
Magnet type: HTS (REBCO) assumed (no Pranos disclosure).
  Most physically appropriate for compact ST (Tokamak Energy Demo4 precedent).
  If resistive copper: recirculating power would be prohibitive at this scale.
  Source: analysis.md §2 Challenge 3; §6 Gap 3

Availability (0.80):
  Fleet vision implies ~87% but 0.80 applied as conservative analogue.
  Ref: Araiinejad & Shirvan (2025) D-T MCF range 75–90%; analysis.md §5
