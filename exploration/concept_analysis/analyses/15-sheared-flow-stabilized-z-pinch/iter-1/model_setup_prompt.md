# 1costingfe Model Setup: Sheared-Flow Z-Pinch (Zap Energy)

You are generating a runnable 1costingfe model setup script for **Sheared-Flow Z-Pinch (Zap Energy)**
(Zap Energy). The script must run via `uv run python model_setup.py` and emit an
LCOE estimate.
...
**Start here.** Open `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\15-sheared-flow-stabilized-z-pinch\analysis.md` and read its `## Design Point` block and
...
3. **`spec` carries only design-point inputs.** Do NOT put the uniform
   ...
   **Archetype-specific spec key blocklist (workarounds for known library
   ...
   - **DIPOLE** (`STAGED_ZPINCH == "DIPOLE"`): do **NOT** pass
   ...
   - **MIF concepts** (`STAGED_ZPINCH ∈ {MAG_TARGET, MAGLIF,
...
## Output
Write the script to: `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\15-sheared-flow-stabilized-z-pinch\iter-1\model_setup.py`
