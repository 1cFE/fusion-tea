...
**5. Pulsed power driver cost scaling (Impact: Moderate)**
...
- **Missing at scale**: Solid-state switch lifetime under 10 Hz, 10⁸+ cycle commercial operation. Capacitor degradation under fast-discharge, repetitive cycling at plant duty. Full driver system cost at commercial scale.
...
**Capacitors and high-voltage switches (pulsed power driver)**

The pulsed power driver is the unique supply chain item for this concept. Commercial capacitor manufacturing is mature but concentrated in a small number of manufacturers. High-voltage, high-energy-density capacitors for fusion-grade pulsed power are specialty products. The NIF at LLNL provides analogy: its pulsed power systems required custom capacitor procurement at significant cost. Scaling to commercial Z-pinch fleets would require significant expansion of capacitor manufacturing capacity — currently a specialty market estimated in the low billions of dollars globally. Solid-state switch (IGBT, thyristor) availability for high-voltage, high-current, high-rep-rate service is more commercially developed but still not off-the-shelf for fusion duty cycles.
...
| 14 | Plant lifetime and component replacement schedule | S5 | truly-unknown | nice-to-have | Century program will provide first data on electrode/driver lifetimes |
