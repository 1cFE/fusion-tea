...
Key deviations from framework defaults:
  ...
  - availability = 0.75  standardized per scoring_framework.md §Plant availability (Pulsed IFE, D-T)
...
_SHARED_KWARGS = dict(
    ...
    # Canonical 0.75 per scoring_framework.md §Plant availability (Pulsed IFE, D-T).
    # No Tier-A override exists (no published HYLIFE-II availability target). Previous
    # value of 0.80 was an analogue-based midpoint with no external citation; policy
    # now requires the canonical value. Cross-concept LCOE comparisons within the
    # pulsed-IFE family are apples-to-apples on this dimension.
    # Source: scoring_framework.md §Plant availability; analysis §S2 Challenge 3
    availability=0.75,              # canonical per scoring_framework.md (Pulsed IFE, D-T)
    ...
    # HIF's primary structural advantage over laser ICF (1-15%). However, when
    # q_eng is provided as a fixed input (from HIBALL 15% recirc), eta_pin no
    # longer controls the recirculating fraction in the pulsed IFE inverse solver —
    # q_eng is the binding constraint. eta_pin only affects the driver energy
    # decomposition within that recirculating budget. The efficiency-advantage
    # story is correctly demonstrated by the q_eng-derived sweep below.
...
print(f"  availability = 0.75  canonical per scoring_framework.md §Plant availability (Pulsed IFE, D-T)")
print(f"                     Previously 0.80 (analogue midpoint, no citation). No Tier-A")
print(f"                     override exists; policy value adopted. Source: scoring_framework.md")
...
print(f"  CAS21 = LIKELY UNDERCOSTED  Framework uses per-MW civil works scaling derived")
print(f"                     from tokamak geometry. An HIF facility requires km-scale")
print(f"                     accelerator tunnel infrastructure — qualitatively different")
print(f"                     civil works scope. This may partly explain the gap between")
print(f"                     model LCOE (~$92/MWh) and the inflation-adjusted HYLIFE-II")
print(f"                     reference (~$162/MWh, 6.5 c/kWh × 2.5 CPI). The model LCOE")
print(f"                     is probably a lower bound; the historical reference is probably")
print(f"                     an upper bound. The accelerator civil works uncertainty should")
print(f"                     be tracked separately from driver capital uncertainty.")
print(f"                     Source: hif-technology-overview.md §HYLIFE-II; analysis §S2.1")
print()
...
print()
print("  Framework artifact notes (do not interpret as HIF design levers):")
print("    eta_pin: when q_eng is an input constraint (as here), eta_pin does not control")
print("      recirculating fraction — see driver efficiency sweep below for correct treatment.")
print("    plasma_t: IFE analog is target gain (q_sci), not plasma temperature. Chamber")
print("      radius appears as a costing geometry parameter; its sensitivity has no HIF")
print("      design interpretation. Varying plasma_t does not represent an HIF design choice.")

# ── Driver Efficiency Scenario Sweep ─────────────────────────────────────────
# HIF's principal economic claim is 30-40% driver efficiency vs. 1-15% for laser ICF.
#
# Framework limitation: in the pulsed IFE path, `q_eng` is an input constraint
# (set here to 6.5 from HIBALL's published 15% recirculating power fraction).
# The inverse solver derives driver energy FROM q_eng, not the reverse:
#   p_driver = (p_recirc - fixed_loads) * eta_pin   [physics.py pulsed_thermal_inverse]
# With q_eng fixed, varying eta_pin only rescales p_driver within a fixed
# recirculating budget — it does NOT reduce the recirculating fraction.
# This means a naive eta_pin-only sweep cannot demonstrate HIF's efficiency
# advantage; it produces a small *positive* elasticity because larger eta_pin
# pushes more energy through the driver channel (increasing some cost sub-accounts
# slightly), which contradicts physical intuition.
#
# Correct approach: when eta_pin improves, the recirculating power fraction
# decreases, raising q_eng. The sweep below derives q_eng from eta_pin using
# the driver-dominated scaling: q_eng(eta) = q_eng_base × (eta / eta_base).
# This holds when driver recirculating power dominates fixed loads — valid
# for HIF where the accelerator is the largest single recirc consumer.
# Source: hif-technology-overview.md §Driver Technology; analysis §S5; §S7

_ETA_BASE = 0.35       # HIF base-case driver efficiency
_QENG_BASE = 6.5       # Corresponding q_eng (HIBALL 15% recirc)

def _derive_qeng(eta: float) -> float:
    """Map driver wall-plug efficiency to q_eng via driver-dominated scaling."""
    return _QENG_BASE * (eta / _ETA_BASE)

_eta_scenarios = [
    (0.10, "Laser ICF reference  (0.10 — 10%)"),
    (0.25, "Below HIF range      (0.25 — pessimistic)"),
    (0.30, "HIF low end          (0.30)"),
    (0.35, "HIF mid-range        (0.35 — base case)"),
    (0.40, "HIF high end         (0.40)"),
    (0.45, "Above HIF range      (0.45 — optimistic)"),
]

print("\nDriver Efficiency Scenario Sweep — physically-derived q_eng mapping:")
print("  (q_eng derived from eta_pin via driver-dominated scaling: q_eng = 6.5 × eta/0.35)")
print(f"  {'Scenario':<42}  {'eta_pin':>8}  {'q_eng':>6}  {'LCOE $/MWh':>11}  {'OC $/kW':>9}  Notes")
print("  " + "-" * 96)

_sweep_kwargs = {k: v for k, v in _SHARED_KWARGS.items() if k not in ("eta_pin", "q_eng")}
for _eta, _label in _eta_scenarios:
    _qeng = _derive_qeng(_eta)
    _note = "  <- base case" if abs(_eta - 0.35) < 0.01 else ""
    if _qeng <= 1.01:
        print(f"  {_label:<42}  {_eta:>8.2f}  {_qeng:>6.2f}  {'N/A (q_eng≤1)':>11}  {'N/A':>9}  <- net power < 0")
        continue
    _rsweep = model.forward(net_electric_mw=NATIVE_MW, eta_pin=_eta, q_eng=_qeng, **_sweep_kwargs)
    print(f"  {_label:<42}  {_eta:>8.2f}  {_qeng:>6.2f}  {_rsweep.costs.lcoe:>11.1f}  "
          f"{_rsweep.costs.overnight_cost:>9.0f}{_note}")

print()
print("  Note: at laser ICF efficiency (eta_pin~0.10), derived q_eng ≈ 1.86 means ~53%")
print("  recirculating power — the plant barely breaks even energetically. At HIF's 30-40%,")
print("  q_eng of 5.6–7.4 gives 13–18% recirc, demonstrating the structural economic")
print("  advantage that is HIF's central claim. Source: analysis §S7; §S5")
print()
...
# ── Availability Scenario Sweep ───────────────────────────────────────────────
# Availability is the dominant LCOE lever (elasticity ≈ −0.96). Canonical base
# case is 0.75 per scoring_framework.md §Plant availability (Pulsed IFE, D-T).
# This sweep tests sensitivity around that canonical central case.
# Source: analysis §S2 Challenge 3; analysis §S5 (H2 hypothesis)
_avail_scenarios = [
    (0.70, "Low availability   (70%)"),
    (0.75, "Base case          (75%)"),
    (0.80, "Above-base         (80%)"),
    (0.85, "Above-target       (85%)"),
    (0.90, "High availability  (90%)"),
]

print("Availability Scenario Sweep (dominant LCOE lever, elasticity ≈ −0.96):")
print(f"  {'Scenario':<42}  {'Avail':>7}  {'LCOE $/MWh':>11}  {'OC $/kW':>9}  Notes")
print("  " + "-" * 84)

_avail_sweep_kwargs = {k: v for k, v in _SHARED_KWARGS.items() if k != "availability"}
for _avail, _label in _avail_scenarios:
    _ravail = model.forward(
        net_electric_mw=NATIVE_MW,
        availability=_avail,
        **_avail_sweep_kwargs,
    )
    _note = "  <- base case" if abs(_avail - 0.75) < 0.001 else ""
    print(f"  {_label:<42}  {_avail:>7.2f}  {_ravail.costs.lcoe:>11.1f}  "
          f"{_ravail.costs.overnight_cost:>9.0f}{_note}")

print()
print("  Note: availability is the single most controllable post-construction LCOE lever.")
print("  Canonical base case is 0.75 (scoring_framework.md). At this level LCOE ≈ $100/MWh.")
print("  No HIF plant has ever operated; rep-rated chamber (6 Hz) wear and target")
print("  delivery reliability are the key determinants. Source: analysis §S2 Challenge 3")
print()
