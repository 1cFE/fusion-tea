...
**Li-6 Enrichment — Shared Constraint with All D-T Breeding Blankets**

Both LiPb (HIBALL) and FLiBe (HYLIFE-II) require Li-6 enrichment for adequate TBR. Natural lithium is ~7.5% Li-6; both designs likely require moderate-to-high enrichment (50–90% Li-6) depending on blanket geometry. Li-6 enrichment is commercially available from Chinese and Russian suppliers using legacy mercury amalgam processes; Western alternatives are under development but not at commercial scale for fusion demand. This constraint is shared with all D-T fusion concepts and is not unique to HIF.
...
| Capacity factor | [analogue] 75–85% | [analogue — comparable IFE concepts; HYLIFE-II designed for continuous rep-rated operation] | low | No HYLIFE-II-specific availability target published; high rep rate reduces maintenance downtime pressure vs. low-rep-rate IFE |
...
| 9 | Li-6 enrichment level required for TBR > 1 in HIF blanket designs | S4 | derivable | important | Calculable from neutronics once blanket geometry is established; sets supply chain requirements |
