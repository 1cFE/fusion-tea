...
- **CAS21** (Buildings & site): The concept eliminates several building-intensive subsystems (tritium plant, hot cells for activated-component processing, heavy biological shielding). No company-published cost figure. **Override proposed**: 50% reduction. CAS21 is a Class-S shared account — the library charges it once for the whole fleet regardless of n_mod. The value `0.50 * generic.costs.cas21` means 50% of the library's default once-charged building cost for a fleet of this laser-ICF module at 1 GWe.
...
overrides:
  ...
  - account: CAS21
    ...
    rationale: |
      CAS21 is a Class-S shared account: the library charges it once for the
      whole 1 GWe fleet regardless of n_mod — that single charge is the fleet's
      amortized building cost. `0.50 * generic.costs.cas21` means 50% of the
      library's default once-charged building scope for a fleet of this
      laser-ICF module at 1 GWe.
      Eliminated facilities relative to the library's modular-fleet default
      for a laser-IFE concept: tritium processing building, hot cell for
      ...
      50% of the library's default modular-fleet building cost is the
      judgment for this scope reduction, justified by elimination of
      radiation-driven and tritium-driven building requirements that dominate
