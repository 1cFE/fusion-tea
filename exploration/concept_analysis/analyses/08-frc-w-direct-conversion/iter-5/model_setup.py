...
Modeling approach:
  Uses the MIF (magneto-inertial fusion) power balance with PulsedConversion.INDUCTIVE_DEC
  to express Helion's pure inductive energy recovery directly. The expanding-FRC coil
  inductive pickup feeds directly back into the cap bank; eta_dec carries the round-trip
  circuit efficiency (~90%), and eta_th is zero (no steam cycle exists).
  Source: costingfe/types.py:57 (PULSED_FRC → INDUCTIVE_DEC); analysis.md §S3 (Direct
          Inductive Energy Recovery)

  The ~5% neutron power fraction deposits in the walls and enters the thermal pool.
  With eta_th=0 that waste heat is not electrically converted, so the neutron fraction
  introduces a small downward bias on net electric relative to the old eta_th proxy.
  Source: analysis.md §S5 (neutron fraction ~5%)
...
Key deviations from 1costingfe MIF defaults:
  ...
  Power balance:
    - pulsed_conversion = INDUCTIVE_DEC — pure inductive recovery, no steam cycle
    - eta_dec = 0.90 (vs 0.85 default) — inductive DEC round-trip circuit efficiency (UNCERTAIN)
    - eta_th = 0.0 — no thermal cycle; defaulted by framework for INDUCTIVE_DEC
...
model = CostModel(
    ...
    # Helion's physics is pure inductive DEC: cap bank → coils → plasma → coils → cap bank.
    # PULSED_FRC maps to INDUCTIVE_DEC in costingfe/types.py:57; MAG_TARGET defaults to THERMAL
    # so this must be set explicitly to select the correct forward function.
...
result = model.forward(
    ...
    #   CAS26: Reduced heat rejection — only ~10% of power as waste heat (eta_dec=0.90
    #          implies 10% DEC circuit losses; thermal pool is neutron + radiation heat,
    #          not converted since eta_th=0).
    ...
    eta_dec=0.90,    # UNCERTAIN: Inductive DEC round-trip circuit efficiency.
                     ...
                     # eta_th is not set here — framework defaults it to 0.0 for INDUCTIVE_DEC.
...
print("\nKey Assumptions:")
print("-" * 60)
print(f"  Inductive DEC efficiency (eta_dec):       {0.90:.0%}")
...
print(f"  Thermal cycle efficiency (eta_th):        {0.0:.0%}  (no steam cycle; framework default)")
...
print("  - eta_dec=0.90 is the inductive DEC round-trip circuit efficiency")
print("    (cap bank → coils → plasma → coils → cap bank). eta_th=0 (no thermal cycle).")
print("    The ~5% neutron power fraction enters the thermal pool and is not electrically")
print("    converted (eta_th=0), introducing a small downward adjustment in net electric")
print("    relative to prior iterations. Neutron fraction:")
