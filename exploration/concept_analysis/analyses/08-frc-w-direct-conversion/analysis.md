...
overrides:
  - account: C220101
    ...
    rationale: |
      The PULSED_FRC archetype already prices C220101 at zero in the library (confirmed:
      generic = 0.0 in model output at all scales). The archetype is calibrated for
      no-blanket pulsed-FRC devices, so no departure from the library default is needed.
      The narrative rationale (D-He3 produces only ~5% neutron energy; no tritium breeding
      blanket, no Li-bearing material) remains accurate as a physics description but does not
      constitute an override — the library already reflects this architecture. Disabled to avoid
      a vacuous 0.10 × 0 = 0 override that re-states the library default.
  ...
  - account: CAS27
    ...
    rationale: |
      The PULSED_FRC archetype already prices CAS27 at zero in the library (confirmed:
      generic = 0.0 in model output at all scales). The archetype carries no blanket
      material inventory for a no-blanket architecture, so no departure is needed.
      The physics rationale (no LiPb or FLiBe fill; He3 self-bred via DD reactions; deuterium
      from water at negligible cost) accurately describes why Helion has no blanket fill cost,
      but the library already reflects this. A He3 startup inventory cost is a genuine gap
      (see Section 6, Gap 7) but cannot be overridden against this account without a
      published figure. Disabled to avoid a vacuous 0.05 × 0 = 0 override.
```

**Count check:** 8 enabled overrides. Expected band for Low Archetype-Fit: 6–12. Count is within band. (C220101 and CAS27 are present but disabled — the PULSED_FRC library already prices both at zero; overriding a zero default has no effect on LCOE.)
...
**Model output interpretation — CAS80 embeds a cost Helion's thesis is designed to eliminate.** The 1 GWe LCOE of 132 $/MWh includes CAS80 priced at the library's D-He3 commercial-procurement default: $524M of the $2404M 1 GWe fleet cost (22%). Helion's architecture — self-breeding He3 from DD reactions via tritium decay — is specifically designed to eliminate commercial He3 procurement. CAS80 overrides are not yet available in the framework (`1costingfe#106`), so this embedded cost cannot be adjusted to reflect self-breeding economics. If He3 self-breeding reduces fuel cost to near-zero (Helion's claim), CAS80 removal would reduce the 1 GWe overnight cost by ~22% and LCOE from ~132 $/MWh to roughly ~103 $/MWh. The quoted headline is therefore materially pessimistic on fuel relative to Helion's economic thesis; any cross-concept comparison should carry this caveat explicitly.

For context: the closest concepts in the landscape by confinement family (MIF) are MagLIF (Pacific Fusion) and MTF Pneumatic Compression (General Fusion). Both are D-T pulsed MIF concepts with thermal conversion cycles and are structurally different in every cost-relevant dimension from Helion's D-He3 direct-conversion design. The MagLIF analysis (concept 07, iter-2/PASS) is used below as an informal reference to anchor delta magnitudes against its modeled 1 GWe values; it is not a formally assigned comparable.

The primary family-delta dimensions, anchored to MagLIF's modeled 1 GWe values (informal reference):

**Against MagLIF (Pacific Fusion, iter-2/PASS — 1 GWe modeled values):**
- CAS23 (turbine plant): Helion = $0; MagLIF = $296.5M — a $296.5M advantage to Helion from eliminating the thermal cycle entirely. Direct inductive energy conversion removes the steam generator, turbine, and condenser.
- C220101 (blanket): Helion = $0; MagLIF = $269.9M — a $269.9M advantage from the absence of a tritium breeding blanket. No Li-bearing material, no tritium extraction loop.
- CAS26 (heat rejection): Helion = $9.0M (fleet); MagLIF = $128.1M — a $119.1M advantage. Helion's DEC recovers 85–95% of fusion energy directly; waste heat is a small fraction of a thermal-cycle plant's cooling burden.
- C220107 (capacitor bank): Helion fleet total = ~$500M (20 modules × $25M/module at NOAK $0.50/J target on a 50 MJ bank); MagLIF = $2,184.5M for 1 GWe — a $1,685M apparent advantage to Helion. Caution: this difference partly reflects the modular structure (Helion's per-module bank is small; MagLIF is a large single-shot driver at 1 GWe) and is sensitive to the unverified NOAK assumption for Helion's $0.50/J target. Directionally valid, but magnitude-uncertain.
- C220103 (magnets): Both concepts avoid HTS REBCO coils; both use resistive conductors. Coil cost is not a differentiator — it has shifted into C220107 for both architectures.
- CAS80 (fuel, not overridable): MagLIF = $35.6M (D-T fleet); Helion = $524M (D-He3 at library commercial-procurement default). The $488M difference reflects the library pricing He3 as a commercial commodity, which directly contradicts Helion's self-breeding thesis. See CAS80 interpretation note above — this account is not overridable and is the primary reason Helion's headline LCOE (132 $/MWh) is higher than MagLIF's (102.9 $/MWh) despite Helion having a far cheaper reactor island.
