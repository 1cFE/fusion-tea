# 1costingfe Model Setup: FRC w/ Direct Conversion (Helion Energy)

You are generating a runnable 1costingfe model setup script for **FRC w/ Direct Conversion (Helion Energy)**
(Helion Energy). The script must run via `uv run python model_setup.py` and emit an
LCOE estimate.
...
**Start here.** Open `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\08-frc-w-direct-conversion\analysis.md` and read its `## Design Point` block and
...
- **Closest example (pattern to imitate):** `\home\reid\1cfe\1costingfe\examples\dhe3_pulsed_frc.py`
...
3. **`spec` carries only design-point inputs.** Do NOT put the uniform
   ...
   **Archetype-specific spec key blocklist (workarounds for known library
   ...
   - **DIPOLE** (`PULSED_FRC == "DIPOLE"`): do **NOT** pass
   ...
   - **MIF concepts** (`PULSED_FRC ∈ {MAG_TARGET, MAGLIF,
...
## Output
Write the script to: `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\08-frc-w-direct-conversion\iter-1\model_setup.py`
