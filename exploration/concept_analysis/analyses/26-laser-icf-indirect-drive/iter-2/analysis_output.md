...
**Dominant LCOE uncertainty 6 — Plant availability.** The model sensitivity sweep shows availability with elasticity −0.97 — the single largest LCOE driver, exceeding laser driver cost (elasticity +0.045), target gain (−0.18), and thermal efficiency (−0.18). The current analysis assumes 75% availability as an unvalidated placeholder; no rep-rate-coupled availability model exists in available sources. The divergence between designs is extreme: Inertia's 3–5 year chamber replacement schedule implies extended planned maintenance outages absent in conventional power plants, while Xcimer's HYLIFE-derived liquid-wall design claims no structural chamber replacement — spanning more than a 10× range in a dominant cost driver. Availability is a first-order LCOE lever and a testable scenario branch: every 10% reduction from the 75% baseline raises LCOE roughly proportionally.
