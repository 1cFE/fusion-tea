...
Key deviations from framework defaults:
    ...
    8. target_unit_cost = 5.0 — per-shot fabricated-capsule consumable (CAS80).
                         NearStar fabricates a pre-magnetized D-D capsule every
                         shot; the framework now costs this on the fission
                         fuel-assembly analogy, and the same knob switches on the
                         CAS22.01.08 target factory. UNCERTAIN (no public data);
                         swept $1/$5/$20 in CF-F3.
...
_SHARED_KWARGS = dict(
    ...
    target_unit_cost=5.0,       # UNCERTAIN: $/shot fabricated-capsule hardware
                                # (pre-magnetized 50 g D-D shell + railgun sabot;
                                # excludes the D-D isotope, costed in the CAS80 fuel
                                # term). No public NearStar cost data exists; $5 is a
                                # nominal mid value between a sub-dollar ICF capsule
                                # and a MagLIF-class liner, swept $1/$5/$20 below
                                # (CF-F3). NearStar fabricates a consumed target each
                                # shot, so a positive value also switches on the
                                # CAS22.01.08 target factory: capital, factory power
                                # (p_target above), and the per-shot consumable now
                                # move together instead of via a cost override.
                                # Source: analysis.md §S5 Missing Parameters
                                # "Capital cost: truly-unknown"; framework knob
                                # documented in costingfe CAS80_target_consumables.md
    ...
    # No capital cost data exists for any NearStar subsystem. The one non-default
    # engineering input bearing on cost is target_unit_cost (above), which is a
    # framework knob, not a CAS cost_override.
...
# This is the RAIL (durable-hardware wear of the railgun bore), distinct from
# the per-shot CAPSULE consumable: the capsule is the consumed target and is now
# costed in CAS80 via target_unit_cost (swept separately in CF-F3), whereas rail
# replacement is launcher wear with no explicit framework line.
...
# ── Target Capsule Consumable Scenarios (CF-F3) ──────────────────────────────
# The fabricated capsule (pre-magnetized 50 g D-D shell + railgun sabot, ex-fuel)
# is consumed every shot and is now costed in CAS80 via target_unit_cost. No
# public NearStar per-capsule cost exists, and at ~12.6 M shots/yr (1 Hz, 40% CF)
# the term is a leading LCOE driver, so it is presented as a scenario sweep
# rather than a point estimate. The same knob also switches on the CAS22.01.08
# target factory, so the table reports both CAS80 and total LCOE.
#   $1  — aggressive NOAK mass production (coal-retrofit cost framing)
#   $5  — nominal: rugged magnetized projectile, MagLIF-liner class (baseline)
#   $20 — complex precision part at modest volume
# Source: no public data (analysis.md §S5 "Capital cost: truly-unknown");
# framework basis in costingfe CAS80_target_consumables.md.
_CAPSULE_COSTS = [1.0, 5.0, 20.0]   # $/shot fabricated capsule hardware

print()
print("=" * 65)
print("Target Capsule Consumable Scenarios — $/shot fabricated capsule")
print("Native 200 MWe; baseline target_unit_cost = $5/shot")
print("=" * 65)
print(f"  {'$/capsule':>10}  {'CAS80 (M$/yr)':>14}  {'C220108 (M$)':>13}  {'LCOE ($/MWh)':>14}")
print("-" * 60)
for tuc in _CAPSULE_COSTS:
    _kw = {**_SHARED_KWARGS, "target_unit_cost": tuc}
    _r = model.forward(net_electric_mw=_NATIVE_MWE, **_kw)
    _note = "  [baseline]" if tuc == 5.0 else ""
    print(
        f"  ${tuc:>8.1f}  {float(_r.costs.cas80):>14.1f}  "
        f"{float(_r.cas22_detail['C220108']):>13.1f}  {float(_r.costs.lcoe):>14.1f}{_note}"
    )
print()
print("  The capsule consumable scales linearly with $/shot and dominates LCOE")
print("  at the high end — the per-target cost is the central IFE/MIF economic")
print("  uncertainty, as for all inertial concepts.")
