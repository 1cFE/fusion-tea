...
3. **Major radius / plant scale** (structural uncertainty): With HH380 design point undisclosed, major radius (and by extension fusion power and net electric output) is the dominant structural unknown. It determines capital cost scaling, magnet material demand, and recirculating power. Any LCOE model must bracket this parameter with low / base / high scenarios (e.g., R = 1.5 m / 2 m / 2.5 m analogised from CFS ARC and CFETR ranges).
...
| Differentiator | vs. Conventional Tokamak | Cost Implication | Direction |
|----------------|--------------------------|-----------------|-----------|
| Full HTS coil set (TF + PF + CS in REBCO) vs. LTS or partial-HTS | Most large tokamaks use LTS (Nb₃Sn/NbTi) for all coils; CFS uses HTS for TF only; Energy Singularity extends HTS to CS and PF | **Penalty**: more REBCO tape per plant, novel CS duty-cycle risk (quench, fatigue), higher coil fabrication cost; **Benefit**: all coils at same cryogenic temperature (20 K) potentially simplifies cryoplant | Net penalty vs. LTS baseline; ambiguous vs. partial-HTS |
| Compact high-field geometry (~14 T on-axis, ~70% SPARC volume) vs. ITER (5.3 T, R = 6.2 m) | Compact design reduces structural material and building volume for a given Q; higher field enables higher pressure at smaller radius | **Advantage**: smaller machine → lower structural material cost, shorter build time, smaller site; potentially lower absolute capital cost at same Q | Advantage (capital cost) |
| AI-based plasma control enabling long-pulse/steady-state operation | Conventional tokamaks rely on feedback control but not AI-native architectures; steady-state operation requires active current drive | **Advantage** (if it works): higher capacity factor, fewer disruptions → better LCOE; **Risk**: unvalidated at burning plasma conditions | Potential advantage; high uncertainty |
| China-domestic supply chain (>95% domestic localization) | Western designs rely on international supply chains for key components | **Advantage for China-domestic deployment**: lower logistics cost, no export control friction; **Barrier for international comparison**: opaque cost basis | Geography-dependent |

The most TEA-relevant differentiator is the **full HTS coil scope (TF + PF + CS)** relative to both the LTS baseline and partial-HTS competitors. The cost penalty stems from: (a) higher REBCO tape demand per plant vs. TF-only HTS, (b) novel CS duty cycle (plasma initiation current ramp at 25 T) with no demonstrated reliability record, and (c) premium CS coil engineering requirements. The potential offsetting benefit (simplified cryoplant with uniform 20 K operating temperature) is real but has not been quantified in any published source.
