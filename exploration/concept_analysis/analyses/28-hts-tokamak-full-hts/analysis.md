...
Archetype: TOKAMAK
Archetype-Fit: High
Comparison-Status: freeform-complete
Comparables:
...
| Field | Value |
|-------|-------|
| **Plant Name** | HH380 [Analogue] |
| **Maturity** | Planned (post-2030) |
| **P_native (net electric)** | 500 MWe [analogue-derived] |
| **Grounding Confidence** | **Low** |

**Grounding rationale**: Energy Singularity's only power-producing machine (HH380 demo power station, planned post-2030) has zero publicly disclosed technical specifications — no net electric output, no geometry, no fusion power target, no subsystem designs. The P_native = 500 MWe value is an **analogue-derived placeholder** based on compact tokamak comparables (CFS ARC, Tokamak Energy ST80), not company-grounded data. This design point enables cost modeling but carries fundamental uncertainty in scale, geometry, and performance.
...
**Override count vs. archetype-fit rubric**: This concept is graded **High archetype-fit** (expected 0-4 enabled overrides). The zero-override count falls within that band and reflects **fundamental data absence**, not library alignment. The analysis identifies **C220103 (confinement magnets)** as the top override candidate when HH380 data becomes available, due to the full-HTS architectural difference (TF + PF + CS all REBCO) from library defaults calibrated to TF-only HTS designs. Until Energy Singularity publishes HH380 magnet specifications or REBCO procurement costs, no company-grounded override is justified.
