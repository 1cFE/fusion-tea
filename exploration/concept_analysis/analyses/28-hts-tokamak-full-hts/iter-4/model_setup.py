"""HTS Tokamak - Full HTS (Energy Singularity) — 1costingfe model setup (iter-4).
...
Modeling approach:
    ...
    This iteration (iter-4) addresses one important assessment finding:

    F-1 (important): structure_t and vessel_t corrected to tokamak-peer-group default.
...
print(f"  Base: R={R0_BASE}m, 500 MWe, 80% avail | C220103 override held fixed at ${c220103_500:.0f}M")
...
for t in [0.10, 0.15, 0.20, 0.25]:
    kw = _base_kwargs(R0_BASE, 500.0, PLASMA_T_BASE, 0.80)
    kw["structure_t"] = t
    kw["vessel_t"] = t
