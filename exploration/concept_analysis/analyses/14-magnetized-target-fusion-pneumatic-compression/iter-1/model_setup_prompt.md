# 1costingfe Model Setup: MTF Pneumatic Compression (General Fusion)

You are generating a runnable 1costingfe model setup script for **MTF Pneumatic Compression (General Fusion)**
(General Fusion). The script must run via `uv run python model_setup.py` and emit an
LCOE estimate.
...
**Start here.** Open `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\14-magnetized-target-fusion-pneumatic-compression\analysis.md` and read its `## Design Point` block and
...
3. **`spec` carries only design-point inputs.** Do NOT put the uniform
   ...
   **Archetype-specific spec key blocklist (workarounds for known library
   ...
   - **DIPOLE** (`MAG_TARGET == "DIPOLE"`): do **NOT** pass
   ...
   - **MIF concepts** (`MAG_TARGET ∈ {MAG_TARGET, MAGLIF,
...
## Output
Write the script to: `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\14-magnetized-target-fusion-pneumatic-compression\iter-1\model_setup.py`
