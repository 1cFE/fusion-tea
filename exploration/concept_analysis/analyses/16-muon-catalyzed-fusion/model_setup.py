...
a positive net power output. The accelerator capital baseline is $5,000M
(commercial-target level: $50M/MW_beam × 100 MW_beam per CF-F1 correction;
$2,000M retained as labeled optimistic scenario in sensitivity sweep).
Even at these aspirational physics parameters, LCOE is >> $1,000/MWh. The
...
class MuCFPlantParams:
    ...
    Demonstrated record: 150 (Los Alamos LAMPF, cold target ~20–800 K)
    [muon-catalyzed-fusion-physics.md §Problems facing practical exploitation].
    Acceleron stretch target: 300 [dossier.md §Summary].
    Baseline 240 sits just below the physics ceiling at 0.4% sticking (= 250).
    TEMPERATURE AND DENSITY DEPENDENCE: N_fus is not an independent free
    parameter — the catalysis cycle rate λ_c and fusion yield c_f increase
    monotonically with both temperature T and density φ for D-T gas targets
    (Yamashita et al. 2022, PMC9013384, EVM-SPM-FIF kinetics model). Historical
    LAMPF results (150 fusions/muon) were at cold targets; Acceleron's stated
    500–1000°C operating range sits at or above the upper bound of the
    historical experimental range. N_fus achievable at Acceleron's operating
    conditions may exceed cold-target values — the commercial-viability threshold
    (N_fus ≥ 200) is physically more plausible at high-T than the LAMPF
    baseline implies. N_fus should ultimately be modeled as f(T, φ).
    Source: analysis.md §S2, §S5. Ref: Yamashita et al. 2022 (PMC9013384).
    HIGH UNCERTAINTY — PHYSICS CEILING (100 / alpha_sticking_pct);
    achievable value also T- and φ-dependent (underexplored above 1000 K)."""

    alpha_sticking_pct: float = 0.4
    """Effective alpha-sticking probability per fusion event [%] — post-reactivation.
    This is the fraction of muons permanently lost per fusion event *after* accounting
    for muon recovery via Auger transfer as the alpha particle decelerates.
    Initial sticking ω_S⁰ ≈ 0.86% (Kamimura & Kino 2021, arXiv:2112.08399); muon
    reactivation recovers ~half, yielding effective values of 0.3–0.5% measured at
    LAMPF [muon-catalyzed-fusion-physics.md §Problems facing practical exploitation].
    Jackson (1957) initial estimate: ~1%.
    Physics ceiling on n_fus_per_muon = 100 / alpha_sticking_pct (effective).
    Source: analysis.md §S2, §S5.
    Ref: arXiv:2112.08399 (initial ω_S⁰); muon-catalyzed-fusion-physics.md (effective).
 ...
 # standardized from 0.5 per scoring_framework.md (Energy Capture: Thermal (unspecified))
    ...
    Analogues: SNS (ORNL) $1.4B, 1 GeV, 1.4 MW beam → ~$1,000M/MW_beam;
               ESS (Sweden) ~€2B, 2 GeV.
    Commercial target per Acceleron: <$50M/MW_beam (20× cost reduction from SNS).
    BASELINE = $50M/MW_beam × 100 MW_beam = $5,000M — commercial cost target,
    not demonstrated; assumes 20× reduction from SNS via industrial production
    (Acceleron roadmap assumption, no published engineering basis).
    $2,000M is the labeled OPTIMISTIC scenario (see sensitivity sweep): consistent
    with Acceleron's internal targets but requires >25× reduction from SNS.
    Source: analysis.md §S2 (Accelerator cost challenge), §S5 (Gap #2).
    Ref: dossier.md §Driver Technology.
    HIGH UNCERTAINTY — range easily $2,000M–$100,000M+ depending on design maturity."""
...
def main() -> None:
    ...
    print("=" * 72)
    print("  NOTE: All sweeps anchor at E_mu=1.2 GeV (aspirational breakthrough).")
    print("  At Acceleron's stated E_mu=2.5 GeV, the plant is a net energy sink")
    print("  and LCOE is undefined regardless of all other parameters.")
    ...
    # Baseline=$5,000M (commercial target: $50M/MW_beam × 100 MW).
    # $2,000M = optimistic/Acceleron internal target (25× below SNS rate).
    # $100,000M = SNS analogue rate ($1,000M/MW_beam × 100 MW).
    sw3 = sensitivity_sweep(
        baseline, "acc_capital_M",
        [200, 500, 1000, 2000, 5000, 10000, 25000, 50000, 100000],
    )
    _print_sweep(sw3,
                 "Accelerator capital cost [$M] (↓ better;"
                 " $5,000M=baseline, $2,000M=optimistic, $100,000M=SNS-analogue)",
                 " M$")
 ...
 # standardized from 0.52 per scoring_framework.md (Energy Capture: Thermal (unspecified))
  ...
  2. ACCELERATOR CAPITAL COST — DOMINANT LCOE COMPONENT
     ...
     At baseline (E_mu=1.2 GeV, N_fus=240, P_beam=100 MW, P_net≈70 MWe):
       Accelerator: $5,000M of ~$6,100M overnight capital (~82%)
       Specific capital: ~$88,000+/kWe (vs. fission NOAK ~$7,000–10,000/kWe)
       ($2,000M optimistic scenario: ~$3,100M overnight, ~$44,000/kWe)

     SNS (ORNL, 1 GeV, 1.4 MW beam): $1,400M → ~$1,000M/MW_beam
     Commercial target: <$50M/MW_beam — a 20× cost reduction from SNS.
     Baseline uses $50M/MW_beam × 100 MW = $5,000M (Acceleron commercial
     target, not demonstrated; assumes 20× reduction via industrial production).
     This requires industrial-scale manufacturing never achieved for GeV linacs.
     Acceleron's active-target architecture is the proposed cost pathway, but
     no published cost estimate exists at power-generation scale.

     To hit $25/MWh at 70 MWe with current physics: acc_capital ≤ ~$50M
     (baseline: $5,000M → ~100× gap; optimistic $2,000M → ~40× gap)
