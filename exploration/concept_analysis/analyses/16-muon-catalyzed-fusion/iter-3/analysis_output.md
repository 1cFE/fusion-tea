...
This "~1%" figure (Jackson 1957) approximates the initial sticking probability. Kamimura & Kino (2021) calculate ω_S⁰ = 0.857% — "~7% smaller than the literature values (≃0.91–0.93%)" — using a coupled-channels Schrödinger equation for the dtμ molecule [arxiv-2112-08399.md §Abstract]. After Auger reactivation, the effective sticking probability falls to 0.3–0.5%, raising the achievable ceiling from ~100 fusions/muon (Jackson's estimate) to 200–350 fusions/muon [muon-catalyzed-fusion-physics.md §Problems facing practical exploitation]. The analysis cites the 0.3–0.5% figure throughout as the effective post-reactivation value — the quantity that enters the LCOE model. Acceleron's 300 fusions/muon target sits near the top of this revised effective range.

The sticking probability is not adjustable by any known, demonstrated technique — it is a function of quantum mechanical muon transfer dynamics. However, at least one active mitigation approach has been proposed: Mori (2021) describes cyclotron resonance acceleration of He⁺μ ions to strip the bound muon before it thermalizes, recovering muons that would otherwise be permanently lost to alpha capture [cited in pmc-articles-pmc9013384.md §Introduction as reference 50: "enforced stripping of negative muons from He⁺ ions to stimulate muon-catalyzed fusion by cyclotron resonance acceleration"]. This technique is early-stage theoretical research with no demonstrated results; it is not integrated into any kinetics model used in this analysis. For TEA purposes, the 0.3–0.5% effective sticking probability and the corresponding 200–350 fusions/muon ceiling remain the applicable constraints. The analytical distinction matters for goal 5 (risks and assumptions): the ceiling reflects the current state of research, not an immutable physical law — an upside risk that the model does not capture.
...
**4. Fusion chamber (target) at commercial scale: DAC is not scalable, but alternatives exist on paper (Impact: High)**

Acceleron's current experimental apparatus uses a diamond anvil cell (DAC) to compress DT fuel to high density [acceleron-arpa-e-presentation-2025.md §Materials and Methods]. A diamond anvil cell is precision laboratory equipment designed for one-off static compression experiments. It bears no architectural relationship to a commercial fusion chamber. What the power plant fusion chamber looks like — how DT fuel is continuously cycled, how fusion neutrons are captured, how product helium is exhausted, and how the chamber survives long-term operation — is entirely unspecified in available sources.

However, the broader μCF literature does document at least one class of alternative target architecture. Yamashita et al. (2022) propose high-temperature adiabatic compression (AC) and shock-wave compression (SWC) of D-T gas as distinct target approaches, explicitly motivated as "a step toward a CF-based compact fusion reactor" [pmc-articles-pmc9013384.md §Towards CF in high temperature compressed gas targets; Conclusion]. A Sato et al. patent (US20200395133A1, "nuclear fusion system using shock-wave compressed gas target") is associated with this research line. Compressed gas targets avoid the single-use, laboratory-instrument nature of the DAC and are potentially more amenable to continuous operation. The TRL assessment is unchanged — no compressed gas μCF target has been demonstrated at power-plant-relevant scale — but the characterization that "the architecture must be reinvented from scratch" should be updated: a competing design pathway exists in the literature, even if it remains at the theoretical/proposal stage.
...
The MCF cost model uses the 1cFE CAS-Structured Free-Form approach rather than 1costingfe's plasma-centric CAS accounts because the dominant capital cost item — the superconducting proton accelerator — has no analog in plasma-fusion account structures (CAS22: magnet system, CAS23: heating systems, CAS26: vacuum systems are all inapplicable). The accelerator is better treated as a single capital block with its own scaling law. Three parameters dominate LCOE sensitivity, in descending order: (1) **E_mu** (muon production energy cost, GeV/muon) — the most uncertain physics parameter, currently 6× above the commercial target; (2) **N_fus** (fusions per muon) — the physics ceiling parameter, bounded above by effective sticking probability; and (3) **accelerator capital cost** ($/kWe) — the cost analogue to the magnet system or driver in other fusion concepts, currently uncharacterized at power-plant scale.

A key coupling between operating conditions and N_fus: the Yamashita et al. (2022) EVM-SPM-FIF kinetics model demonstrates that N_fus increases monotonically with both temperature (T) and density (φ) for D-T gas targets [pmc-articles-pmc9013384.md §Towards CF in high temperature compressed gas targets]. Historical MCF experiments, including the LAMPF 150 fusions/muon record, were conducted at 20–800 K. Yamashita et al. explicitly "point out the importance of CF experiments at high temperatures above 1000 K" as an underexplored frontier. Acceleron's stated operating temperature of 500–1000°C (~800–1300 K) sits at or above this frontier. This means operating temperature is not only a Brayton efficiency lever — it is an engineering lever on N_fus itself. The model should treat N_fus as a function of T and φ rather than a free parameter independent of operating conditions; at Acceleron's target conditions, the 200–300 fusions/muon goal is physically more plausible than the cold-target baseline suggests, though it remains undemonstrated.

The key testable hypothesis the model explores: *Is there any combination of E_mu ≤ 3 GeV and N_fus ≥ 150 that satisfies net electricity output AND LCOE < $0.10/kWh under independently derived capital and O&M assumptions?* The model demonstrates that the answer is no: net positive electricity is unachievable at E_mu ≤ 3 GeV under standard conversion assumptions, and positive net output requires E_mu ≤ ~1.5 GeV — a threshold beyond any operating point Acceleron has described or targeted. Commercial LCOE analysis is therefore a post-threshold exercise contingent on a physics breakthrough not yet described by any program. Sensitivity sweeps in the model are anchored at an aspirational E_mu = 1.2 GeV baseline; at Acceleron's stated target of E_mu = 2.5 GeV, all LCOE values shown in the sensitivity tables are undefined — the plant is a net energy sink regardless of any engineering parameter variation.
...
| Fusions per muon (near-term target) | ~200 | acceleron-arpa-e-presentation-2025.md §Cost contour | medium | Operating point on $0.025/kWh iso-cost line; read from contour figure. N_fus is temperature- and density-dependent (increases monotonically with both T and φ per Yamashita et al. 2022 EVM-SPM-FIF model [pmc-articles-pmc9013384.md §Towards CF in high temperature compressed gas targets]); Acceleron's high-T operating range (800–1300 K) is in the underexplored frontier above historical MCF experiments |
| Fusions per muon (stretch target) | ~300 | dossier.md §Summary | medium | Acceleron's stated target; near top of physics ceiling with 0.3–0.5% effective sticking. More physically plausible at Acceleron's high-T, high-density operating conditions than cold-target experiments suggest, but undemonstrated |
| Fusions per muon (demonstrated maximum) | 150 | muon-catalyzed-fusion-physics.md §Problems facing practical exploitation | high | "Steven E. Jones' team achieved 150 d-t fusions per muon" at Los Alamos LAMPF at cold conditions (20–800 K); "almost enough to reach theoretical break-even." High-T conditions (800–1300 K) are projected to increase N_fus beyond this baseline |
...
| 4 | Fusion chamber architecture for commercial scale — DAC cannot scale | S2, S3, S5 | truly-unknown | blocking | DAC not scalable; compressed gas alternative (adiabatic compression AC / shock-wave compression SWC of D-T gas) documented in literature (Yamashita et al. 2022 [pmc-articles-pmc9013384.md §Towards CF in high temperature compressed gas targets]; Sato et al. patent US20200395133A1) but not demonstrated at power-plant scale. No design has been proposed that integrates with a commercial muon beam at relevant throughput. Must be defined before any subsystem cost modeling is possible |
