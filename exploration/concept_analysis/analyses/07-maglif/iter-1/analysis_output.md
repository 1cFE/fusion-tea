...
**3. Driver cost is a novel cost category (high impact).** The pulsed power driver (capacitor banks, switches, transmission lines) has no analogue in other fusion concepts. The Z-IFE study estimated $372M (median) for a 1 PW LTD-based driver, with LTD cavities comprising 96% of the cost ($353M for 12,600 cavities at ~$28,000 median per cavity). For the IMG architecture Pacific Fusion is pursuing, no published cost estimate exists. The systems model used $15/J as a driver unit cost, derived from the claim that "current pulsed power machines cost ~$30/J and the linear transformer driver should be a factor of two lower."[^5]
...
|-----------|-------|--------|------------|------|
| Plant configuration | 10 chambers, each with independent driver, heat transfer, and power conversion; shared heat rejection | z-ife-sand2006-7148-thermal-cycles.md §3.1.1.3 | high | Original SNL baseline |
...
| Driver architecture | LTD-based, 1 PW; 210 stacks × 60 LTD cavities = 12,600 total | z-ife-sand2006-7148-thermal-cycles.md §3.1.2 | high | 75 m diameter × 10 m high water tank |
...
| Driver capital cost (detailed estimate) | $372M median (1 PW LTD driver) | z-ife-sand2006-7148-thermal-cycles.md §3.1.2 | medium | 96% is LTD cavities ($28,000 median per cavity); 95th percentile: $862M |
...
**C220107 — Pulsed-power capacitor bank:** This is the dominant driver cost. The Z-IFE study provides a detailed bottom-up cost estimate for a 1 PW LTD-based driver: **$372M median** (12,600 LTD cavities at $28,000 median each, comprising 96% of driver cost). The driver cost scaling is given as C_tot = 372 × (TW/1000)^0.6 in $M (2004 dollars).

This is a company-grounded figure from a national laboratory design study with Monte Carlo uncertainty quantification. However, this is for LTD architecture, not IMG. The IMG architecture (Pacific Fusion, Fuse Energy) claims to be "at least 5x cheaper" and "2x more energy efficient" (Fuse Energy), with 90% energy delivery efficiency (vs. 60% for LTD). No IMG-specific dollar figure has been published.

The Z-IFE driver cost of $372M is the most grounded figure available and is directly relevant to the named design point (Z-IFE 10-chamber baseline). The library's default $/J stored energy formula may not capture this specific LTD cost structure. CPI adjustment from 2004 to 2024: approximately 1.57×.
...
overrides:
  - account: C220107
    ...
    rationale: |
      ...
      gives ~$584M. This is the only bottom-up pulsed-power driver cost estimate
      for a Z-pinch-class fusion power plant. The 10-chamber baseline uses one
      driver shared across chambers (via staggered firing), so this is a single
      capital item. The library default $/J formula may not capture the specific
      LTD module cost structure. Note: IMG architecture (Pacific Fusion) claims
      significant cost reduction over LTD, but no published IMG driver cost exists.
```
...
**1 enabled override** (C220107 — pulsed power driver). The archetype-fit grade is High, with an expected band of 0–4 overrides. One override is consistent with this band.

The single override reflects the fact that the pulsed power driver is the architecturally distinctive and cost-dominant subsystem for this concept, and the Z-IFE study provides the only bottom-up cost estimate available. All other accounts either lack extractable company-grounded figures (costs embedded in Osiris systems code) or are adequately represented by library defaults.
...
| 5 | IMG driver cost at power-plant scale not published; only LTD cost available ($372M, 2004$) | S2, S5 | not-yet-sourced | important | Pacific Fusion cost disclosures; pulsed power industry cost benchmarks |
