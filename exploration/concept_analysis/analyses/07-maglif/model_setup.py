"""MagLIF (D-T) — 1costingfe cost model setup (iter-11).
...
Iter-11 additions (addressing assessment F-1: availability standardization):
  F-1: AVAILABILITY standardized to 0.75 per scoring_framework.md §"Plant availability"
       (Pulsed MIF, D-T canonical value). Previous value of 0.85 (Z-IFE assumption,
       no Tier-A citation) replaced. Cross-concept LCOE comparisons within the pulsed
       MIF/IFE family are now apples-to-apples on this dimension.
...
# Availability: canonical 0.75 per scoring_framework.md §"Plant availability"
# (Pulsed MIF, D-T). Previous value of 0.85 (Z-IFE assumption, no Tier-A citation)
# replaced. Tier-A override would require an externally-published availability target
# with a stated maintenance/duty basis — none exists for this concept.
AVAILABILITY = 0.75         # CANONICAL: scoring_framework.md §Plant availability
                             # (Pulsed MIF, D-T); policy-driven, not concept-specific.
...
# Annual energy: 1000 × 8760 × 0.75 = 6,570,000 MWh/yr
#
