...
- **Hypothesis 1 (Rep rate break-even)**: Model-computed — see REP RATE SCENARIO SWEEP in model_setup.py. Net power parameterized as yield_per_shot_GJ × rep_rate_hz × eta_th × mn × 1000 MWe at fixed single-chamber capital (CAS90 annualized). LCOE computed algebraically at 0.1, 0.25, 0.5, 1.0, and 1.8 Hz. Calibration: model gives ~7.6 ¢/kWeh at 0.5 Hz vs. Z-IFE reference 7.0 ¢/kWeh (8% discrepancy, acceptable). Break-even with advanced fission (~40–60 $/MWh) requires rep rate ≥ ~1 Hz at this capital base.
