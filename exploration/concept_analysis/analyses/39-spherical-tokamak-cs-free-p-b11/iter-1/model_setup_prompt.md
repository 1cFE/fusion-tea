# 1costingfe Model Setup: Spherical Tokamak CS-Free PB11 (ENN)

You are generating a runnable 1costingfe model setup script for **Spherical Tokamak CS-Free PB11 (ENN)**
(ENN Energy). The script must run via `uv run python model_setup.py` and emit an
LCOE estimate.
...
**Start here.** Open `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\39-spherical-tokamak-cs-free-p-b11\analysis.md` and read its `## Design Point` block and
...
3. **`spec` carries only design-point inputs.** Do NOT put the uniform
   ...
   **Archetype-specific spec key blocklist (workarounds for known library
   ...
   - **DIPOLE** (`TOKAMAK == "DIPOLE"`): do **NOT** pass
   ...
   - **MIF concepts** (`TOKAMAK ∈ {MAG_TARGET, MAGLIF,
...
## Output
Write the script to: `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\39-spherical-tokamak-cs-free-p-b11\iter-1\model_setup.py`
