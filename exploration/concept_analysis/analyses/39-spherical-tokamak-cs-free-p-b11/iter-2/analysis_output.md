...
- **H1 (DEC failure)**: The DEC-enabled base case yields LCOE ~96 $/MWh at 500 MWe (NOAK). The DEC-failure scenario — routing all alpha power (f_dec = 0.85 of fusion output) through a fallback thermal cycle at η_th = 0.35 rather than a DEC at η_de = 0.80 — collapses gross electric output at constant fusion power, forcing a substantially larger (and more expensive) plant to maintain 500 MWe net. The resulting LCOE is expected to be catastrophically uncompetitive, making DEC a go/no-go condition rather than a sensitivity parameter.
