# 1costingfe Model Setup: Spherical Tokamak HTS (Tokamak Energy)

You are generating a runnable 1costingfe model setup script for **Spherical Tokamak HTS (Tokamak Energy)**
(Tokamak Energy). The script must run via `uv run python model_setup.py` and emit an
LCOE estimate.
...
**Start here.** Open `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\21-spherical-tokamak-hts\analysis.md` and read its `## Design Point` block and
...
## Output
Write the script to: `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\21-spherical-tokamak-hts\iter-1\model_setup.py`
