...
The C-2W/Norman campaign produced the most detailed published FRC physics dataset for TAE's beam-driven approach. Key parameters are in the public record: ion temperatures up to ~3 keV (total plasma temperature Ttot up to ~3 keV, Te ~300 eV), averaged electron density ~1–3 × 10¹⁹ m⁻³, trapped magnetic flux ~5–10 mWb in standard operation (up to ~16 mWb in records), and external axial magnetic field ~1 kG [tae-c2w-machine-details.md §Key Features and Plasma Parameters]. The 2025 NBI-only formation paper (Nature Communications, s41467-025-58849-5) is the highest-profile peer-reviewed disclosure in several years, demonstrating plasma formation without traditional plasma guns or formation sections [tae-nbi-breakthrough-2025.md §The 'Norm' Breakthrough]. TAE demonstrated first p-B11 fusion in a magnetically confined plasma in 2023 (with NIFS Japan) — a milestone confirmed in the dossier but the primary paper is not in the current source set [dossier.md §Fuel].
...
If Q_plasma is modest (say, 3–5), and NBI wall-plug efficiency is ~50–60% [analogue from grokipedia-tae-technologies.md "high-energy neutral deuterium atoms"], the NBI parasitic load can easily consume a large fraction of the gross electric output. For a 50 MWe Da Vinci plant with ~30% thermal efficiency (steam Rankine baseline), the gross electric from a Q_plasma = 5 machine would be approximately P_gross = P_NBI × Q_plasma × η_th = P_NBI × 5 × 0.30 = 1.5 × P_NBI. If NBI draws 1/0.55 × P_NBI in wall-plug power, Q_engineering becomes:  

Q_eng = P_net / P_gross = (1.5 P_NBI – 1.82 P_NBI) / (1.5 P_NBI) = –0.21  

[inferred: NBI power from fusion Q_plasma; wall-plug efficiency from NBI analogue; gross power from thermal efficiency; derivation shows Q_plasma ~5 is insufficient for net electricity with thermal conversion + NBI recirculating power]

This back-of-envelope calculation suggests Da Vinci must achieve Q_plasma >> 10 before NBI recirculating power and thermal cycle losses allow net positive electricity output at all. The required Q_plasma value depends critically on NBI beam energy (which sets wall-plug efficiency and beam-plasma coupling efficiency) — none of which is published for Da Vinci.

**Viability threshold (testable proposition):** The Q_eng breakeven occurs at Q_plasma = 1/(η_th × η_NBI). At η_th = 0.30 and η_NBI = 0.55, breakeven is Q_plasma ≈ 6.1 — but this is mathematical breakeven at zero margin. Commercial operation requires comfortable margin above the floor, so: **IF Q_plasma ≥ 10–15 AND η_NBI ≥ 0.55, THEN net electricity is achievable with steam conversion; ELSE the concept does not produce net electricity and has no LCOE.** If beam energy is high and positive-ion sources are used (η_NBI ≈ 0.40), the breakeven rises to Q_plasma ≈ 8.3, and the commercial target rises to Q_plasma ≥ 20. These are the parameter bounds the sensitivity model should sweep: Q_plasma from 5 to 50, η_NBI from 0.35 to 0.65.
...
- **On paper only**: Net energy gain (Q_plasma > 1) for p-B11 in any confinement device. Non-Maxwellian beam-driven plasma at T_i ~150 keV with sustained T_i >> T_e (required for bremsstrahlung suppression). Confinement scaling from 3 keV to 250 keV in beam-driven FRC. Fusion power levels above milliwatts in magnetically confined p-B11 plasma.
- **Missing at scale**: Demonstration that beam-driven FRC can sustain T_i >> T_e at reactor-relevant densities without equilibration erasing the temperature advantage. Measurement of energy confinement time τ_E at 150+ keV conditions. Copernicus intermediate-step validation data (not yet available as of 2026). Any published Lawson parameter (nτT) for a p-B11 FRC plasma approaching reactor relevance.
...
| C-2W total plasma temperature (record) | ~3 keV (Ttot) | tae-c2w-machine-details.md §Plasma Parameters | high | Te ~300 eV; Ti substantially higher; experimental record |
...
| Q_plasma required for net electricity (NBI+steam) | [estimated] >> 5–10 | [estimated: derivation in Section 2; NBI wall-plug efficiency ~50–60%; Q_plasma = 5 insufficient for net electricity with steam conversion + NBI overhead; see Section 2 derivation] | low | Exact value depends on NBI beam energy and coupling efficiency |
...
| NBI wall-plug efficiency at Da Vinci beam energies | not-yet-sourced | blocking | Depends on beam energy (>120 keV); negative-ion NBI is ~50–60%; positive-ion is lower at high energy |
