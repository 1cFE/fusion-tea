...
**Frame audit (F-2 compliance):** Every override has been verified for correct per-module vs. whole-plant framing. Relative overrides (C220101, C220102, C220106, CAS21, CAS70) are anchored to the generic reference and scale correctly. Absolute overrides: C220104 and C220108 are Class-U sub-accounts stored as per-module values ($200M and $20M respectively), so Class-U replication delivers the correct whole-plant totals at 1 GWe ($2,000M driver, $200M factory). CAS27 is $0.5M absolute on a Class-P account and is held flat by the framework (CAS27: native=0.5, 1 GWe=0.5 in model output). No other absolute overrides exist on this concept.
...
overrides:
  ...
  - account: C220104
    value: 200.0  # per-module M$ (= $2,000M whole-plant ÷ n_mod=10); Class-U replication → $2,000M at 1 GWe
    enabled: true
    provenance: derived
    ...
    rationale: |
      ...
      Whole-plant driver cost derivation: Marvel states ~500 laser systems for a
      commercial power plant (optics-news-16-4-4 §500 lasers needed). LLNL's LIFE-era
      cost target is <$1.5B for a GW-class driver (nanosecond Nd:glass/Yb:S-FAP).
      Marvel's femtosecond CPA architecture uses a less mature manufacturing base
      (Ti:sapphire-class gain media, CPA gratings) — applying a 1.3–2× technology
      immaturity premium. Midpoint: $1.5B × 1.33 ≈ $2,000M whole-plant. Sensitivity
      range: $1,500M–$3,000M. At 500 beamlines this implies $3–6M per beamline
      (NOAK), consistent with LLNL showing diodes account for 33–50% of beamline
      cost (osti-servlets-purl-3008974 §Section 3).

      Frame: C220104 is a Class-U (per-unit) reactor-island sub-account. The stored
      value is $200M per module (= $2,000M ÷ n_mod=10). The library's Class-U
      replication multiplies by n_mod=10 at 1 GWe, restoring the intended $2,000M
      whole-plant driver cost. A 100 MWe pilot module with ~50 lasers at $3–6M/
      beamline NOAK correctly costs ~$200M at the modular-fleet default scale.
  ...
  - account: C220108
    value: 20.0  # per-module M$ (= $200M whole-plant ÷ n_mod=10); Class-U replication → $200M at 1 GWe
    enabled: true
    provenance: derived
    ...
    rationale: |
      ...
      Whole-plant factory cost derivation: at 1 GWe (10 modules, 10 Hz each) the
      fleet demands 3.15B targets/year = 630,000 wafers/year (at 5,000 targets/wafer
      per dossier §Driver Technology). A dedicated clean-room line at this throughput
      costs $150–300M CAPEX (mature lithography nodes, not leading-edge logic).
      Midpoint: $200M whole-plant. Per-target CAPEX: $200M / 3.15B = $0.063/target,
      vs. 04-laser-icf HB11 factory at ~$5/target — structural semiconductor advantage.

      Frame: C220108 is a Class-U (per-unit) reactor-island sub-account. The stored
      value is $20M per module (= $200M ÷ n_mod=10). The library's Class-U replication
      multiplies by n_mod=10 at 1 GWe, restoring the intended $200M whole-plant target
      factory cost. At the 100 MWe pilot (1 module), $20M represents a pilot-scale
      dedicated lithography cell (63,000 wafers/year) — physically consistent with a
      $15–25M clean-room line at that throughput. Sensitivity range: $150M–$300M
      whole-plant ($15M–$30M per module).
...
**Cost direction: Advantage (Marvel).** Marvel's targets leverage existing semiconductor manufacturing infrastructure and have no per-target electromagnet or cryogenic assembly. The volume is 10× higher (10 Hz vs. 1 Hz) but the per-unit cost is orders of magnitude lower. At 1 GWe scale, target factory CAPEX: ~$200M (concept 23, Class-U replication of $20M/module × 10 modules, serving 3.15B targets/year) vs. ~$157.5M (concept 04, 500 MWe). Marvel's per-target CAPEX is ~$0.06/target vs. concept 04's ~$5/target — a structural advantage from semiconductor lithography, partially offset by the 10× higher throughput requirement.
