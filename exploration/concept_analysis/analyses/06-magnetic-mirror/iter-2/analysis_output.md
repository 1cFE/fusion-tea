...
For cost-model purposes, the thermal efficiency parameter (`eta_th`) must reflect only the residual thermal/radiation fraction — bremsstrahlung and synchrotron radiation deposited on walls rather than reabsorbed by the plasma. The ARPA-E presentation asserts synchrotron radiation is "manageable through reabsorption" [arpa-e-2025-fisch-presentation-notes.md, §Summary of Derisked Questions] but gives no quantitative reabsorption fraction; the expected thermal-wall fraction is ~15–25% of fusion power, with the bulk of energy exiting via charged-particle channels captured by the DEC system. The scoring framework's Direct (charged particle) canonical efficiency applies to the DEC parameter (`eta_de`); `eta_th` must separately capture only the radiation-wall channel — setting `eta_th` to the DEC canonical value conflates two distinct power-balance legs and double-counts the direct conversion benefit.

This conflation is a general framework-table artifact rather than a concept-specific modeling choice: the scoring_framework.md canonical table assigns efficiency values for "Direct (charged particle)" to the `eta_th` row, whereas in the underlying costing model (`costingfe`), DEC efficiency feeds `eta_de` on a separate power-balance leg from the thermal cycle. Any DEC-dominant or aneutronic concept (this analysis, 08-frc-w-direct-conversion, or hybrid-DEC variants) faces the same mismatch. Correct application requires either routing the Direct canonical to `eta_de` rather than `eta_th`, or explicitly invoking the scoring framework's "Justified deviations" clause to retain a physics-grounded residual-thermal value for `eta_th` in the 0.15–0.30 band.
