...
No peer-reviewed plant study has been published for projectile ICF. The Hawker (2020) framework paper (Phil. Trans. R. Soc. A 379, rsta.2020.0053; extracted as `iter-03/sources/pmc-articles-pmc7658748.md`) analyzes the general economics of IFE and is the closest public treatment, though it is concept-agnostic rather than projectile-specific. Authored by FLF's CEO, it provides IFE-relevant parameter ranges (driver cost $2–10/J, plant cost $1,000–6,000/kWe) and Pearson correlation analysis showing driver lifetime is a stronger LCOE lever than driver capital cost.
...
A cost model must also distinguish **driver capital cost from driver operational lifetime** as separate LCOE levers — two failure modes with different time constants and different mitigation paths. Hawker (2020) (pmc-articles-pmc7658748.md §§Correlation Analysis, Table 3) finds that across the IFE design space, driver lifetime has a stronger Pearson correlation with LCOE (−0.134) than driver capital cost (+0.075). The interpretation: a cheap driver that wears out rapidly generates higher lifecycle costs than a more expensive driver with long service life. The paper notes that "reliable operation is the most important aspect for the driver." For a 40-year plant at 0.1 Hz, the launcher executes ~1.3 × 10⁸ shots. Driver capital cost is a one-time CAS22 charge; driver lifetime-driven replacement is a recurring operational cost with no natural CAS slot in conventional plant cost models.

For projectile ICF specifically, the relevant sub-parameters for driver lifetime are: (a) barrel bore erosion rate at 60 km/s — an extreme wear regime with no empirical precedent — (b) sabot and projectile material degradation per shot under acceleration stress, and (c) capacitor bank and switching element replacement frequency under repetitive high-energy cycling. None of these have been published for the Machine 4 design. A driver achieving 10⁵ shots before barrel replacement (~11 days at 0.1 Hz) produces far higher lifecycle costs than one achieving 10⁹ shots (~300 years at 0.1 Hz). This 4-order-of-magnitude span in plausible driver lifetime is a cost uncertainty comparable in magnitude to the driver capital cost uncertainty.
...
Hawker (2020) (pmc-articles-pmc7658748.md §§Dependence on Individual Parameters, Table 3) demonstrates that the gain-LCOE relationship is continuous: "If values near the bottom of the range are accessible, competitive LCOE numbers exist even with gain less than 100." The 200× threshold arises under central/default cost assumptions (driver at $6/J, plant at ~$3,500/kWe); under optimistic cost parameters, gains below 100× can produce competitive LCOE. Conversely, gains above 400× show diminishing LCOE returns. The cost model should sweep gain continuously from 10× to 1000×, not treat 200× as a binary go/no-go gate — though the 200× floor is a reasonable reference scenario under base-case driver and plant costs.
...
**Recommended approach: free-form scenario modeling** with gain, driver capital cost, and driver operational lifetime as the three primary swept parameters, rather than a bottom-up CAS-structured estimate. This is appropriate when the dominant cost uncertainties span orders of magnitude. Hawker (2020) (pmc-articles-pmc7658748.md §§Correlation Analysis, Table 3) provides empirical grounding for the parameter priority ordering: gain (Pearson −0.164) and driver lifetime (−0.134) are the two strongest LCOE correlates, with driver capital cost (+0.075) notably weaker — consistent with the lifecycle cost logic that reliable operation matters more than initial purchase price.

Key hypotheses the cost model should test as testable propositions:

1. **Gain sensitivity (continuous, not binary)**: *LCOE is a continuous function of gain from 10× to 1000×. Under central cost assumptions (driver ~$6/J, plant ~$3,500/kWe), competitive LCOE requires gain ≥ 200×. Under optimistic cost assumptions (driver ~$2/J, plant ~$1,000/kWe), gains below 100× can still yield competitive LCOE. Gains above 400× show diminishing returns.* Test by sweeping gain from 10× to 1000× at three cost scenarios (optimistic, central, pessimistic), not as a binary threshold.

2. **Driver capital cost sensitivity**: *The LCOE is sensitive to driver capital cost (Hawker IFE range: $2–10/J, corresponding to Machine Three at $1.7/J and NIF at $9.5/J); Hawker (2020) Table 3 cites Machine Three at $1.7/J as a non-rep-rated lower bound and NIF at $9.5/J as an upper reference.* Test by sweeping driver CAPEX across this $2–10/J range at fixed gain = 200×.

3. **Driver lifetime sensitivity**: *Driver operational lifetime (shots to failure/replacement) has a stronger LCOE correlation than driver capital cost (Pearson −0.134 vs +0.075). At 0.1 Hz over 40 years, the launcher executes ~1.3 × 10⁸ shots. A driver requiring barrel replacement every 10⁵ shots imposes 1,300 replacement cycles over plant life — potentially dominant over initial capital cost.* Sweep driver lifetime from 10⁴ to 10⁹ shots at fixed gain = 200× and driver capital cost at central scenario.

4. **Rep rate requirement at gain shortfall**: *If gain falls below 200× under central cost assumptions, the rep rate required to maintain 333 MWe output rises, increasing wear on chamber components and the launcher. This is a coupled degradation, not a cliff edge — lower gain requires higher rep rate, which reduces driver lifetime per plant-year, which increases the effective driver lifetime cost.* Model as a coupled constraint, not a binary go/no-go.

5. **Thermal efficiency baseline**: *Hawker (2020) model sweep used 30–60% thermal efficiency (default 40%). Assuming 33–35% steam Rankine efficiency introduces negligible error relative to gain and driver uncertainties; it is not a high-priority sensitivity.* Confirm this hierarchy holds by fixing thermal efficiency at 35% and comparing LCOE sensitivity to gain, driver cost, and driver lifetime sensitivities.

6. **Tritium surplus revenue**: *If tritium byproduct revenue is credited at current scarcity pricing (~$30,000/g), LCOE drops by X $/MWh — test as a scenario branch, not a base case. Include sensitivity to tritium price from $0/g (fleet saturation) to $30,000/g (current scarcity). At 25 kg/year and a 333 MWe plant, the current-price revenue (~$750M/year) approaches 60% of electricity revenue — but this price is structurally self-eroding at fleet scale.*
...
- **Missing at scale**: No prototype of a flowing liquid lithium curtain chamber has been built or operated under fusion neutron flux. The fluid dynamics of maintaining thick curtain geometry under repetitive blast loading from fusion shots is undemonstrated. Liquid lithium compatibility with structural materials at plant lifetime neutron fluence is uncharacterized. FLF-specific lithium pump power has not been published, but a rough bound is available from the HYLIFE ICF reactor EM pump study (UCRL-53356, Baker et al. 1983, osti-servlets-purl-6360934.md): HYLIFE's 11 electromagnetic pumps circulate ~72 m³/s of liquid lithium at ~1.32 MW electrical per pump (~14.5 MW total), suggesting a recirculating pump fraction of roughly 1–2% for an IFE plant at HYLIFE scale. Scaled to FLF's 333 MWe design point, Li pumping is unlikely to be a dominant recirculating power fraction, though chamber geometry differences could shift this estimate.
...
| Driver cost — IFE design space (Hawker model) | $2–10/J | pmc-articles-pmc7658748.md §Model Parameters (Table 2) | medium | Machine Three (non-rep-rated): $1.7/J; NIF (non-rep-rated): $9.5/J; rep-rated operation expected to be higher |
| Plant capital cost — IFE design space (Hawker model) | $1,000–6,000/kWe | pmc-articles-pmc7658748.md §Model Parameters (Table 2) | medium | Model sweep range; provides bracketing context for FLF's <$5B/<500 MWe = ~$10,000/kWe stated target |
| Target cost — competitive threshold (Hawker model) | ~$10/target | pmc-articles-pmc7658748.md §Dependence on Individual Parameters | medium | "Target cost threshold effect at ~$10/target" in model sweep; consistent with Goodin economic bound |
| Net thermal efficiency — IFE model sweep (Hawker) | 30–60% (default 40%) | pmc-articles-pmc7658748.md §Model Parameters (Table 2) | medium | Steam Rankine baseline (33–35%) is within this range; Li loop losses are the incremental uncertainty |
...
| Driver operational lifetime / shot count to failure | truly-unknown | blocking | Hawker (2020) finds driver lifetime (Pearson −0.134) is a stronger LCOE lever than driver capital cost (+0.075). Bore erosion at 60 km/s, sabot degradation, capacitor replacement frequency all uncharacterized for Machine 4 design |
...
| Net thermal efficiency (full cycle) | sourced (Hawker 2020) | important | Hawker (2020) model sweep: 30–60%, default 40% (pmc-articles-pmc7658748.md §Table 2). Steam Rankine baseline (33–35%) is within this range; Li loop losses are the residual uncertainty but not a priority sensitivity relative to gain and driver lifetime |
...
| 6 | Net thermal efficiency including Li loop losses | S5 | sourced | important | Hawker (2020) (pmc-articles-pmc7658748.md §Model Parameters, Table 2) provides model sweep range: 30–60%, default 40%. Steam Rankine baseline (33–35%) falls within this range; Li loop losses are the residual gap. Not a high-priority sensitivity relative to gain and driver lifetime |
...
| 8 | Liquid lithium pump power and capital cost | S3, S4, S5 | partially-sourced | important | UCRL-53356 (Baker et al., LLNL 1983, osti-servlets-purl-6360934.md §Abstract, §HYLIFE Design Point) provides EM pump design for HYLIFE liquid-Li ICF: 8.08 m³/s per pump, 82.5 kPa head, 50–60% efficiency, 1.32 MW electrical per pump. At 11 pumps for ~72 m³/s total HYLIFE flow (~14.5 MW pump power total), the recirculating fraction at HYLIFE scale is roughly 1–2% of plant output. This is the closest available analogue — same fluid (liquid Li), same reactor type (IFE). FLF-specific flow rates and chamber geometry remain unpublished |
...
6. **Hawker, N.J. (2020). "A simplified economic model for inertial fusion energy."** Phil. Trans. R. Soc. A 379, rsta.2020.0053. Available as `iter-03/sources/pmc-articles-pmc7658748.md`. Authored by FLF's CEO; the primary peer-reviewed analytical foundation for FLF's economic arguments. Key data used in this analysis: IFE driver cost range $2–10/J (Machine Three $1.7/J, NIF $9.5/J; §text following Eq. 2.7), plant cost range $1,000–6,000/kWe and target cost ~$10/target threshold (§Table 2), thermal efficiency model sweep 30–60% default 40% (§Table 2), and Pearson correlation coefficients for LCOE sensitivity: gain −0.164, driver lifetime −0.134, driver capital cost +0.075 (§§Correlation Analysis, Table 3). The paper demonstrates that the gain-LCOE relationship is continuous, not binary, with competitive LCOE achievable below 100× under optimistic cost parameters.
...
11. **Baker, R.S. et al. (1983). "Design of an electromagnetic pump for the HYLIFE ICF reactor."** UCRL-53356, Lawrence Livermore National Laboratory. Available as `iter-03/sources/osti-servlets-purl-6360934.md`. Provides detailed EM pump engineering for liquid lithium ICF circulation: 8.08 m³/s flow rate per pump, 82.5 kPa head, 50–60% expected pump efficiency for HREMP and ALIP designs, 1.32 MW electrical input per pump. Used in this analysis to rough-bound the recirculating pump power fraction for FLF's liquid lithium curtain system. Best available analogue: same fluid (liquid Li), same reactor class (IFE).
