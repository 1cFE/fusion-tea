...
overrides:
  - account: C220103
    ...
    rationale: |
      ...
      (per-module cost for the 90 MWe native plant). Breakdown: REBCO tape at
      $40/kA·m, Inconel 718 structure, 5× fabrication factor for superconducting
      components. Library default is geometry-based and may not capture demountable
      joint costs or window-pane coil geometry. Class U account — library scales by
      n_mod for the 1 GWe fleet.
  ...
  - account: C220108
    ...
    source: "manta-reference-design.md §7.1, Table C1"
    rationale: |
      MANTA publishes divertor cost at $150M for the double-null tungsten monoblock
      design (per-module cost for the 90 MWe native plant). NT-specific geometry and
      reduced heat load (P_SOL = 23.5 MW vs. ~80 MW for conventional PT tokamaks)
      may reduce replacement frequency, but capital cost is explicitly stated.
      Class U account — library scales by n_mod for the 1 GWe fleet.
...
Expected band for Archetype-Fit = High is 0-4 enabled overrides. Actual enabled count: 3 (C220103, C220108, CAS27). Within expected range.
...
- **C220104** (supplementary heating): MANTA specifies $370M for 40 MW ICRF at 110 MHz (He-3 minority). However, Ball et al. demonstrate ohmic-only operation is viable at Q = 500 for the same device parameters. The heating requirement is genuinely uncertain; without a clear baseline case, no override is justified and the library default is retained. If ohmic-only operation is validated, the ICRF cost disappears; if auxiliary heating is confirmed, $370M is the design-point figure.
...
- **CAS21** (buildings): MANTA assumes brownfield site saving ~$400M vs. greenfield, but this is a siting assumption, not a design-point feature. Library default retained.
...
- **CAS70** (O&M): MANTA assumes ~1 person/MWe staffing at $150k/employee-year (~$15M/yr for 90 MWe), approximately 30% below typical tokamak staffing estimates. However, CAS70 is not overridable in 1costingFE today — overrides on this account are silently dropped (1cFE/1costingfe#106). Library default retained; no override authored.
