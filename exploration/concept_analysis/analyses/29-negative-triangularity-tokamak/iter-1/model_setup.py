...
spec = dict(
    ...
    p_input=40,        # ICRF auxiliary heating wallplug (MW) — manta-reference-design.md §2, Table 1
                       # Note: Ball et al. claim ohmic-only (0 MW) viable at Q=500, but
                       # MANTA reference retains 40 MW ICRF (110 MHz, He-3 minority heating)
...
overrides = [
    ...
    {"account": "CAS70", "value": 0.70 * generic.costs.cas70, "enabled": True,
     "provenance": "derived", "source": "manta-reference-design.md §7.2, Table C5",
     "cost_basis": "noak",
     "rationale": "MANTA assumes ~1 person/MWe staffing at $150k/employee-year = ~$15M/yr "
