...
|-----------|-------|--------|------------|------|
| Major radius R0 | 8.0 m | aip-2023-paper-abstract.md §II, Table I line 129 | high | spec key: `R0` |
| Minor radius a | 2.0 m | aip-2023-paper-abstract.md §II, Table I line 130 | high | spec key: `plasma_t` |
| Aspect ratio A | 4.0 | [inferred: R0/a = 8.0/2.0] | high | informational only |
...
| Plasma volume | ~320 m³ | [inferred: 2π²Raκ ≈ 2π² × 8 × 2 × 1 ≈ 316 m³] | medium | informational — library back-solves from R0, a, elon |
...
| External heating power (ECH) | 84 MW (wall-plug) | [inferred: 250 MW fusion / Q_eng=2.0 = 125 MW coupled to plasma; at 50% gyrotron efficiency (line 268), wall-plug = 125/0.5/η_wp ≈ 84 MW, but see note] | medium | spec key: `p_input` — auxiliary heating wall-plug power, NOT fusion power |
...
| Gyrotron count and power | 60× 250 GHz, 1 MW CW gyrotrons across 10 ports | aip-2023-paper-abstract.md §II.D, lines 262-264 | high | concept-specific ECRH architecture; 60 MW total CW output at 50% wall-plug efficiency → 120 MW wall-plug; discrepancy with 84 MW ECH above suggests Q_eng calculation in AIP paper may include non-ECH recirculating loads |
...
**Derivation chain for p_input**: The AIP paper states Q_eng = 2.0 and P_fus = 250 MW (Table I). By definition, Q_eng = P_fus / P_recirculating. Therefore, P_recirculating = 250 MW / 2.0 = 125 MW. However, the paper also states 60 gyrotrons × 1 MW = 60 MW CW microwave output, and gyrotron wall-plug efficiency η_wp = 50% (line 268), so ECH wall-plug = 60 MW / 0.5 = 120 MW. The discrepancy (125 MW vs. 120 MW recirculating) suggests Q_eng includes non-ECH recirculating loads (cryogenic pumps, liquid metal circulation, controls). For the `p_input` spec kwarg, we use 120 MW as the auxiliary heating wall-plug power (the library will add cryogenic and other house loads separately). **Confidence: medium** due to this ambiguity.
...
overrides:
  - account: C220103
    ...
    rationale: |
      ...
      ~8 m major radius with two continuous helical coils yields no cost anchor. The
...
| 18 | Vacuum vessel mass, structural analysis, and cost for heliotron geometry at 8 m major radius | S3, S5b | not-yet-sourced | nice-to-have | LHD vacuum vessel as-built cost inflated to current year; or ITER vessel cost adjusted for size and geometry differences |
...
Thea Energy's planar-coil stellarator uses an array of simple flat HTS coils arranged in a toroidal configuration, with field shaping achieved by current distribution rather than coil geometry. HESTIA uses two continuous helical HTS coils that wind through a complex 3D path (double-helix configuration). The planar-coil approach trades physics optimization (less confinement quality per unit field strength) for manufacturing simplicity (each coil is a flat pancake, easy to wind and stack). The helical-coil approach achieves better plasma confinement (heliotron physics is well-validated by LHD) but requires flexible REBCO conductor (WISE technology) and specialized winding machinery.
