...
Design-Point-Name: HESTIA Fusion Pilot Plant — reference operating case (Miyazawa & Goto, Phys. Plasmas 2023)
...
**Note on p_input and Q_eng**: The AIP paper explicitly states (lines 268-272) that 40 MW wall-plug electricity is required to inject 20 MW of ECH power into HESTIA via 60 gyrotrons operated alternately at 50% wall-plug efficiency. For the `p_input` spec kwarg, we use 20 MW as the delivered power to plasma (Table I line 155, P_VCM = 20 MW). The Q_eng = 2.0 stated in Table I implies P_recirculating = P_fus / Q_eng = 250 MW / 2.0 = 125 MW total recirculating power, which includes the 40 MW ECH wall-plug (delivering 20 MW to plasma) plus ~85 MW of other house loads (cryogenic refrigeration, liquid metal circulation pumps, controls, etc.). The library will compute these additional recirculating loads internally based on the plasma physics and engineering systems.
