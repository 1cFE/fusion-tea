...
**Top three LCOE levers (ranked by sensitivity)**:
...
- **H3**: 10 Hz is a sub-optimal rep rate for COE minimization (Meier optimum: 20–25 Hz); the 10 Hz constraint imposes approximately +4% COE penalty — test by sweeping rep rate from 5 to 25 Hz
...
**04-laser-icf (HB11 Energy — p-B11 fast ignition)** is structurally the most direct parallel: it shares the fast ignition physics step (petawatt ignition laser generating an energetic particle beam to heat a compressed core) and the dual-laser architecture (compression driver + short-pulse ignitor). Key differences: HB11 uses p-B11 fuel (aneutronic, no tritium breeding, no 14 MeV neutron load) and relies on laser-driven magnetic field amplification rather than TNSA proton acceleration. The dual-driver cost uncertainty is structurally identical, but the p-B11 reaction cross-section is far lower than D-T, requiring even higher gain — making HB11 arguably the higher-risk variant on the fast ignition physics step. The D-T fuel cycle in Focused Energy vs. aneutronic p-B11 in HB11 creates fundamentally different cost structures (tritium breeding, neutron management) outside the shared driver architecture.

The handwritten exemplar for **26-laser-icf-indirect-drive** (Inertia Enterprises / Xcimer) provides the most directly relevant cross-concept data for IFE economics (DPSSL cost class, chamber, target economics). Key divergences for fast ignition:

- *Driver technology*: Indirect drive uses DPSSL (Inertia, ~$700–1,000/J) or KrF excimer (Xcimer, ~$100–120/J FOAK). Fast ignition (Focused Energy) uses DPSSL compression + petawatt ignition — potentially two separate capital line items vs. one driver system. The DPSSL cost class is shared with Inertia's approach; the petawatt laser adds cost with no analogue.
- *Rep rate*: Focused Energy (10 Hz) is much closer to Inertia (10 Hz) than to Xcimer (sub-Hz). Chamber clearing requirements are similarly demanding.
- *Target complexity*: CHS direct-drive capsules are symmetric spheres. Fast ignition cone-in-shell capsules are inherently asymmetric and more complex to fabricate, likely more expensive per unit. Inertia's $1/target target cost cannot be assumed transferable.
...
- *Energy conversion*: All IFE concepts using D-T converge on thermal steam cycles. This is shared infrastructure and can be cost-analogued from conventional nuclear BOP data.
- *Tritium breeding*: Focused Energy's undisclosed blanket vs. Xcimer's well-characterized FLiBe HYLIFE-III design. FLiBe cost data from HYLIFE literature (cross-concept memory note on sparse FLiBe data) cannot be applied here without blanket type confirmation.

**Key implication for cross-concept comparisons**: Focused Energy's fast ignition concept is strictly more speculative than its nearest IFE neighbors, with an additional undemonstrated physics step (proton fast ignition) on top of the already-undemonstrated direct-drive ignition. The TEA should flag this as a risk multiplier on any cost estimate derived from CHS IFE analogues.
