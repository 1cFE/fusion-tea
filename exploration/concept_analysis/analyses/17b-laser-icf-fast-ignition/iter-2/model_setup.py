...
# ── Repetition Rate Sweep (F-3) ───────────────────────────────────────
# Meier 2006 (osti-servlets-purl-1438678) finds COE is minimized at 20–25 Hz
# for both CI and FI configurations. Operating at 10 Hz (Focused Energy
# commercial target) incurs approximately +4% COE penalty vs. 20 Hz optimum;
# 5 Hz incurs approximately +12% penalty.
# Focused Energy discloses 10 Hz as the commercial target (Callahan interview),
# but this is an engineering-constrained design point, not the economic optimum.
# Source: osti-servlets-purl-1438678.md §Systems modeling; focused-energy-callahan-interview.md §Rep rate.
print("\nRepetition Rate Sweep")
print("(Meier 2006: COE minimized at 20-25 Hz; 10 Hz = Focused Energy target, ~+4% vs. optimum)")
...
for f_hz in [5.0, 10.0, 15.0, 20.0, 25.0]:
    ...
    note = ""
    if f_hz == 10.0:
        note = "← Focused Energy target (~+4% vs. Meier optimum)"
    elif f_hz == 20.0:
        note = "← Meier 2006 COE-minimum"
    elif f_hz == 5.0:
        note = "(~+12% vs. Meier optimum)"
