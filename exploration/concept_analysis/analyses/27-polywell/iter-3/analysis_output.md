...
The entire Park 2025 reactor design hinges on γ = 0.1, described as "a parametric expression to represent the reduction in the energy loss rate" from potential-well formation. The authors acknowledge: *"currently, we lack a quantitative model for the reduction in the loss rate"* [1]. The γ parameter couples both beam power and fusion power through the Park 2025 scaling relations — a higher γ increases the required beam power for a given confinement state. This coupling creates two distinct operational branches that must not be conflated in cost modeling:

- **Branch A (fixed fusion output, higher beam power):** If γ = 0.2 and the plant is sized to deliver 980 MW fusion, beam input rises to ~156 MW (vs. 78 MW at γ = 0.1). Recirculating power fraction roughly doubles; net electric output falls from ~300 MWe to ~220 MWe. LCOE penalty: moderate, plant still generates power.
- **Branch B (fixed beam power, lower fusion output):** If γ = 0.2 and beam power is held at 78 MW, fusion power falls to ~490 MW. A plant designed for 300 MWe now produces ~125 MWe. LCOE penalty: severe, because capital cost is fixed while output is halved — an underutilization penalty that compounds the recirculating power penalty.

A sensitivity sweep that varies only fusion power (branch B) while holding beam power fixed does not represent branch A; mixing these in different model scenarios means no scenario correctly represents true γ-deviation physics. Any cost model must implement a parameterized γ that updates both p_fus and p_beam simultaneously using the Park 2025 scaling relation. Sensitivity: γ = 0.05 requires only 39 MW input (same 980 MW fusion); γ = 0.2 forces the branch A / branch B choice described above. At γ = 0.2 in either branch, Q_plasma falls to ~6.3; engineering Q could approach breakeven once conversion losses are included. The 2019 University of Sydney experiments found *"little or no trace of virtual electrode formation"* at higher plasma densities, and calculated that electron supply rates of 200,000 A would be required to sustain a virtual cathode at commercial plasma densities [2]. EMC2 disputes this but has not published counter-experimental evidence. A cost model cannot be anchored to Park 2025's performance projections without a validated γ.
...
The three critical physics bets driving Polywell TEA uncertainty, stated as decision-gate hypotheses:

**H1 (γ / confinement):** Wiffle-Ball confinement achieves γ ≤ 0.1 at commercial plasma density (~10²¹ m⁻³). *Test vehicle*: FPNS program with high-beta operation. *Invalidation criterion*: If FPNS data reveals γ > 0.1, both branch A and branch B scenarios for a 980 MW plant deteriorate to near-breakeven or below, invalidating the moderate scenario in the cost model. *Timeline*: FPNS Phase 1 (~2 years from $20M program start).

**H2 (virtual cathode):** Virtual cathode formation is achievable at commercial densities without the ~200,000 A electron supply rate calculated by University of Sydney (2019). *Test vehicle*: Independent experiment at reactor-relevant plasma density, or published EMC2 counter-data from WB-8 or subsequent devices. *Invalidation criterion*: If the Sydney calculation is confirmed, the core confinement mechanism fails regardless of γ — the device cannot sustain the potential well that accelerates ions, collapsing the physics basis entirely. *Current status*: No counter-experimental evidence published; EMC2 disputes the calculation but has not provided data.

**H3 (bremsstrahlung):** Bremsstrahlung radiation does not eliminate net energy gain for a thermalized 20 keV D-T plasma at reactor density. *Test vehicle*: First-principles calculation using Park 2025's thermalized plasma assumption (not Rider 1995's non-Maxwellian assumption, which Park 2025 supersedes by assuming thermalization). *Invalidation criterion*: If bremsstrahlung losses exceed ~5–10% of fusion power at 20 keV, gross thermal output is reduced correspondingly — a significant but non-blocking penalty for LCOE. *Note*: This hypothesis is partially resolved by the thermalization assumption in Park 2025; however, the implication (thermalized plasma eliminates any direct-conversion advantage) is not acknowledged in the Park 2025 analysis.
...
- **Cost implication (HIGH UNCERT)**: The Polywell blanket is a concept-specific engineering challenge, not a generic MFE component. ARIES-class D-T reactor studies estimate blanket first-wall costs at $50–150M for comparable neutron power levels in standard toroidal geometry. The polyhedral geometry, with its shadowing problem, is likely to require thicker or higher-enrichment lithium sections and may cost more, not less, than a standard blanket. A generic MFE analogue formula (producing ~$7M) is inapplicable here and should not be used as the cost basis without explicit HIGH UNCERT flagging and a wide uncertainty range ($50–200M).
...
| Net electric output (approx., γ=0.1) | ~300 MWe | [inferred: 980 MW × 40% thermal efficiency − 78 MW recirculating at ~80% beam supply efficiency] | very low | Branch A baseline; blocking uncertainties on every component |
| Net electric output (approx., γ=0.2 Branch A) | ~220 MWe | [inferred: 980 MW × 40% − 156 MW recirculating; p_fus held constant, p_beam doubles] | very low | Fixed-output scenario; recirculating fraction ~29% → ~50% |
| Net electric output (approx., γ=0.2 Branch B) | ~125 MWe | [inferred: 490 MW × 40% − 78 MW recirculating; p_beam held constant, p_fus halved] | very low | Fixed-beam scenario; capital underutilization penalty compounds LCOE |
| Engineering Q_eng (approx.) | ~2–4 | [inferred: gross electric / recirculating; wide range reflects γ branch and beam efficiency uncertainty] | very low | Plausible if physics holds; Branch B at γ=0.2 approaches Q_eng ~1.5 |
...
| 5 | Tritium breeding blanket design — coil neutron shadowing creates geometry-specific challenge with no proposed solution; blanket cost is HIGH UNCERT ($50–200M range vs. ~$7M generic analogue) and is a major cost driver not reflected by standard formula | S3, S4, S5 | truly-unknown | blocking | Neutronics analysis for polyhedral cusp geometry; no public study identified; ARIES-class studies at comparable neutron power indicate $50–150M as reference cost range |
