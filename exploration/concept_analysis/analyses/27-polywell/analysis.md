...
**Reference Design**: Park et al. (2025) D-T reactor scaling study
**Native Scale**: ~290 MWe net electric (derived from 980 MW fusion power, 45% thermal efficiency, 78 MW electron beam injection)
**Maturity**: Theoretical scaling (Q=10 physics model, no engineering design)
**Grounding Confidence**: Medium (physics parameters from peer-reviewed PIC simulation; engineering parameters assumed)

**Critical Limitation**: This is not an EMC2 power plant design. It is a theoretical reactor scaling study establishing a Q=10 **D-T** reference case (50:50 D-T fuel mixture, 14.1 MeV neutrons, tritium breeding blanket required). No engineering subsystem designs, cost breakdowns, or balance-of-plant specifications exist. All derived cost estimates use library defaults with zero concept-specific overrides. The fuel type is D-T per Park et al. (2025) and the dossier; any cost model using p-B11 would fundamentally misrepresent this design point (D-T requires tritium breeding, fuel cycle infrastructure, and neutron shielding absent in aneutronic fuels).
...
**Parameter chain uncertainty**: Every value downstream of "Electron beam input power" depends on the γ=0.1 assumption, which has **never been validated experimentally**. If γ=0.2, electron beam input doubles to 156 MW, wall-plug power becomes 195 MW, and net electric drops to ~193 MWe (halving plant output). If γ=0.05, net electric increases to ~368 MWe. The uncertainty range on net power is **±60%**. **This is not a measurement uncertainty — it is an unmeasured free parameter that Park et al. (2025) derives from "qualitative interpretation of PIC simulation results."** The cost model should bound this uncertainty with sensitivity scenarios at γ=0.05 (optimistic), 0.1 (baseline), and 0.2 (pessimistic) to show how this single physics assumption propagates to LCOE. Without experimental validation of γ, the nominal P_native = 290 MWe is speculative, and cost estimates derived from it should be interpreted as illustrative of the archetype's cost structure, not as a grounded plant design.
...
**Override count vs. rubric**: Archetype-Fit is Med → expected 3-8 overrides. Actual count is **zero**, well below the band. This discrepancy reflects a mismatch between archetype-fit grading criteria and data availability:

- **Why Med fit was assigned** (upstream): The POLYWELL archetype exists as a distinct cost model (separate from generic IEC or mirror archetypes), implying EMC2's design has concept-specific features that differentiate costs. The polyhedral coil geometry, electron beam injection, and cusp confinement are unique subsystem configurations. In principle, if EMC2 published a power plant design, these features would justify multiple overrides.

- **Why zero overrides result**: Park et al. (2025) provides only physics scaling parameters (geometry, fields, power levels), not engineering cost data. No magnet procurement costs, no blanket design, no balance-of-plant specifications exist in the literature. The analysis walks every canonical account and finds no company-grounded quantities to override with.

- **Resolution**: **For cost modeling purposes, the effective archetype-fit is Low (library defaults only), not Med.** The upstream Med grade anticipates future data availability if EMC2 publishes an engineering design, but the current evidence supports zero overrides. If this were an engineering design (comparable to ARIES-AT for tokamaks), the Med fit would be appropriate. For a theoretical physics scaling study, the cost model will produce a **generic POLYWELL archetype estimate** using library defaults for all accounts, with the understanding that this is a placeholder pending EMC2 engineering work. The zero-override outcome is honest to the evidence, even if it underruns the rubric expectation. **Upstream re-grading to Low would better reflect current data availability for cost modeling.**
