...
**Critical Limitation**: This is not an EMC2 power plant design. It is a theoretical reactor scaling study establishing a Q=10 D-T reference case. No engineering subsystem designs, cost breakdowns, or balance-of-plant specifications exist. All derived cost estimates use library defaults with zero concept-specific overrides.
...
- **Resolution**: The archetype-fit grade anticipates future data availability, not current state. If this were an engineering design (comparable to ARIES-AT for tokamaks), the Med fit would be appropriate. For a theoretical physics scaling study, the fit effectively degrades to Low (library defaults only). The cost model will produce a **generic POLYWELL archetype estimate** using library defaults for all accounts, with the understanding that this is a placeholder pending EMC2 engineering work. The zero-override outcome is honest to the evidence, even if it underruns the rubric expectation.
