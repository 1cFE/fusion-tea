...
class PolywellPlantParams:
    ...
    def _compute_cas22(self, power: dict, geom: dict) -> dict:
        ...
        # Formula: blanket_unit_cost_dt × volume × (p_th / P_TH_REF)^0.6
...
def main():
    ...
    # Conservative: γ=0.2 → Q~6.3, p_fus~490 MW (Park 2025 worst-case scenario)
    # p_beam doubles to 156 MW at same fusion power, OR p_fus halves at same beam power.
    # Interpretation: same 78 MW beam, but lower γ halves the confinement quality.
    conservative = PolywellPlantParams(
        p_fus_MW=490.0,             # γ=0.2, Q~6.3
        p_beam_MW=78.0,             # beam input unchanged
    ...
    sweeps = [
        ("p_fus_MW",
         ...
         "Fusion power [MW] — proxy for γ (Q scales proportionally)"),
        ("thermal_efficiency",
