...
This script:
  ...
  2. Derives thermal efficiency from the published power balance (800 MW fusion
     × 1.10 blanket multiplier → ~880 MW thermal; net 350 MWe + ~65 MWe
     estimated recirculating → ~415 MWe gross; η_th ≈ 47%).  Conservative
     central estimate of 0.40 is used (below the 47% implied value); actual
     Rankine efficiency is uncertain.  The published bound "> 30%" is a floor,
     not the design operating point.
...
- mn = 1.10 (canonical) — D-T canonical per scoring_framework.md; HCPB+Be may reach 1.15 with Tier-A cite
- eta_th = 0.40 — UNCERTAIN; conservative below implied 47% (415/880 MW); Rankine practice uncertain
...
AVAILABILITY = 0.85         # CANONICAL: project-wide policy value per scoring_framework.md
                            # §Plant availability (MCF steady-state, D-T).  Previously 0.87
                            # (unprincipled — "between 85–90% MCF range, shifted toward upper half");
                            # no published Type One Energy availability target qualifies as a
                            # Tier-A override to the canonical 0.85.  2-year operating cycle gives
                            # ~96% theoretical maximum (730/760 days) as aspirational upper bound.
                            # Source: scoring_framework.md §Plant availability
...
MN = 1.10                   # CANONICAL: D-T neutron energy multiplier per scoring_framework.md
                            # §"Blanket energy multiplication".  HCPB+Be designs may achieve
                            # 1.10–1.20 in EU-DEMO neutronics literature; 1.15 would require a
                            # Tier-A external cite.  TBR=1.30 (HCPB+Be, OpenMC) is a separate
                            # parameter and unaffected by this choice.
                            # Source: scoring_framework.md §Blanket energy multiplication;
...
# Availability (elasticity ≈ −0.93) is the second-highest LCOE sensitivity
# parameter, nearly equal to 3D HTS coil cost (elasticity ≈ +0.99).  The
# 2-year operating cycle implies a ~96% theoretical maximum (730/760 days),
# but actual availability from ECRH failures, tritium processing interruptions,
# and island divertor degradation is unknown.  Four scenario points bracket the
# realistic range:
#   80% — pessimistic, early-stage D-T MCF plant (Araiinejad & Shirvan 2025)
#   85% — canonical central case (scoring_framework.md §Plant availability; MCF steady-state, D-T)
#   93% — mid-range, comparable to mature baseload thermal plant
#   96% — aspirational, 2-year cycle theoretical maximum
_shared_avail = {k: v for k, v in _SHARED_KWARGS.items() if k != "availability"}

print()
print("=" * 60)
print("Availability Scenario Sweep (elasticity ≈ −0.93)")
print("=" * 60)
print(f"  {'Scenario':<20} {'Availability':>14} {'LCOE 350MWe':>14} {'LCOE 1GW':>12}")
print("  " + "-" * 62)
_avail_scenarios = [
    ("pessimistic", 0.80),
    ("canonical (central)", 0.85),
    ("mid-range", 0.93),
    ("aspirational", 0.96),
]
_avail_lcoe_native = []
for label, av in _avail_scenarios:
    _ra = model.forward(net_electric_mw=NET_ELECTRIC_MW, availability=av, **_shared_avail)
    _ra1gw = model.forward(
        net_electric_mw=1000.0, override_reference_mw=NET_ELECTRIC_MW,
        availability=av, **_shared_avail,
    )
    _avail_lcoe_native.append(_ra.costs.lcoe)
    _tag = "  ← default" if abs(av - AVAILABILITY) < 0.001 else ""
    print(f"  {label + _tag:<34} {av:.0%} {_ra.costs.lcoe:>12.1f} $/MWh {_ra1gw.costs.lcoe:>8.1f} $/MWh")
print()
print(f"  LCOE range (350 MWe): {min(_avail_lcoe_native):.0f}–{max(_avail_lcoe_native):.0f} $/MWh across availability scenarios")
print(f"  Both coil cost and availability uncertainty produce comparable LCOE ranges;"
      f" neither should be treated as a fixed assumption.")
