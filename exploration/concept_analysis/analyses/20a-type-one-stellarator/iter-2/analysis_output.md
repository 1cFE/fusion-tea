...
**TEA cost direction**: Island divertors represent a likely **cost penalty** relative to conventional tokamak divertors on two accounts. Capital (CAS22): the 3D target geometry has no published unit cost, the only manufacturing reference is W7-X-scale LTS equipment, and no commercial supply chain exists for island divertor targets at burning-plasma scale. O&M (CAS70): targets operate under high continuous steady-state heat flux for 2-year exposures before replacement access — a more demanding endurance requirement than the periodically-accessible tokamak divertor. Magnitude is unknown and must be treated as an upward pressure on both CAS22 and CAS70 in any cost model.
