# 1costingfe Model Setup: Type One Stellarator (Type One Energy)

You are generating a runnable 1costingfe model setup script for **Type One Stellarator (Type One Energy)**
(Type One Energy). The script must run via `uv run python model_setup.py` and emit an
LCOE estimate.
...
**Start here.** Open `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\20a-type-one-stellarator\analysis.md` and read its `## Design Point` block and
...
## Output
Write the script to: `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\20a-type-one-stellarator\iter-1\model_setup.py`
