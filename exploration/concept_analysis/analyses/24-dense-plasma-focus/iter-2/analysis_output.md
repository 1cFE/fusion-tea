...
**Enabled overrides: 6** (CAS21, CAS23, CAS24, CAS26, CAS27, CAS80)

The archetype-fit is Low, expecting 6–12 enabled overrides. My count of 6 falls within the expected band. CAS70 is proposed but disabled due to a framework limitation (the 1costingFE library does not currently inject operating-cost overrides via the same path as capital costs — see the CAS70 entry above). If CAS70 support is added, the enabled count would rise to 7. The overrides are heavily concentrated in the "this concept is radically different from any conventional fusion plant" category — zeroing out or drastically reducing accounts that assume utility-scale thermal-cycle infrastructure. Notably absent are overrides for C220104/C220107 (driver) and C220109 (DEC), where the concept is most novel but where no company-grounded cost data exists to construct an accountable departure from library defaults.
