...
overrides:
  ...
  # p-B11 is aneutronic: no tritium breeding blanket is needed or possible.
  # The x-ray photoelectric converter captures x-ray energy directly as part of the
  # integrated device (dimensions 40–50 cm inner radius, ~50 cm length per module).
  # A thin radiation liner is required for secondary neutron side reactions (<1% of
  # fusion energy) and for structural enclosure, but no breeding blanket or neutron
  ...
  - account: C220101
    ...
    rationale: |
      ...
      or structurally meaningful for this design. The x-ray photovoltaic converter
      (40–50 cm inner radius, ~50 cm length) captures x-ray energy directly and is
      integral to the device. A thin structural liner at ~4% of the library's per-module
      blanket cost accounts for minimal radiation shielding of secondary neutrons.
      Library's modular-fleet default for this account assumes a tritium-breeding
      blanket or energy-capture blanket with significant neutron loading — not
      applicable here.

  # C220102 — Radiation shield
  # <1% neutron energy fraction means biological shielding requirements are minimal.
  # Side-reaction neutrons have modest flux; thin concrete or borated shield adequate.
  # Class U (per-unit).
  - account: C220102
    ...
    rationale: |
      ...
      costly containment structure" and is "safe enough to place in residential
      neighborhoods." Secondary neutrons from side reactions represent <1% of total
      fusion energy output. A thin biological shield for secondary neutron flux and
      x-ray scatter suffices — no heavy neutron shielding is needed. Library's
      per-module shield default is sized for significant neutron wall loading; ~5% of
      that default covers the minimal shield required for this design in the
      modular-fleet context.
  ...
  # Per-module cap bank cost implied: ~$400K. Current commercial capacitor pricing of
  # ~$5/J × 115 kJ stored ≈ $575K/module — library default is approximately in this
  # range. The <$1M total-device claim includes engineering, assembly, and controls at
  # costs far below conventional pulsed-power infrastructure, implying the library's
  # default (built from infrastructure-scale pulsed power projects) significantly
  # overstates the NOAK mass-production cost.
  # Class U (per-unit).
  - account: C220107
    ...
    rationale: |
      ...
      allocates ~40% of the $1M device cost to the capacitor bank = ~$400K per module.
      The library's default $/J estimate for a pulsed-power bank, derived from
      infrastructure-scale projects, likely exceeds this mass-production target by a factor
      of ~2.5× once NOAK learning is applied. Override to 0.40× the library's per-module
      fleet default reflects this departure. Caveat: the company's $1M figure is a
      paper-concept mass-production projection, not a demonstrated cost; low confidence.
      Anchored to the library's 1 GWe modular-fleet default (200 modules × cost per module).
...
**Override count: 9 enabled overrides.** Target band for Low archetype-fit is 6–12. Count is within band.
