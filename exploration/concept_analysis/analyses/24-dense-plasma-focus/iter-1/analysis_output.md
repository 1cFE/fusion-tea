...
No override. The device is extremely compact (~3 tonnes, ~30 m³) with no external magnets, no vacuum vessel in the tokamak sense, and minimal structural support requirements. The structural costs would be far below library defaults for any conventional fusion concept, but no published cost figure exists.
...
No override. The device's small size and low/no activation (aneutronic fuel) would dramatically reduce remote handling requirements compared to D-T concepts. Monthly electrode replacement is envisioned as contact maintenance, not remote handling. No company-grounded cost figure.
...
```yaml
  - account: CAS80
    ...
    rationale: |
      ...
      Even at this level, fuel cost is negligible relative to capital
      amortization. Natural boron is abundant; isotopic enrichment to
      99.9% B-11 is the cost driver.
```

### Override Count Check

**Enabled overrides: 7** (CAS21, CAS23, CAS24, CAS26, CAS27, CAS70, CAS80)

The archetype-fit is Low, expecting 6–12 enabled overrides. My count of 7 falls within the expected band. The overrides are heavily concentrated in the "this concept is radically different from any conventional fusion plant" category — zeroing out or drastically reducing accounts that assume utility-scale thermal-cycle infrastructure. Notably absent are overrides for C220104/C220107 (driver) and C220109 (DEC), where the concept is most novel but where no company-grounded cost data exists to construct an accountable departure from library defaults.
...
The Dense Plasma Focus is classified under confinement family "Other" — it does not fit cleanly into MFE, IFE, MIF, or Electrostatic categories. The plasma is self-confined by its own current-generated magnetic fields for ~10 ns, at near-solid densities, in a geometry (plasmoid at the anode tip) that has no analogue among the other 38 concepts in the corpus.

While no formal comparable exists, the concept's structural uniqueness relative to the broader corpus can be characterized along several dimensions:
