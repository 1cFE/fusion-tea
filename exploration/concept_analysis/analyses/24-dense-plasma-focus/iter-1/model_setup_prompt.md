# 1costingfe Model Setup: Dense Plasma Focus (LPP Fusion)

You are generating a runnable 1costingfe model setup script for **Dense Plasma Focus (LPP Fusion)**
(LPPFusion). The script must run via `uv run python model_setup.py` and emit an
LCOE estimate.
...
**Start here.** Open `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\24-dense-plasma-focus\analysis.md` and read its `## Design Point` block and
...
3. **`spec` carries only design-point inputs.** Do NOT put the uniform
   ...
   **Archetype-specific spec key blocklist (workarounds for known library
   ...
   - **DIPOLE** (`DENSE_PLASMA_FOCUS == "DIPOLE"`): do **NOT** pass
   ...
   - **MIF concepts** (`DENSE_PLASMA_FOCUS ∈ {MAG_TARGET, MAGLIF,
...
## Output
Write the script to: `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\24-dense-plasma-focus\iter-1\model_setup.py`
