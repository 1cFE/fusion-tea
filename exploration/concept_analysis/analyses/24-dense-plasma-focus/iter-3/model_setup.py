...
#    7 enabled overrides (CAS70 + CAS80 disabled — framework limitation:
#    operating-cost accounts are not overridable, 1cFE/1costingfe#106).
#    The enabled set reflects the radical structural difference between Focus
#    Fusion's 5 MWe, ~3 tonne, aneutronic, direct-conversion device and any
#    conventional fusion plant architecture.
overrides = [
    # C220105: Primary structure — device is ~3 tonnes total, ~1000× lighter than MFE.
    {
        "account": "C220105",
        "value": 0.03 * generic.cas22_detail["C220105"],
        "enabled": True,
        "provenance": "derived",
    ...
    # C220110: Remote handling — aneutronic p-B11, contact-maintained, no rad-hardened tooling.
    {
        "account": "C220110",
        "value": 0.05 * generic.cas22_detail["C220110"],
        "enabled": True,
        "provenance": "derived",
    ...
    # Disabled: the 1costingFE library does not support CAS70 overrides — operating-
    # cost accounts (CAS70, CAS80) are computed by cas70_om()/cas80_fuel() and bypass
    # the co.get() injection path used for capital costs (CAS10–CAS28). The override
    # was silently ignored. Native LCOE carries the full generic CAS70 until the
    # framework adds operating-cost override support.
    {
        "account": "CAS70",
    ...
    # Disabled: same framework limitation as CAS70 — cas80_fuel() computes the
    # fuel cost independently and does not consult cost_overrides.
    {
        "account": "CAS80",
        "value": 0.03,
        "enabled": False,
