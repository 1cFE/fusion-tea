...
In scenario comparisons, the physics failure case (Q_sci < 1.41) should appear as a first row with LCOE = UNDEFINED — not as a cost scenario but as the viability gate that all cost scenarios presuppose. Decision-makers must see this gate explicitly; a table showing only cost variation implies that near-breakeven operation is merely expensive rather than commercially impossible. Similarly, in sensitivity sweeps, any parameter value that drives net electric power to zero or below should output UNDEFINED rather than a finite (if extreme) numeric value, since no LCOE exists when the plant produces no net electricity.
...
**Taxonomy position:** DPF is classified as **Other (Non-Standard self-pinch)** in the concept taxonomy — it does not fit cleanly into MFE, IFE, MIF, or Electrostatic categories [dossier.md, §Confinement Family: "high" confidence]. The plasma is self-confined by its own current-generated magnetic field, but only transiently (~10 ns), distinguishing it from sustained MFE confinement; there are no external magnets and no magnetized target (ruling out MIF). The nearest structural analog by confinement mechanism is Concept 15 (Zap Energy sheared-flow Z-pinch, MFE/Open/Linear), though the DPF's fully transient pulsed nature and absence of any external magnetic field make it structurally more distinct than the Z-pinch heritage suggests. No family-level cost patterns from MFE, IFE, or MIF concepts are directly transferable; the DPF must be modeled as fully standalone with a custom CAS structure (as reflected in the model: CAS22.01, 22.03, 22.04, CAS23 all set to zero by design).
