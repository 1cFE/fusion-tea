...
propositions identified in analysis.md §Modeling Approach Rationale:
  ...
  3. Recirculating power dominance: The in-vessel coil (resistive vs SC) and
     its maintenance interval dominate LCOE more than any other variable.
...
- Elio et al. (2024) "Technical Report: The Polomac approach to fusion energy."
  JTSP. [abstract only; concept claims]
...
class PoloMacPlantParams:
    ...
    Source: No coil design has been published. 700 MW copper path is nonviable.
    Ref: elio-2014-fed-poloidal-confinement.md §Coil support and supply;
    ...
    Source: No heating system specified.
    Ref: analysis.md §Section 2 (blocking gap: 'No plasma heating method
    specified — severity: blocking').
    ASSUMED: Fixed overhead for control and heating system losses (e.g., NBI
    efficiency ~60%; some fraction lost to beam interception). For a system
    delivering P_fus/Q_sci to plasma, wall-plug draw is ~P_fus/(Q_sci*eta_heat)
    but eta_heat is absorbed into effective Q_eng through this overhead term.
    MODERATE UNCERTAINTY."""

    heating_system_cost_M: float = 150.0
    """Capital cost of auxiliary heating system [M$]. Maps to C220104.
    Source: No heating system specified for PoloMac.
    ASSUMED: NBI/RF system at ~$0.30M/MW of installed heating power.
    Analogue: ITER NBI ~$500M for 33 MW; DEMO NBI concepts ~$200-300M for 50 MW.
    For PoloMac with moderate heating (50 MW installed), ~$150M.
    ...
    Sensitivity: at CF=0.50, LCOE rises ~40%; at CF=0.85, falls ~17%.
    HIGH UNCERTAINTY."""
    ...
    def _compute_cas22(self, power: dict, geom: dict) -> dict:
        ...
        # No heating system specified for PoloMac. Using direct cost for NBI/RF analogue.
        # Heating power needed: P_fus / Q_sci. At Q=10, P_fus=500 MW → P_heat = 50 MW.
        # Cost scaled loosely by heating power: $150M baseline for ~50 MW installed.
        r["C220104"] = self.heating_system_cost_M   # [override] NBI/RF heating system
...
def print_results(p: PoloMacPlantParams, r: dict):
    ...
    print("   Blocking gaps: no Q_sci, no P_fus, no heating, no confinement physics.")
...
def main():
    ...
    print("=" * 72)
    print("""
  1. RECIRCULATING POWER / SC COIL VIABILITY
     The 700 MW copper coil draw (Elio 2014) is prohibitive. The model assumes
     a superconducting coil reduces this to ~15 MW cryogenic load, but no SC
     coil design has been published. Until a credible SC coil geometry, field
     target, and radiation-hardening approach is specified, the Q_eng of any
     PoloMac design cannot be determined. If the SC coil cryo load exceeds
     ~100 MW (e.g., due to large-bore HTS in high-radiation environment), the
     recirculating fraction exceeds 50% and LCOE rises above 50 ¢/kWh even
     at optimistic plasma Q.

  2. IN-VESSEL COIL CAPITAL COST AND REPLACEMENT INTERVAL
     The SC coil capital cost (sc_coil_capital_M) and its lifetime (FPY before
     replacement) are the two highest-sensitivity parameters in the model.
     A coil costing $800M with a 4 FPY replacement interval contributes
     ~20–30 ¢/kWh in annualized replacement costs alone. At $300M and 15 FPY,
     this drops to ~3 ¢/kWh. No neutron fluence limit, shielding design, or
     radiation-hard HTS coil design exists for PoloMac — these parameters are
     entirely unconstrained by available data.

  3. PLASMA Q AND FUSION POWER (CONFINEMENT PHYSICS)
     The D-D ignition problem is physics-severity-blocking. D-D requires ~6×
     higher plasma pressure at equivalent density than D-T, and achievable
     confinement time in a dipole geometry at fusion-relevant conditions is
     unknown. The model is insensitive to this only at very high Q (≥15), where
     heating power becomes a small fraction of gross electric. At Q=5, the
     heating system draws ~30% of gross electric, making LCOE highly sensitive
     to Q. This parameter cannot be resolved without plasma physics experiments
     that do not exist for PoloMac.
""")
