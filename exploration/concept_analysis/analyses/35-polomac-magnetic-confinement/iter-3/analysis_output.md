...
3. **D-T vs D-D scenario comparison**: Deutelio explicitly claims D-T operation at "3× weaker field than a tokamak" as its near-term target, with D-D (no blanket) as the long-term ambition (jtsp-2024-polomac-technical-report.md, §Abstract). Since Q is the highest-sensitivity parameter in the model, and D-T ignition is far more credible than D-D ignition at equivalent conditions, a D-T scenario branch is needed to test whether the blanket cost penalty (~$200–400M) exceeds the Q benefit from the lower ignition threshold. This is a model-level gap: the current D-D-only model cannot address the central TEA tradeoff between Deutelio's two claimed operating modes.
