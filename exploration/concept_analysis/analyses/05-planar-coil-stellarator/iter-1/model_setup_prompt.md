# 1costingfe Model Setup: Planar-Coil Stellarator (Thea Energy)

You are generating a runnable 1costingfe model setup script for **Planar-Coil Stellarator (Thea Energy)**
(Thea Energy). The script must run via `uv run python model_setup.py` and emit an
LCOE estimate.
...
**Start here.** Open `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\05-planar-coil-stellarator\analysis.md` and read its `## Design Point` block and
...
```python
"""1costingfe model: Planar-Coil Stellarator (Thea Energy) (Thea Energy).
...
## Output
Write the script to: `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\05-planar-coil-stellarator\iter-1\model_setup.py`
