# 1costingfe Model Setup: State-Backed Tokamak (Neo / ASIPP-class)

You are generating a runnable 1costingfe model setup script for **State-Backed Tokamak (Neo / ASIPP-class)**
(Neo Fusion). The script must run via `uv run python model_setup.py` and emit an
LCOE estimate.
...
**Start here.** Open `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\33-state-backed-tokamak-best\analysis.md` and read its `## Design Point` block and
...
## Output
Write the script to: `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\33-state-backed-tokamak-best\iter-1\model_setup.py`
