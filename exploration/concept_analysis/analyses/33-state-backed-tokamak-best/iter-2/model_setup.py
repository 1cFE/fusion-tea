...
# differentiator. Applies factor to base CAS21 (buildings). Range: 1× (Western),
# 2× (central Chinese estimate), 4× (optimistic Chinese estimate).
_base_cas21 = float(c.cas21)
print()
print("Scenario: Chinese construction cost discount (CAS21 buildings)")
print("  Source: analysis.md §S2 Challenge 1; 2–4× reduction vs. Western baseline")
print(f"  Base CAS21 (Western/NOAK): {_base_cas21:.0f} M$")
for _factor, _label in [(1.0, "No discount (Western baseline)"),
                         (2.0, "2× Chinese discount — central estimate"),
                         (4.0, "4× Chinese discount — optimistic estimate")]:
    _r = model.forward(
        net_electric_mw=NET_ELECTRIC_MW,
        cost_overrides={"CAS21": _base_cas21 / _factor},
        **_SHARED_KWARGS,
    )
    print(f"  {_label}: CAS21={_base_cas21/_factor:.0f} M$ → LCOE={_r.costs.lcoe:.1f} $/MWh")

# Regulatory cost factor: Stewart & Shirvan 2.2× applied to CAS21
# May not apply in Chinese regulatory context; included as Western-upper-bound scenario
_r_reg = model.forward(
    net_electric_mw=NET_ELECTRIC_MW,
    cost_overrides={"CAS21": _base_cas21 * 2.2},
    **_SHARED_KWARGS,
)
print(f"  Western 2.2× regulatory factor: CAS21={_base_cas21*2.2:.0f} M$"
      f" → LCOE={_r_reg.costs.lcoe:.1f} $/MWh")

# F-1 (new): Capacity factor — quasi-steady-state vs pulsed CFETR-lineage operation
# CFETR Phase I (OSTI 1465662): duty cycle 0.3–0.5 for pulsed intermediate step.
# Central case (AVAILABILITY=0.80) assumes quasi-steady-state PFPP; pulsed cases show
...
for _cf, _label in [
    ...
    (0.80, "Central     — quasi-steady-state PFPP (current base case)"),
...
# F-1 (new): CF sweep — quantifies LCOE impact over full quasi-steady-state to pulsed range
# Source: CFETR Phase I duty cycle 0.3–0.5 (OSTI 1465662) establishes pulsed lower bound;
# Araiinejad & Shirvan (2025) 75–90% establishes quasi-steady-state upper bound.
print()
print("Capacity factor sweep (CF = 0.35 → 0.90: pulsed CFETR to quasi-steady-state):")
_cf_sweep = [0.35, 0.40, 0.50, 0.60, 0.70, 0.75, 0.80, 0.90]
