# 1costingfe Model Setup: Levitated Dipole (OpenStar Technologies)

You are generating a runnable 1costingfe model setup script for **Levitated Dipole (OpenStar Technologies)**
(OpenStar Technologies). The script must run via `uv run python model_setup.py` and emit an
LCOE estimate.
...
**Start here.** Open `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\12-levitated-dipole\analysis.md` and read its `## Design Point` block and
...
```python
"""1costingfe model: Levitated Dipole (OpenStar Technologies) (OpenStar Technologies).
...
3. **`spec` carries only design-point inputs.** Do NOT put the uniform
   ...
   **Archetype-specific spec key blocklist (workarounds for known library
   ...
   - **DIPOLE** (`DIPOLE == "DIPOLE"`): do **NOT** pass
   ...
   - **MIF concepts** (`DIPOLE ∈ {MAG_TARGET, MAGLIF,
...
## Output
Write the script to: `C:\Users\mallo\Deterministic_Concept_scoring\fusion-tea\exploration\concept_analysis\analyses\12-levitated-dipole\iter-1\model_setup.py`
