...
- **ICRH coupling in dipole geometry**: ICRH is selected over ECRH for its ~70% wall-plug efficiency vs. 30–40% for gyrotrons (§2.2.7). This is not yet validated in a dipole field topology. If ICRH coupling proves unachievable and the design falls back to ECRH, heating wall-plug power rises from 63.6 MW to ~111–148 MW (at 40% and 30% wall-plug efficiency, respectively), adding ~47–85 MW of additional recirculating load. Net electrical output drops from 208 MWe to ~123–160 MWe — a 23–41% reduction. *Unlike the confinement scaling bet, this is a continuous cost penalty rather than a binary viability threshold: any sub-70% heating efficiency raises LCOE proportionally. NBI is a lower-risk fallback but carries its own efficiency and supply-chain penalties; the design must resolve heating method compatibility at Tahi scale before final plant sizing.*
...
- **Missing at scale**: Any experimental demonstration of ICRH in a levitated dipole configuration. RF coupling models for dipole field geometry. Antenna geometry compatible with coil docking access and neutron shielding. If ICRH proves incompatible with the dipole field topology, the paper identifies ECRH (30–40% wall-plug efficiency, demonstrated on LDX/RT-1) and NBI as fallback options. At the ECRH 30–40% range, heating wall-plug rises from 63.6 MW (ICRH) to ~111–148 MW, cutting net output from 208 MWe to ~123–160 MWe — a 23–41% reduction (see Section 2, Key Hypotheses).
...
| Max first wall loading (A) | 0.198 MW/m² | arxiv-2602-20564-plasma-state-clarification.md §4.3 | medium | Outboard midplane limiter; includes neutron + radiative heating; Inconel 718 + W coating; compare: 1–2.5 MW/m² at equivalent tokamak divertors |
| Max neutron shield wall loading (A) | 0.753 MW/m² | arxiv-2602-20564-plasma-state-clarification.md §4.3 | medium | Peak at outboard side of core magnet; limited by tungsten damage threshold |
...
| First wall lifetime and replacement schedule | truly-unknown | important | Heat flux is sourced (0.198 MW/m² peak); lifetime under neutron irradiation + thermal cycling not addressed — paper treats first wall as semi-permanent infrastructure |
...
| 9 | First wall lifetime and replacement schedule | S3, S5 | truly-unknown | important | Max first wall loading sourced at 0.198 MW/m² (§4.3); material confirmed as Inconel 718 + W coating (§2.2.5). Lifetime under neutron irradiation and thermal cycling not addressed |
...
- **No plasma current → no disruptions; particle exhaust falls on the outer wall rather than a divertor**: Levitated dipole carries no toroidal plasma current; MHD disruptions are mechanistically absent. No disruption mitigation system, thermal dump resistors, or first-wall fatigue from disruption energy deposition. In LCOE terms: no disruption-driven first-wall replacement schedule and no need for thermal energy storage for grid decoupling. However, without a dedicated divertor, particle and heat exhaust falls on the outer vacuum vessel via an outer midplane limiter — the paper explicitly notes "we have elected to study a dipole configuration that is limited on the outer midplane" (§2.1.4). The TEA implication is a testable hypothesis: if first-wall heat flux is substantially lower than a comparable tokamak's divertor heat flux, the "no divertor" advantage is real; if comparable, the advantage shrinks or disappears. The source data closes this loop: the paper reports maximum first wall loading of 0.198 MW/m² for Reactor A (§4.3 Neutron Transport), compared to 1–2.5 MW/m² at equivalent-power tokamak divertors — a factor of 5–12× lower. The levitated dipole advantage is genuine: its first wall sees a small fraction of the thermal load of a tokamak divertor, reducing the need for high-heat-flux plasma-facing component grades and eliminating the high-frequency divertor module replacement cycle. The first wall material is Inconel 718 with a tungsten surface coating (§2.2.5). First wall lifetime under combined neutron irradiation and thermal cycling is not quantified (Section 6, gap 9), but the thermal loading case for the levitated dipole is strongly favorable relative to any tokamak divertor.
...
**Heating method**: ST-E1 uses ECRH exclusively during flat-top; levitated dipole uses ICRH as the baseline, claiming higher wall-plug efficiency (70% vs. 30–40% for gyrotrons, per §2.2.7). If realized, this significantly improves Q_engineering. However, ICRH coupling in a dipole geometry is undemonstrated, while ECRH in tokamaks is operationally mature — and ECRH has actually been demonstrated on dipole experiments (LDX, RT-1), making it the lower-risk fallback.
