...
class LevitatedDipolePlantParams:
 ...
 # standardized from 0.4 per scoring_framework.md (Energy Capture: Thermal (unspecified))
    ...
    first_wall_repl_M_USD_yr: float = 0.0
    """Annual first wall scheduled replacement cost [$M/yr].
    DEFAULT ZERO -- this is a TESTABLE HYPOTHESIS, not a confirmed zero cost.
    A levitated dipole has no dedicated divertor; particle exhaust falls on the
    outer wall. If steady-state first-wall heat flux is lower than divertor heat
    flux in a comparable tokamak, the "no divertor" cost advantage is real and
    this parameter stays near zero. If wall loading is comparable, this parameter
    rises to divertor-replacement levels ($10-40M/yr at tokamak scale) and the
    claimed advantage shrinks or disappears.
    See analysis.md §Section 7 (F-1) for the TEA framing of this hypothesis.
    HIGH UNCERTAINTY -- no thermal load data for dipole first wall at reactor scale."""
    ...
    def _compute_economics(self, power: dict, costs: dict, cas22: dict) -> dict:
        ...
        # (c) First wall annual replacement [F-1: "no divertor" testable hypothesis]
        # Default = 0.0 (assumption: first wall is NOT a significant scheduled
        # replacement item). Non-zero values test the scenario where particle exhaust
        # on the outer wall produces replacement costs offsetting the divertor savings.
        if self.first_wall_repl_M_USD_yr > 0.0:
            annuity_pv_fw = self.first_wall_repl_M_USD_yr * (1 - (1 + i) ** (-n)) / i
            r["CAS72_first_wall"] = crf * annuity_pv_fw
        else:
            r["CAS72_first_wall"] = 0.0

        r["CAS72"] = r["CAS72_blanket"] + r["CAS72_coil"] + r["CAS72_first_wall"]
...
def print_results(p: LevitatedDipolePlantParams, res: dict):
    ...
    fw_note = "[testable hypothesis -- default zero]" if params.first_wall_repl_M_USD_yr == 0.0 else ""
    print(f"    First wall repl (annual):         ${econ['CAS72_first_wall']:>8.1f}M/yr  {fw_note}")
...
def main():
    ...
    sweeps = [
        ...
        ("eta_aux",
         ...
         "ICRH wall-plug efficiency (F-2: ICRH→ECRH fallback bet)",
         "0.50-0.55 = ECRH fallback if ICRH coupling in dipole geometry fails; 0.70 = baseline ICRH"),
    ...
    # Manual sweep: first wall annual replacement cost (F-1: "no divertor" hypothesis)
    print(f"  First wall annual replacement cost [M$/yr]")
    print(f"  (F-1: tests whether outer-wall exhaust offsets 'no divertor' advantage;")
    print(f"   default=0 assumes first wall NOT a significant scheduled replacement item):")
    for v in [0.0, 5.0, 10.0, 20.0, 40.0, 60.0]:
        p = LevitatedDipolePlantParams(first_wall_repl_M_USD_yr=v)
        r = p.compute()
        delta = r["lcoe_cents_per_kWh"] - base_lcoe
        note = " [baseline assumption]" if v == 0.0 else ""
        print(f"    {v:>6.0f} M$/yr  -> {r['lcoe_cents_per_kWh']:6.2f} c/kWh  ({delta:+.2f}){note}")
    print()
 ...
 # standardized from 0.44 per scoring_framework.md (Energy Capture: Thermal (unspecified))
    ...
    # ECRH fallback scenario (F-2): ICRH coupling fails in dipole geometry;
    # plant falls back to ECRH at ~52% wall-plug efficiency.
    ecrh_fallback = LevitatedDipolePlantParams(
        eta_aux=0.52,   # ECRH mid-range (50-55%); vs. 70% ICRH baseline
    )
