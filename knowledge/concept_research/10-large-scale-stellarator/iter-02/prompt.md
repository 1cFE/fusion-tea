# Fusion Concept Research: Large-Scale Stellarator (D-T)
...
- **Name**: Large-Scale Stellarator (D-T)
- **Company**: Gauss Fusion
- **Confinement approach**: Magnetic Confinement
- **Description**: Large stellarator building on W7-X heritage. Conventional (non-planar) coil design at power-plant scale. Steady-state, disruption-free.
...
1. **Start broad**: Search the web for "Gauss Fusion fusion technology" and similar queries. Look for the company's website, Wikipedia page, Fusion Industry Association profile, press releases, and investor presentations.
...
- **Confidence**: high
- **Citation**: [IPP event](https://www.ipp.mpg.de/events/40887/10021); multiple sources confirm GIGA is a quasi-isodynamic stellarator in the HELIAS lineage.
- **Notes**: The initial CSV described this as "Conventional (non-planar) coil design" — accurate for the coil geometry, but the confinement concept is specifically QI (quasi-isodynamic), directly descending from the HSR4/18 HELIAS reactor study (4 field periods, 18m major radius). The coils are non-planar modular coils (like W7-X), distinguishing Gauss from planar-coil approaches (Thea Energy) or compact HTS QI (Proxima Fusion).
...
- **Notes**: FusionXInvest also mentions a later D-D phase, but the primary commercial fuel cycle is D-T. Supply chain requirements (75 tonnes lithium, breeder blankets) confirm D-T as the baseline.

### Primary Heating
- **Value**: RF (ECRH)
- **Confidence**: medium
- **Citation**: Inferred from W7-X heritage and universal stellarator practice. W7-X uses ECRH exclusively as primary heating; HELIAS reactor studies assume ECRH.
- **Notes**: Gauss Fusion has not publicly specified their heating system. ECRH is the overwhelmingly likely choice given W7-X/HELIAS heritage. W7-X currently uses 140 GHz gyrotrons. A 3 GW thermal plant would likely need 50-100+ MW of ECRH for startup, with much less during steady-state burn.

### Energy Capture
- **Value**: Thermal (unspecified)
- **Confidence**: medium
- **Citation**: [Modern Power Systems](https://www.modernpowersystems.com/news/gauss-fusion-unveils-conceptual-design-for-giga-fusion-power-plant/) — CDR covers "heat extraction and cooling loops for power generation" and "power conversion for grid connection." Supply chain mentions vacuum vessel steel, breeder blankets.
- **Notes**: Specific cycle (Rankine steam vs. sCO2 Brayton) not publicly disclosed. At 3 GW thermal to 1 GW electric, the implied thermal efficiency is ~33%, consistent with conventional steam Rankine. HELIAS studies have explored both He-cooled and DCLL blanket options feeding thermal cycles.

### Plasma State
- **Value**: Burning
- **Confidence**: high
- **Citation**: A 3 GW thermal stellarator with 1 GW electric output requires a high-Q burning plasma. HELIAS reactor studies target ignited plasma (Q → infinity).
- **Notes**: At 3 GW fusion power, alpha heating (~600 MW) vastly exceeds any plausible external heating (~50-100 MW ECRH), confirming a burning plasma state.

### Magnet Type
- **Value**: LTS+HTS
- **Confidence**: high
- **Citation**: [MT29 CERN Indico abstract](https://indico.cern.ch/event/1431972/contributions/6419980/) — "GFG is developing both LTS and HTS options with a common cross-section (circular with a diameter of about 55mm) and current (100kA)." Supply chain: "~800 tonnes LTS + 26M meters HTS superconductors" ([binding.energy](https://binding.energy/gauss-fusion-commercial-roadmap/)).
- **Notes**: Dual-track development strategy with a common conductor format (55mm circular, 100kA) allowing either LTS or HTS in the same coil design. Tokamak Energy partnership (Oct 2025) specifically for HTS development. Original HELIAS HSR4/18 used NbTi at ~10T; GIGA targets 12-13T peak coil field. 40 non-planar modular coils at ~300 tonnes each, comparable to ITER TF magnets.

### Tritium Breeding
- **Value**: Li blanket (unspecified)
- **Confidence**: medium
- **Citation**: Supply chain mentions "~75 tonnes lithium inventory" and "breeder blankets" ([binding.energy](https://binding.energy/gauss-fusion-commercial-roadmap/)). CDR lists "closed tritium fuel cycle" as key development challenge. Dedicated "Tritium Fuel Cycle & Breeding Module" team leads.
- **Notes**: Specific blanket type not publicly disclosed. HELIAS reactor studies have explored DCLL (Dual Coolant Lithium Lead / LiPb) and HCPB (Helium-Cooled Pebble Bed). DCLL is considered more promising for the complex HELIAS geometry. Partnership with KIT (EU blanket R&D lead) suggests lithium-based blanket, but whether LiPb, FLiBe, or solid ceramic is not disclosed.

### Neutron Management
- **Value**: Heavy shielding (14 MeV)
- **Confidence**: high
- **Citation**: D-T fuel produces 14.1 MeV neutrons. First wall neutron load 1 MW/m² with 5-year blanket life. CDR addresses materials "capable of withstanding extreme thermal and neutron loads." Supply chain includes tungsten, RAFM steel, beryllium.
- **Notes**: If blanket type turns out to be DCLL (LiPb), `Integrated blanket/shield` might be more appropriate since liquid metal blankets serve dual purpose. With current uncertainty on blanket type, `Heavy shielding (14 MeV)` is the conservative classification. The 1 MW/m² first-wall neutron load is moderate (some concepts target 2+ MW/m²).
...
- **Confidence**: high
- **Citation**: [MT29 abstract](https://indico.cern.ch/event/1431972/contributions/6419980/) — 40 coils, 5 shapes × 8, non-planar, conductor-in-plate concept, demountable joints (~250/coil, ~1 nOhm resistance), 100 kA conductor current, ~55mm diameter conductor.
- **Notes**: The distinguishing "hard technology bet" is the combination of: (1) large-scale non-planar modular coils at ITER-comparable size (~300 tonnes each, 30-35m perimeter), (2) demountable joints enabling sector-based maintenance, (3) dual LTS/HTS conductor strategy with common cross-section, and (4) conductor-in-plate construction avoiding traditional coil casings. Distinct from Proxima Fusion (compact HTS QI) and Thea Energy (planar HTS coil arrays).

## Remaining Gaps

1. **Primary Heating** (medium confidence): No Gauss Fusion source explicitly states ECRH. Inferred from W7-X heritage and universal stellarator practice. A Gauss Fusion technical paper or CDR excerpt would raise confidence to high.

2. **Energy Capture** (medium confidence): Thermal conversion confirmed but specific cycle (steam Rankine vs. sCO2) not disclosed. The ~33% implied efficiency hints at steam Rankine. CDR likely specifies this.

3. **Tritium Breeding** (medium confidence): Lithium-based blanket confirmed via supply chain data, but specific type (LiPb/DCLL, FLiBe, HCPB) not disclosed. HELIAS studies favor DCLL (LiPb). Access to CDR or a Gauss Fusion blanket paper would resolve this.

4. **Neutron Management** (high confidence but may need reclassification): Currently `Heavy shielding (14 MeV)` — could be `Integrated blanket/shield` if the blanket type turns out to be a liquid breeder serving dual purpose. Depends on blanket type gap above.

5. **Magnet conductor specifics**: LTS conductor type (NbTi vs. Nb3Sn) not explicitly stated. At 12-13T peak field, Nb3Sn is more likely. HTS track confirmed REBCO via Tokamak Energy partnership.

Another research iteration could help if it targets the CDR content directly or finds Gauss Fusion technical publications specifying heating, blanket type, and power conversion cycle.
...
4. [FusionXInvest company profile](https://fusionxinvest.com/company-profile/4362/gauss-fusion/) — Fuel, funding, team
5. [IPP — Physics and engineering of GIGA](https://www.ipp.mpg.de/events/40887/10021) — Technical seminar
6. [Gauss Fusion website](https://gauss-fusion.com/) — Official overview
7. [World Nuclear News — GIGA blueprint](https://www.world-nuclear-news.org/articles/gauss-fusion-releases-blueprint-for-development-of-its-giga-fusion-plant) — CDR announcement
8. [Tokamak Energy HTS collaboration](https://tokamakenergy.com/2025/10/07/tokamak-energy-signs-strategic-hts-collaboration-with-gauss-fusion/) — HTS magnet partnership
9. [ResearchGate — HSR4/18 HELIAS reactor](https://www.researchgate.net/publication/27263728_The_Helias_reactor_HSR418) — Predecessor design study
10. [CIEMAT — DCLL BB for HELIAS](https://www.sciencedirect.com/science/article/pii/S0360544223033649) — Blanket studies for HELIAS geometry
