# Fusion Concept Research: Sheared-Flow Stabilized Z-Pinch (D-T)
...
- **Name**: Sheared-Flow Stabilized Z-Pinch (D-T)
- **Company**: Zap Energy
- **Confinement approach**: Magnetized Target Fusion
- **Description**: Axial current creates self-pinching magnetic field. Sheared axial flow stabilizes MHD instabilities. Pulsed/quasi-steady operation. Simple geometry, no external magnets.
...
1. **Start broad**: Search the web for "Zap Energy fusion technology" and similar queries. Look for the company's website, Wikipedia page, Fusion Industry Association profile, press releases, and investor presentations.
...
- **Notes**: Initial CSV listed "Magnetized Target Fusion" (MIF) — corrected per schema rules. There is no external compression driver; confinement is purely magnetic via self-generated fields.

### Confinement Concept
- **Value**: Z-pinch (sheared-flow)
- **Confidence**: high
- **Citation**: https://www.zapenergy.com/how-it-works; schema vocabulary exact match
- **Notes**: Proprietary feature is sheared-flow stabilization pioneered by Uri Shumlak at UW, which stabilizes the Z-pinch against MHD kink and sausage instabilities.
...
- **Confidence**: high
- **Citation**: https://www.zapenergy.com/how-it-works — "the higher the current sent through the plasma, the tighter it will be constricted by magnetic fields and the hotter and denser it will become"; schema description match
- **Notes**: The large axial current both confines the plasma (via J x B pinch force) and heats it ohmically. No separate heating system (no NBI, no RF). Compression from the pinch effect also contributes adiabatic heating.

### Energy Capture
- **Value**: Thermal (steam)
- **Confidence**: medium
- **Citation**: Blog analysis at benbridgerengineering.com (2023): "LiPb to steam cycle"; Engineering Paradigms paper describes LiPb as primary heat transfer medium
- **Notes**: The published reactor concept uses LiPb liquid metal feeding a steam cycle. The "steam" detail comes from a third-party summary of the paywalled Engineering Paradigms paper, not the paper directly. The thermal cycle choice (Rankine vs sCO2) may evolve. Confidence is medium pending direct paper access.

### Plasma State
- **Value**: Pinch
- **Confidence**: high
- **Citation**: Schema definition: "Self-compressed plasma column maintained by its own current. Characteristic of Z-pinch."
- **Notes**: Plasma column (~50 cm long, ~1 mm diameter) is self-compressed by J x B force. Beta ~ 1 (plasma pressure equals magnetic pressure). Sheared flow extends pinch lifetime but doesn't change fundamental plasma state.

### Magnet Type
- **Value**: Self-confined
- **Confidence**: high
- **Citation**: https://www.zapenergy.com/how-it-works — "confines plasma using the magnetic field generated by a powerful electrical current in the plasma instead of using external magnets"; schema lists Z-pinch as example for Self-confined
...
- **Notes**: Thick (~3 m) LiPb blanket serves as both tritium breeder and neutron shield in a single integrated system. Compact plasma geometry concentrates neutron emission. No separate shield structure described.
...
- **Notes**: Each Z-pinch event is a discrete pulse lasting microseconds to milliseconds. Century demo has operated at 0.1-0.2 Hz; 10 Hz is the commercial reactor target.

### Repetition Rate
- **Value**: ~10 Hz
- **Confidence**: high
- **Citation**: https://www.zapenergy.com/how-it-works — "ten plasma pulses per second"; Engineering Paradigms paper: 10 Hz
- **Notes**: 10 Hz is the target commercial repetition rate. Current experimental devices operate at much lower rates (Century at 0.1-0.2 Hz). The rep rate determines time-averaged power output from the 190 MWt-per-pulse reactor concept.
...
- **Notes**: Capacitor bank discharging through pulse-forming networks delivers shaped current waveforms (fast rise, long flattop). Target currents: 650 kA-1.5 MA. Wall-plug to plasma efficiency ~70%. PFN design with inductively isolated capacitors allows flexible waveform shaping.

## Remaining Gaps

1. **Energy Capture** (medium confidence): The "steam" cycle detail comes from a third-party blog summary of the paywalled Engineering Paradigms paper (Taylor & Francis, 2023). Direct paper access would confirm whether steam Rankine is explicitly stated or if other thermal cycles (sCO2) are considered. Another iteration with direct paper access could resolve this, but the impact is minor — the value would likely remain `Thermal (steam)` or shift to `Thermal (unspecified)`.

All other columns are at high confidence with authoritative sources.
...
4. [Zap Energy — Century demo press release](https://www.zapenergy.com/news/zap-attracts-130m-as-demo-system-begins-operations) — Century specs, liquid bismuth, pulsed power demo
5. [Century: Lightning Strikes 12 Times Per Minute](https://www.zapenergy.com/news/lightning-strikes-12-times-per-minute-century) — Century 0.2 Hz, 500 kA, 100+ shots
6. [DOE Certifies Zap Energy Milestone](https://www.zapenergy.com/news/doe-century-milestone) — 1000+ shots at >= 100 kA
7. [FuZE-3 gigapascal pressures press release](https://www.zapenergy.com/news/zap-energy-exceeds-gigapascal-fusion-plasma-pressures-on-new-fusion-device-fuze-3) — 830 MPa electron, 1.6 GPa total
8. [Ben Bridger Engineering blog on Zap Energy (2023)](https://benbridgerengineering.com/2023/08/29/zap-energy/) — "LiPb to steam cycle" detail, TBR 1.1
9. [IEEE Spectrum: Magnetic-Confinement Fusion Without the Magnets](https://spectrum.ieee.org/zap-energy-fusion-reactor) — MFE classification context
10. Saved source files in `iter-01/sources/`: zap-energy-website-how-it-works.md, engineering-paradigms-paper-summary.md, century-demo-system.md, fuze-q-and-fuze-3.md
