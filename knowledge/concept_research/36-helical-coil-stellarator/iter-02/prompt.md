# Fusion Concept Research: Helical Coil Stellarator (D-T)
...
- **Name**: Helical Coil Stellarator (D-T)
- **Company**: Helical Fusion
- **Confinement approach**: Magnetic Confinement
- **Description**: Advanced nuclear fusion reactors using helical coil technology. Spun out from Japan's National Institute for Fusion Science (NIFS). Tokyo-based.
...
1. **Start broad**: Search the web for "Helical Fusion fusion technology" and similar queries. Look for the company's website, Wikipedia page, Fusion Industry Association profile, press releases, and investor presentations.
...
- **Notes**: The specific geometry is a heliotron — a stellarator variant using two continuous helical winding coils in a double-helix structure, directly derived from the LHD at NIFS. Distinct from modular coil stellarators (W7-X) and planar coil stellarators. The company sometimes uses the term "Helical-Stellarator."
...
- **Notes**: ECRH via gyrotrons is the primary and seemingly sole heating method. The company is developing 250 GHz / 1 MW continuous-wave gyrotrons. No mention of NBI as supplementary heating. Consistent with heliotron physics — no current drive needed, making NBI unnecessary.

### Energy Capture
- **Value**: Thermal (sCO2)
- **Confidence**: medium
- **Citation**: Helical Fusion collaborative research list mentions "CO2 gas turbines" as one of 14 research areas
- **Notes**: The company lists "power generation systems (CO2 gas turbines)" among collaborative research projects, strongly suggesting a supercritical CO2 Brayton cycle. However, no single source explicitly confirms "sCO2 Brayton cycle" as the baseline. The liquid metal blanket system would be a natural heat source for an sCO2 cycle.
...
- **Confidence**: high
- **Citation**: AIP Physics of Plasmas 30, 050601 (2023); HTS coil test press release (Oct 2025) — 40 kA at 7 T, 15 K; WISE conductor uses REBCO tapes
- **Notes**: Uses proprietary WISE (Wound and Impregnated Stacked Elastic tapes) conductor made from stacked REBCO tapes. Designed for flexibility in 3D helical coil winding, then impregnated with low-melting-point alloy for structural rigidity. Target field: 8 T at coil center. Continuous helical winding (not modular) is a distinguishing feature. Coil manufacturing machine developed with Sugino Machine.
...
- **Notes**: The liquid metal blanket serves triple duty: tritium breeding (lithium content), neutron shielding, and first-wall protection via free-surface flow. Eliminates the need for a separate divertor system. GALOP test system validates a gas-driven liquid metal pump (no rotating components). Specific liquid metal composition (pure Li vs. LiPb vs. other) not confirmed in available sources.

### Neutron Management
- **Value**: Integrated blanket/shield
- **Confidence**: high
- **Citation**: AIP Physics of Plasmas 30, 050601 (2023) — liquid metal blanket with free-surface first wall
- **Notes**: Liquid metal blanket provides integrated tritium breeding, neutron moderation/capture, heat removal, and first-wall protection. 14.1 MeV D-T neutron environment. Free-surface liquid metal flow on the first wall provides continuous surface renewal and eliminates plasma-facing component erosion concerns.
...
- **Confidence**: high
- **Citation**: AIP Physics of Plasmas 30, 050601 (2023); HTS coil test (Oct 2025); gyrotron R&D announcements
- **Notes**: Two key technology bets: (1) WISE HTS conductor enabling continuous helical coils for the heliotron geometry — structural differentiator vs. modular stellarator coils; (2) high-power CW ECRH gyrotrons at 250 GHz for steady-state plasma heating. Coil manufacturing approach (flexible REBCO tape stacking + low-melting-point alloy impregnation) is proprietary. Dedicated coil manufacturing machine developed with Sugino Machine.

## Remaining Gaps

1. **Energy Capture** (medium confidence): The CO2 gas turbine reference is strong but not a definitive confirmation of the specific thermodynamic cycle (sCO2 Brayton vs. other). A technical paper or presentation explicitly stating the power conversion system design would raise confidence to high.

2. **Tritium Breeding — specific liquid metal composition**: The liquid metal type (pure Li, LiPb, or other) is not confirmed in available public sources. The AIP paper full text (behind paywall) may specify this. Given the FFHR heritage (which used FLiBe in some designs and liquid Li in others), this could go either way.
...
4. **Power output — thermal**: Only electric output (50 MWe) is publicly stated. Thermal power can be estimated from Q~13 and conversion efficiency but isn't directly stated.
