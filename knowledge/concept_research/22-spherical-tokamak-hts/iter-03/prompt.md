...
Low aspect-ratio (spherical) tokamak using high-temperature superconducting (HTS) REBCO magnets, operating on D-T fuel. The ST-E1 pilot plant design evolved significantly through multiple revisions, from initial parameters (A=2.0, R=4.25 m, 85 MW net) to the final pre-conceptual design point Revision D (DPP 2025): A=2.3, R=5.0 m, B=5.25 T on-axis, targeting 450-750 MWe net. Distinct from conventional compact tokamaks (CFS ARC-class) in its lower aspect ratio, which enables higher plasma beta at the cost of more challenging center-stack engineering and outboard-only blanket coverage. Deliberate pulsed operation (15+ minute pulses) is considered "more desirable than steady-state" for spherical tokamaks due to limited central solenoid space. The ST80-HTS device (completion 2026) targets 15-minute pulses as a bridging step to ST-E1.
...
- **Value**: `RF + NBI`
- **Confidence**: medium
- **Citation**: Research iter-02 — ST40 uses combined NBI + ECRH (1 MW gyrotron at 104/137 GHz from Kyoto Fusioneering added 2025 alongside existing NBI); combined approach being tested to inform pilot plant design
- **Notes**: Changed from `RF (ECRH)` in iter-01. ST40 demonstrably uses both heating methods. ECRH is emphasized for current drive capability in the limited-CS geometry, but NBI provides bulk heating. The exact MW allocation for ST-E1 remains unspecified.

### Energy Capture
- **Value**: `Thermal (unspecified)`
- **Confidence**: medium
- **Citation**: Research iter-01 — inferred from D-T fuel cycle and power output targets (450-750 MWe)
- **Notes**: Specific thermal cycle (steam Rankine vs sCO2 Brayton) not disclosed. No new information from iter-02.

### Plasma State
- **Value**: `Burning`
- **Confidence**: medium
- **Citation**: Research iter-01 — inferred from 450-750 MWe net power targets
- **Notes**: Q value not publicly stated. Burning plasma state inferred from the power output targets, which would require significant fusion gain. Could be `Sustained` if Q turns out to be moderate. No new information from iter-02.
...
- **Notes**: Pulse lengths of ~15+ minutes. Pulsed operation is a deliberate design choice, not a limitation, driven by limited central solenoid space in the spherical tokamak geometry. Long pulse duration could warrant `Quasi-steady` classification, but company self-describes as pulsed.
...
1. **Energy Capture** (medium confidence): Thermal cycle type (steam vs sCO2) not disclosed. Two iterations searched with no result — Tokamak Energy may not have selected a power conversion cycle yet. Unlikely to resolve without direct company disclosure or a new technical publication.

2. **Plasma State** (medium confidence): Q value not publicly stated. Burning vs sustained depends on target Q. Two iterations searched. A DPP or journal publication on ST-E1 plasma performance targets could resolve this.

3. **Primary Heating** (medium confidence): The NBI vs ECRH MW allocation for ST-E1 specifically is not published. ST40's combined approach is confirmed. A future ST-E1 engineering design publication could clarify the split.

4. **Repetition Rate / Operation Mode consistency**: The 15+ minute pulse with N/A rep rate and `Pulsed` operation mode is internally consistent per schema rules, but sits at the boundary between `Pulsed` and `Quasi-steady`. Resolving requires clarity on inter-pulse duty cycle and thermal energy storage approach.
...
2. ST-E1 design evolution documentation — tracks progression from initial to Revision D parameters (iter-02/sources/tokamak-energy-st-e1-design-evolution.md)
3. Tokamak Energy heating systems — ST40 NBI + ECRH combined heating (iter-02/sources/tokamak-energy-heating-systems.md)
4. Humphry-Baker & Smith 2019 — WC cermet center-stack shielding study (iter-02/sources/spherical-tokamak-center-stack-shielding.md)
5. Tokamak Energy roadmap — ST80-HTS bridging device, 15-minute pulse target (iter-02/sources/tokamak-energy-roadmap.md)
6. Tokamak Energy publications on pulsed spherical tokamak operation — "more desirable than steady-state"
7. ST40 experimental results — NBI + ECRH heating, ECRH current drive studies
8. Outboard liquid lithium blanket studies — TBR=1.2
9. Lab experiments: MAST-U (Culham/UKAEA), NSTX-U (PPPL), START (Culham), Globus-M2 (Ioffe Institute)
