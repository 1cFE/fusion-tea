...
- **Notes**: No exact vocab match. Driver is electromagnetic (railgun) rather than pneumatic piston or pulsed-power liner. Closest fit is `Magnetized target (pneumatic)` as the mechanical-compression bucket. Flag for schema review — possible new value `Magnetized target (projectile)`.
...
- **Value**: `TBD`
- **Confidence**: low
- **Citation**: Not disclosed on NearStar website
- **Notes**: D-D is neutronic (2.45 MeV + 14.1 MeV from secondary D-T); molten Pb chamber implies thermal capture, but no cycle specified. Plausibly `Thermal (unspecified)` given the molten lead heat-sink architecture.
...
- **Energy Capture**: not disclosed publicly. Molten Pb suggests `Thermal (unspecified)` but explicit confirmation would need an ARPA-E award abstract, technical paper, or investor deck.
- **Magnet Type / pellet pre-magnetization**: pellet is described as "magnetized" but the seed-field generation mechanism (embedded coil, capacitor-driven θ-pinch, etc.) is not public. Patents or APS-DPP abstracts by NearStar/UAH/TAMU staff might resolve this.
- **Confinement Concept vocab mismatch**: railgun-projectile MTF doesn't cleanly map to `Magnetized target (pneumatic)` (General Fusion piston) or `Magnetized target (pulsed power)` (MagLIF). Schema may need a `Magnetized target (projectile)` value.
- **Lab Experiments**: UAH and Texas A&M partnerships mentioned but specific experiments/facilities not identified — another web/literature pass could resolve.

## Key Sources

1. https://www.nearstarfusion.com/ — NearStar Fusion homepage
2. https://www.nearstarfusion.com/learn-more — technical overview (fetched)
3. https://climateinsider.com/2025/02/10/virginia-venture-partners-and-ecosphere-ventures-invest-in-nearstar-fusion/ — funding context
4. https://vipc.org/vipc-invests-in-nearstar-fusion-to-advance-clean-energy-and-virginias-nuclear-fusion-ecosystem/ — funding context
5. https://fusionxinvest.com/company-profile/4346/nearstar-fusion/ — company profile
6. `iter-01/sources/nearstar-website-summary.md` — local extract
