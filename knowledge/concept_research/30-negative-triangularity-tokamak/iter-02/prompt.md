# Fusion Concept Research: Negative Triangularity Tokamak (D-T)
...
- **Name**: Negative Triangularity Tokamak (D-T)
- **Company**: Firefly Fusion
- **Confinement approach**: Magnetic Confinement
- **Description**: Compact tokamak optimized for negative triangularity plasma shape. Higher confinement, disruption-resilient. Prototype called LUCIOLE. Franco-Swiss company collaborating with DIII-D National Facility. Targets early 2030s for burning plasma demo.
...
1. **Start broad**: Search the web for "Firefly Fusion fusion technology" and similar queries. Look for the company's website, Wikipedia page, Fusion Industry Association profile, press releases, and investor presentations.
...
- **Notes**: "Microwaves" strongly suggests ECRH. However, co-founder Ball's research ([Balestri, Ball, Coda 2024](https://arxiv.org/html/2407.06439v2)) explores ohmic-only operation for compact high-field NT tokamaks, which would eliminate auxiliary heating entirely. If Firefly pursues ohmic-only, this value would change. No schema vocabulary exists for ohmic-only; would need schema review.
...
- **Notes**: No source discusses energy capture or power conversion. Default assignment per schema convention.
...
- **Notes**: Q > 5 is at the boundary between Sustained and Burning. Company explicitly uses "burning plasma" framing. Ball et al. analyze scenarios with Q > 10, which is firmly Burning.

### Magnet Type
- **Value**: `HTS (wound)`
- **Confidence**: high
- **Citation**: [GreyB interview](https://greyb.com/blog/firefly-fusion-scouted-interview) — "HTS superconducting magnets, 10-12 T"; [Fusion Energy Base](https://www.fusionenergybase.com/organizations/firefly-fusion) — confirms phased strategy; [DIII-D page](https://d3dfusion.org/fireflyfusion/) — "actively-cooled copper magnets" for LUCIOLE
- **Notes**: Phased approach: copper magnets for LUCIOLE prototype (enabling rapid iteration), HTS for commercial plants. Table value reflects commercial target per research recommendation.

### Tritium Breeding
- **Value**: `TBD`
- **Confidence**: low
- **Citation**: No source discusses tritium breeding or blanket design.
- **Notes**: As a D-T concept, tritium breeding is required. No information found in any source. Early-stage company (founded 2024) — blanket design likely not yet determined.

### Neutron Management
- **Value**: `Heavy shielding (14 MeV)`
- **Confidence**: low
- **Citation**: Inferred from D-T fuel cycle — 14.1 MeV neutrons require heavy shielding.
- **Notes**: No source discusses neutron management. Value is a structural inference from the fuel choice, not a company disclosure. Could be `Integrated blanket/shield` if a FLiBe or similar dual-purpose blanket is eventually selected.

### Operation Mode
- **Value**: `Steady-state`
- **Confidence**: medium
- **Citation**: Baseline CSV says "Continuous". [Venture Kick](https://www.venturekick.ch/firefly-fusion) and [GreyB interview](https://greyb.com/blog/firefly-fusion-scouted-interview) do not clarify pulsed vs. steady-state.
- **Notes**: CONFLICT: Baseline CSV states "Continuous" (mapped to Steady-state), but the closest published NT reference design (MANTA, Rutherford et al. 2024) is pulsed. Firefly has not explicitly stated their operation mode. Kept as Steady-state based on baseline CSV, but this is uncertain. A future iteration should try to resolve this.

### Repetition Rate
- **Value**: `N/A`
- **Confidence**: medium
- **Citation**: Follows from Operation Mode = Steady-state.
- **Notes**: If Operation Mode is revised to Pulsed, this would need reassessment.
...
- **Notes**: The distinguishing technology bet is the combination of high-field HTS magnets with negative triangularity geometry. NT is not just a plasma shape choice but a confinement optimization that claims to reduce required heating power by 5x.

## Remaining Gaps

| Column | Current Status | What's been searched | What might resolve it |
|--------|---------------|---------------------|----------------------|
| **Primary Heating** | Medium confidence (ECRH inferred from "microwaves") | Venture Kick, GreyB interview, Ball et al. paper | Direct Firefly technical publication or conference presentation specifying heating system. Ohmic-only possibility adds uncertainty. |
| **Energy Capture** | Medium confidence (default Thermal) | All 5 sources — none discuss power conversion | Firefly technical documentation or a reference design paper. Unlikely to be resolved until company matures. |
| **Tritium Breeding** | TBD | All 5 sources — none discuss blanket | Same as above. Very early-stage company (founded 2024). |
| **Neutron Management** | Low confidence (inferred from D-T) | All 5 sources — none discuss shielding | Linked to tritium breeding — blanket choice will determine neutron management approach. |
| **Operation Mode** | Medium confidence (Steady-state from CSV, but MANTA is pulsed) | CSV, all 5 sources | Direct Firefly statement on pulsed vs. steady-state. MANTA reference design publications may clarify. |

Another research iteration could target: (1) MANTA reference design papers for operation mode clarity, (2) Firefly conference presentations or preprints for heating/blanket details, (3) FIA member profile if available.
...
2. **DIII-D National Fusion Facility Collaboration Page** — https://d3dfusion.org/fireflyfusion/ — Confirms DIII-D collaboration, LUCIOLE prototype, copper magnets for prototype. Saved: `iter-01/sources/firefly-fusion-diii-d-collaboration.md`
3. **Balestri, Ball, Coda (2024) — Ohmic NT Tokamak Feasibility** — https://arxiv.org/html/2407.06439v2 — Academic basis for ohmic-only NT operation at compact high-field parameters. Saved: `iter-01/sources/ball-balestri-ohmic-nt-paper.md`
4. **Venture Kick Profile** — https://www.venturekick.ch/firefly-fusion — "Microwaves" heating mention, CHF 50k funding. Saved: `iter-01/sources/venture-kick-profile.md`
5. **Fusion Energy Base Profile** — https://www.fusionenergybase.com/organizations/firefly-fusion — Confirms phased magnet strategy, location, founding date. Saved: `iter-01/sources/fusion-energy-base-profile.md`
...
- **Tritium Breeding**: low confidence — needs better source
- **Neutron Management**: low confidence — needs better source
