# Fusion Concept Research: Particle Accelerator-Driven Fusion (D-T)
...
- **Name**: Particle Accelerator-Driven Fusion (D-T)
- **Company**: SHINE Technologies
- **Confinement approach**: Accelerator-Driven
- **Description**: High-current particle accelerator drives beam-on-target D-T fusion for neutron services (Mo-99 medical isotopes, materials testing). Commercial operations since 2010; not a power-generating reactor.
...
1. **Start broad**: Search the web for "SHINE Technologies fusion technology" and similar queries. Look for the company's website, Wikipedia page, Fusion Industry Association profile, press releases, and investor presentations.
...
- **Notes**: Not in the standardized vocabulary list under `Other`. Distinct from `IEC / Fusor` and `Dense plasma focus`. Flag for schema review to add `Beam-target` or `Accelerator-driven beam-target` under family `Other`.
...
- **Notes**: Schema vocabulary description references "convergence point" (IEC geometry). SHINE's geometry is linear beam-on-target, not convergent — no closer vocabulary fits. Flag for schema review (potential `Accelerator (beam-target)` value).

### Energy Capture
- **Value**: `Neutron applications`
- **Confidence**: high
- **Citation**: SHINE corporate materials; system is a neutron source, not a power plant
...
- **Notes**: Tritium for the target gas is procured externally, not bred.
...
- **Quantitative accelerator parameters** (beam current in mA, neutron yield per beam power, target gas pressure): would require Piefer et al. ANL paper or NRC license documents. Not required for the differentiation table.
