...
$$LCOE = \frac{\sum_{i=1}^{N} C_i / (1 + d)^i}{\sum_{i=1}^{N} E_i / (1 + d)^i}$$
(2.1)
...
$$E_i = \frac{(365 \times 24 \times 60 \times 60)}{(10^6 \times 60 \times 60)} P_{e} p_{av}$$
(2.2)
...
|  |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- |
|  | Phil. Trans. where costs are minimized using very high fusion yield and low frequency, reducing plant therefore cost, while also reducing target cost. A second aspect of capital cost is therefore which is proportional to the fusion energy per event, which here will be called the ‘yield Essentially this represents the cost of the reaction vessel. engineering of a system for large yield but low electrical power, which is the parameter we’re trying to constrain by adding the yield cost, might not be ‘typical’. For example, large release and low frequency may lead to a meaningful variation in the temperature of the coolant over the shot cycle. This will potentially introduce a complexity in the primary and possibly a compromise in efficiency. However, to the author, a system with this characteristic does not seem inherently implausible, so long as the central chamber can reliably energy release. |  | 5royalsocietypublishing.org/journal/rsta ............................................................... |  |  |  |
|  | R. cost is |  |  |  |  |  |
|  | Soc. A 378: 20200053 yield given by CY = βEf Yc , (2.5) is the fusion energy per shot and β is a constant describing the cost in $/GJ. This cost can be bounded by recognizing that the primary coolant itself is, for both the First HYLIFE designs, the most expensive material in the reaction vessel. Both propose pure the primary coolant. The bulk cost for high-purity lithium metal is roughly $100/kg. All options, such as FLiBe or enriched lithium, are likely to be more expensive. In comparison, of steel is $1–5/kg. lower bound for the coolant mass can be found based on the mass required to absorb energy without egregious temperature rise. Using the heat capacity of lithium and a temperature rise of 400 C gives a cost of $70 k/GJ. An upper bound can be found from the HYLIFE which has a total lithium inventory of 800 tonnes, giving a cost of $44 M/GJ. It should however, that the $3600/MWe quoted above includes the lithium cost, i.e. there is a counting issue with these numbers. This is why the author thinks that the HYLIFE yield reasonably thought of as an upper bound. driver cost appears in both construction and operation because the replacement of parts important aspect in the cost of a fusion power plant. The lifetime of the driver itself is an consideration but there will also be replacement parts required in the reaction vessel perhaps elsewhere. In this simple model, these costs are all wrapped into a ‘replacement cost, representing a requirement for continued capital expenditure. driver cost is often discussed in terms of the cost per Joule of energy delivered to target. In we will use the cost per Joule of energy in the bank. This allows the driver efficiency separated from the cost more cleanly and reduces the range of values. The driver cost is γ Ed |  |  |  |  |  |
|  | Cd,con = (2.6) |  |  |  |  |  |
|  | Yc Cd,op = γ Ed Ld , (2.7) is the driver bank energy, Ld is the driver lifetime in years and γ is a constant describing cost in $/J. Some rough values for γ can be found by looking at existing machines. The costs roughly $4bn and has a 422 MJ capacitor bank, giving a cost of $9.5/J. First Light’s |  |  |  |  |  |
|  | Three has a bank energy of 2.5 MJ and cost $4.3 M, giving a cost of $1.7/J. It should be neither of these are rep rated machines. The increased engineering challenges inherent rated machine, for example increased wear on components, would likely increase the |  |  |  |  |  |
...
|  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
| where Eb is the energy multiple in the blanket or primary coolant and Pfus is the fusion power. |  |  |  |  |  |
| The energy multiple comes from the reaction between the fusion neutrons and lithium, which |  |  |  |  |  |
| regardless of any energy multiple must be used to produce tritium. The reaction with 6Li is |  |  |  |  |  |
| exothermic, but the reaction with 7Li is endothermic. This means that the overall sum can be endo￾or |  |  |  |  |  |
| exothermic depending on the approach used. A reasonable range for this value is between 0.6 |  |  |  |  |  |
| and 1.4. |  |  |  |  |  |
...
|  |  |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 75 50 25 0 10–2 10–1 >5 GJ 1–5 <1 GJ 1 frequency (Hz) LCOE | 11royalsocietypublishing.org/journal/rsta ............................................................... 10 GJ |  |  |  |  |  |  |
| Figure 3. LCOE versus frequency, coloured by fusion energy per shot. The sampling is as in figure 2, the less common in this case. An optimum frequency can be seen, but the location of this depends lower yield designs having an optimum at higher frequency. (Online version in colour.) and 3, respectively. In the Monte Carlo approach used, the gain was between 1 and the where that | Phil. Trans. R. Soc. A 378: with the low yield designs on the fusion yield, with randomly set to a value with the |  |  |  |  |  |  |
| limiting gain, limiting gain was commensurate selected driver energy used for that calculation. The results showed little this fuel configuration is fundamentally able to deliver enough gain to reach The second gain curve [19], given in equations (2.18–2.20), is much more of absolute gain attainable, reaching a maximum of approximately 250 for considered here. This gain curve adds the fuel mass density, ρ, the fuel implosion velocity, V, and the coupling efficiency, μc, as independent approach to sampling is used as for the first gain curve, i.e. between a gain specified by the gain curve. The results are shown in figure 4. In this case the impact is more gain removes the lowest cost designs, which need gains greater than smaller plant sizes are affected more than the larger ones by this compromise also depends on the actual cost levels, which in figure 4 Broader exploration of the results shows low-cost design points remain stringent limit on gain. (d) Minimum cost design point We will now return to the scan over all parameters but with the intention of lowest cost design points. Figure 5 shows a histogram of all calculated appears to show the possibility of very low-cost configurations. An example configuration for which the model finds an LCOE availability = 80%; driver lifetime = 40 M shots; thermal efficiency = 50%; plant cost = $1.5 k/kWe; driver cost = $3/J; target cost = $2/target, O&M cost = $20/kWe-yr; target energy = 5 MJ; frequency = 0.05 Hz; gain efficiency = 20%. This results in a thermal power of 300 MWth and electrical The dominant cost is the initial capital expenditure, where the plant cost cost is $75 M, and the yield cost is $25 M. The driver cost is low because of and high efficiency. Whether a system able to meet the frequency and lifetime really be built for this cost is very uncertain. The other question is about the plant costs. Costs for combined-cycle gas Rankine cycle and the associated space and infrastructure is required, can be | 20200053 randomly change, indicating that cost competitiveness. restrictive in terms the driver energies internal energy, e, the parameters. The same of one and the value pronounced; the limit on approximately 400. The restriction. The degree of remain at the defaults. even with the more understanding the values for LCOE and of $24.6/MWh is: discount rate = 4%; yield cost = $5 M/GJ; = 1000 and driver power 147.5 MWe. is $220 M, the driver the low target energy requirements could power plants, where a less than $1000/kWe; |  |  |  |  |  |  |
