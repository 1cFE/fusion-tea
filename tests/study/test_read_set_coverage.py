...
def _entry_ref(target: str) -> str:
    return f"stellarator_plant_params: StellaratorPlantParams {target}"


ENTRY_REF = _entry_ref("../inputs/stellarator_plant_params.json")
...
def test_a_ref_outside_the_pinned_file_list_fails(real_copy, tmp_path):
    ...
    (real_copy.path / "handwritten" / "params.json").write_text(
        (real_copy.path / "inputs" / "stellarator_plant_params.json").read_text()
    )
    real_copy.edit(PIPELINE, ENTRY_REF, _entry_ref("../handwritten/params.json"))
...
def test_a_ref_outside_the_package_root_fails(real_copy, tmp_path):
    ...
    outside.write_text((real_copy.path / "inputs" / "stellarator_plant_params.json").read_text())
    real_copy.edit(PIPELINE, ENTRY_REF, _entry_ref("../../outside.json"))
    out_path = tmp_path / "indicators.json"
    rc, out, err = real_copy.run(out=out_path)
    assert rc != 0
...
def test_the_coverage_check_runs_before_the_inputs_are_opened(real_copy, tmp_path):
    ...
    real_copy.edit(PIPELINE, ENTRY_REF, _entry_ref("../../not_even_json.txt"))
...
def test_a_renamed_inputs_file_is_followed_not_guessed(real_copy, tmp_path):
    ...
    (real_copy.path / "inputs" / "stellarator_plant_params.json").unlink()
    real_copy.edit(PIPELINE, ENTRY_REF, _entry_ref("../inputs/renamed.json"))
    ...
    assert "inputs/stellarator_plant_params.json" not in files
