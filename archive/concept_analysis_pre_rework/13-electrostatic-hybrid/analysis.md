...
No O&M cost data exists for the Orbitron at any scale. A breakdown by cost type, for LCOE model placeholder use:

- **Fixed O&M** (labor + scheduled maintenance): Dominant cost for a multi-module plant will be periodic cathode inspection and replacement, HV feedthrough servicing, and vacuum system maintenance. At module scale (~1–100 kWe), per-kWe labor costs are highly unfavorable — a technician visit costs the same regardless of module output. Placeholder: 3–5% of CAPEX/yr, biased high relative to large-plant fusion concepts due to small module size and novel radiation environment. No data to anchor this.
- **Variable O&M** (consumables, unplanned failures): Cathode erosion rate under neutron bombardment and ion bombardment is unknown; cathode replacement is the primary variable cost driver. HV feedthrough dielectric replacement under cumulative neutron dose is a secondary concern. No published lifetime data on either. Placeholder: $5–15/MWh (order-of-magnitude estimate borrowing from fission small modular reactor analogy; not validated for this concept).
- **Scheduled maintenance intervals**: Not disclosed. At the Marty prototype scale, maintenance intervals are experimental program cadence, not commercial schedule. No commercial maintenance cycle has been designed.
- **Unplanned outage costs**: Cathode failure and HV arc events are the most likely unplanned downtime triggers. Both are unknown-rate events at fusion-relevant operating conditions. Steady-state operation is favorable for capacity factor if component reliability is high, but neutron-induced material degradation may impose shorter-than-expected replacement cycles.
...
| Fixed O&M cost (placeholder) | [estimated] 3–5% of CAPEX/yr | [estimated: unfavorable per-kWe labor scaling at module size; no Orbitron data; biased high vs. large-plant fusion analogues] | low | Dominant cost: cathode + HV feedthrough scheduled maintenance; per-kWe labor cost highly unfavorable at 1–100 kWe module scale |
| Variable O&M cost (placeholder) | [estimated] $5–15/MWh | [estimated: small modular reactor analogy; no fusion-module data; cathode replacement rate unknown] | low | Primary driver is cathode erosion under neutron/ion bombardment; HV dielectric replacement secondary; no component lifetime data exists |
| Scheduled maintenance interval | [estimated] unknown; placeholder 1–2 yr | [estimated: analogy to vacuum/HV systems in accelerator facilities; neutron damage may shorten interval below this; no Orbitron data] | low | No commercial maintenance schedule disclosed; cathode and feedthrough lifetime under 14 MeV neutron flux is undetermined |
...
| O&M cost structure (fixed vs. variable, scheduled + unplanned) | truly-unknown | important | Placeholder provided (3–5% CAPEX/yr fixed; $5–15/MWh variable); cathode and HV feedthrough component lifetimes under neutron bombardment are the key unknowns driving both rates |
...
| 10 | O&M cost structure (fixed + variable, scheduled + unplanned) — component lifetimes under neutron bombardment are the binding unknown | S2, S5 | truly-unknown | important | Placeholder rates provided in S2 and S5 (3–5% CAPEX/yr fixed; $5–15/MWh variable; 1–2 yr maintenance interval); actual rates depend on cathode erosion and HV dielectric lifetime under 14 MeV neutron flux — no published data exists |
