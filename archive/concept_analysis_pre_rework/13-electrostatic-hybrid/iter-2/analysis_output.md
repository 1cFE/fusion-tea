...
1. **Q_engineering** — the single most dominant uncertainty. LCOE is undefined below Q_engineering ≈ 2–3 for any thermal cycle; even at Q_engineering = 5 the conversion losses at kWe scale may preclude economic viability. This is not yet experimentally constrained — the space-charge-mitigated density regime required for Q>1 has not been demonstrated in any device.
...
3. **Tritium consumption rate (a function of Q)** — unlike all other D-T concepts in the portfolio, the Orbitron has no breeding blanket and no disclosed path to one. Tritium must be purchased indefinitely, making this a permanent OPEX driver. At Q_physics ≈ 1, the tritium consumption rate per unit electricity output is high and LCOE-impacting. These three parameters — not the qualitative challenges above — are the axes the back-solve model should sweep.
...
A conventional LCOE model cannot be built for the Orbitron at current maturity. The appropriate TEA treatment is a back-solve: parameterize Q_engineering and capital cost per kWe, and compute the LCOE surface to identify which (Q_engineering, $/kWe) combinations yield commercially viable LCOE ≤ $100/MWh. The model should:

1. **Sweep Q_engineering from 1 to 20** and capital cost per kWe from $500 to $10,000/kWe — the plausible range for a modular manufactured device.
2. **Use two thermal efficiency scenarios**: thermoelectric at 10–15% (realistic at kWe scale) and a hypothetical multi-module turbine array at 25–35% (optimistic, requires megawatt aggregate scale).
3. **Include tritium fuel cost as a function of Q** — since no breeding blanket exists, tritium consumption per MWh at each Q value is a third cost axis. At Q_engineering ≈ 1–2, tritium cost at >$35,000/g could add $5–50/MWh depending on consumption rate.
4. **Identify the minimum Q_engineering threshold** under each efficiency scenario — this is the key quantitative output that converts "back-solve" from a methodology into a D1+ result. Under the thermoelectric scenario, the minimum Q_engineering for LCOE ≤ $100/MWh is likely substantially higher than the ~2–3 floor for turbine-based conversion.
5. **Compare thresholds against known physics barriers** — if the Coulomb collision problem constrains Q to << 5, the required Q_engineering under realistic efficiency assumptions may not be achievable regardless of capital cost.

This is a fundamentally different modeling posture than for mature MFE or IFE concepts where physics is anchored and cost is the primary uncertainty. The model output should be presented as an explicit viability map, not a central LCOE estimate.
...
**12. Approved D1+ Analysis: Spherical Tokamak - HTS (21-spherical-tokamak-hts)**
