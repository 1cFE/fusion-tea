...
Viability threshold (Q_plasma breakeven, ignoring auxiliaries):
  ...
  At f_rad=0.15, η_th=0.32, η_NBI=0.26:  Q_break ≈ 14.1 (mathematical breakeven)
  Commercial minimum: Q_plasma ≥ 25–35 for 50 MWe net with margin.
  (η_NBI = 0.26 reflects corrected two-step efficiency chain from C-2W data:
   source ~0.60 × coupling ~0.43 (duct losses <50%, 15% shine-through).
   Source: OSTI 2441289, Nuclear Fusion 2024.)
...
class PB11FRCPlantParams:
    ...
    Mathematical breakeven ≈ 14.1 (at η_th=0.32, η_NBI=0.26, f_rad=0.15);
    commercial minimum Q ≥ 25–35 for 50 MWe net with adequate margin.
    Default updated from Q=15 to Q=30 to reflect corrected η_NBI physics:
    with η_NBI=0.26 (two-step efficiency chain), Q=15 falls below Q_break≈14.1
    with no margin for auxiliaries — Q≥25 needed for meaningful net output.
    Current record: C-2W/Norman at ~1 keV total temperature, Q << 1.
    Da Vinci target: ~150 keV ion temperature — ~150× extrapolation unvalidated.
    Ref: analysis.md §Section 2 Challenge 3; OSTI 2441289. HIGH UNCERTAINTY.
    TESTABLE PROPOSITION: IF Q_plasma ≥ 25–35 AND η_NBI ≥ 0.26, THEN net
    ...
    Source: C-2W/Norman operated at up to 21 MW NBI [tae-c2w-machine-details.md].
    26 MW is a modest scale-up. At corrected η_NBI=0.26, this requires ~100 MW
    wall-plug NBI draw. At Q=30, P_net ≈ (30×26×0.85×0.32) − 100 − 7.5 ≈ 104 MWe
    — larger than the 50 MWe Da Vinci target; for a 50 MWe plant at Q=30, only
    ~13 MW plasma-coupled NBI is needed (wall-plug ~50 MW). Da Vinci beam energy
    (>120 keV) requires negative-ion NBI, introducing neutralization challenges.
    Ref: analysis.md §Section 3 NBI System at 250+ keV. MODERATE UNCERTAINTY."""

    eta_nbi_source: float = 0.60
    """NBI source efficiency (wall-plug power → beam power at NBI output).
    Source: Negative-ion NBI source efficiency for fusion-class systems: ~55–65%.
    This is the efficiency of the NBI power supply + ion source + accelerator step,
    before any duct transmission or plasma coupling losses.
    Ref: OSTI 2441289 (Nuclear Fusion 2024, C-2W); analysis.md §Section 2 Challenge 3.
    MODERATE UNCERTAINTY."""

    eta_nbi_couple: float = 0.43
    """NBI plasma coupling efficiency (beam power → plasma-absorbed power).
    Includes duct/geometric transmission losses and beam shine-through.
    C-2W experimental data (OSTI 2441289): NB power reaching plasma is <50% of
    NBI electrical input from duct/geometric losses alone (i.e., coupling ≤ 0.50);
    an additional 15±5% shine-through was measured at C-2W conditions.
    Net coupling ≈ 0.50 (geometric) × 0.85 (shine-through) = 0.425 ≈ 0.43.
    Ref: OSTI 2441289 §NBI System [iter-02/sources/]. HIGH UNCERTAINTY
    (C-2W geometry; Da Vinci scale coupling efficiency unvalidated)."""

    eta_nbi: float = 0.26
    """NBI total wall-plug efficiency (wall-plug power → plasma-absorbed power).
    Corrected to reflect two-step efficiency chain from C-2W experimental data:
      η_NBI = η_NBI_source × η_NBI_couple = 0.60 × 0.43 ≈ 0.26
    Source: OSTI 2441289 (Nuclear Fusion 2024, C-2W): duct/geometric losses
    alone reduce NB power reaching the plasma to <50% of NBI electrical input;
    additional 15±5% beam shine-through measured. NBI source efficiency ~60%.
    Prior estimate (η_NBI=0.55) was the source-only step and overstated the
    plasma-coupled efficiency by ~2×.
    Range: 0.20 (conservative: low source × high losses) to 0.32 (optimistic:
    high source × favorable coupling); 0.45 upper bound under optimistic assumptions.
    At η_NBI=0.26: Q_break (steam, f_rad=0.15, η_th=0.32) ≈ 14.1;
    commercial minimum Q ≥ 25–35 (roughly double the prior ~10–15 estimate).
 ...
 # standardized from 0.32 per scoring_framework.md (Energy Capture: Thermal (steam))
    ...
    u_b11_per_kg: float = 50.0
    """B-11 fuel unit cost [$/kg].
    Source: Natural boron ~$2–5/kg; lightly enriched B-11 ~$20–75/kg.
    Using $50/kg as a representative lightly enriched FOAK estimate.
    Prior value ($10,000/kg from 1costingfe FOAK placeholder) was not physically
    motivated — it implied industrial isotope-separation pricing inapplicable to
    the large-scale supply TAE's concept requires.
    At correct annual consumption (~140 kg/yr at 390 MW fusion power), even at
    $50/kg fuel cost is ~$7,000/yr — negligible compared to O&M and capital.
    ...
    def _compute_economics(self, power: dict, costs: dict, cas22: dict) -> dict:
        ...
        # Annual B-11 purchased = actual mass burned (net consumption).
        # Prior code divided by 0.05 burn fraction, inflating by 20× — this
        # confused gross fuel feed with net annual purchase. The annual purchase
        # equals the burned mass regardless of recycle fraction.
        annual_b11_kg = b11_burn_rate_kg_s * self.plant_availability * 3600 * 8760
...
def main():
    ...
    print("=" * 70)
    print("""
The p-B11 fusion reaction requires ion temperatures ~150 keV —
~150× above the C-2W/Norman record of ~1 keV (recent campaigns, OSTI 2441289).
Q_plasma > 1 has never been demonstrated for p-B11 in ANY confinement device.

NBI efficiency correction (OSTI 2441289, Nuclear Fusion 2024):
  η_NBI is a two-step chain, not a single wall-plug number:
    η_NBI_source  ≈ 0.55–0.65  (wall-plug → beam, NBI source efficiency)
    η_NBI_couple  ≈ 0.35–0.50  (beam → plasma: duct losses <50% + 15% shine-through)
    η_NBI_total   ≈ 0.20–0.32  (product; 0.45 upper bound, optimistic)
  Prior analyses using η_NBI~0.55 were citing the source-only step;
  plasma-coupled efficiency is ~2× lower per C-2W experimental data.

Mathematical Q_plasma breakeven (rated power, ignoring auxiliaries):
  Q_break = 1 / [(1 − f_rad) × η_conv × η_NBI]

  Branch B (steam, f_rad=0.15, η_th=0.32, η_NBI=0.26):  Q_break ≈ 14.1
  Branch B (steam, f_rad=0.15, η_th=0.32, η_NBI=0.20):  Q_break ≈ 18.4  (conservative)
  Branch B (steam, f_rad=0.15, η_th=0.32, η_NBI=0.40):  Q_break ≈  9.2  (optimistic)
  Branch C (ICC,   f_rad=0.15, η_icc=0.90, η_NBI=0.26): Q_break ≈  5.0

Commercial minimum (with 50 MWe margin and auxiliaries):
  Branch B, η_NBI=0.26:  Q_plasma ≥ 25–35  (roughly 2× prior estimate of 10–15)
  Branch B, η_NBI=0.20:  Q_plasma ≥ 35–50
  Branch B, η_NBI=0.40:  Q_plasma ≥ 15–20
...
""")
    ...
    # ----------------------------------------------------------------
    print("\n" + "#" * 70)
    print("# BRANCH B BASELINE: Q=30, η_NBI=0.26, CF=85%, Steam 32%")
    print("# (corrected η_NBI from two-step efficiency chain, OSTI 2441289)")
    print("#" * 70)
    ...
    # ----------------------------------------------------------------
    print("\n" + "#" * 70)
    print("# BRANCH C: Q=30, η_NBI=0.26, CF=85%, ICC 90%")
    print("#" * 70)
    ...
    print(f"  Q_break ≈ 14.1 (steam, η_NBI=0.26 corrected); commercial min ≈ 25–35")
    print(f"  {'Q_plasma':>10}  {'Result'}")
    q_values = [3.0, 5.0, 10.0, 14.0, 15.0, 20.0, 25.0, 30.0, 40.0, 50.0]
    for row in sensitivity_sweep(p_b_baseline, "q_plasma", q_values):
        flag = " *** BREAKEVEN" if abs(row["param_value"] - 14.0) < 0.5 else ""
        flag = " *** COMMERCIAL MIN" if abs(row["param_value"] - 25.0) < 0.1 else flag
        print(f"  {row['param_value']:>10.1f}  {_lcoe_str(row)}{flag}")

    # 2. NBI total wall-plug efficiency (corrected range from OSTI 2441289)
    print(f"\n--- 2. NBI total wall-plug efficiency sweep ---")
    print(f"  Range 0.20–0.45 reflects corrected two-step chain: source 0.55–0.65")
    print(f"  × coupling 0.35–0.50 (C-2W duct losses + 15% shine-through).")
    print(f"  Prior sweep [0.35–0.65] overstated plasma-coupled efficiency by ~2×.")
    print(f"  Cite: OSTI 2441289 (Nuclear Fusion 2024).")
    print(f"  {'η_NBI':>10}  {'Result'}")
    for row in sensitivity_sweep(p_b_baseline, "eta_nbi",
                                  [0.20, 0.25, 0.26, 0.30, 0.35, 0.40, 0.45]):
        flag = " *** baseline" if abs(row["param_value"] - 0.26) < 0.001 else ""
        print(f"  {row['param_value']:>10.2f}  {_lcoe_str(row)}{flag}")
    ...
    # 7. Radiation loss fraction (bremsstrahlung) — the #1 physics unknown
    print(f"\n--- 7. Radiation loss fraction f_rad sweep ---")
    print(f"  f_rad is the single most uncertain physics parameter: in a fully")
    print(f"  thermalized p-B11 plasma, f_rad > 1.0 is possible (bremsstrahlung")
    print(f"  exceeds fusion output). TAE's T_i >> T_e strategy aims to suppress")
    print(f"  this — but the degree of suppression at 150 keV is undemonstrated.")
    print(f"  Threshold above which P_net ≤ 0 is identified for each Q.")
    print(f"  {'f_rad':>10}  {'Result'}")
    f_rad_values = [0.05, 0.10, 0.15, 0.30, 0.50, 0.70, 0.90]
    for row in sensitivity_sweep(p_b_baseline, "f_rad", f_rad_values):
        flag = " *** baseline" if abs(row["param_value"] - 0.15) < 0.001 else ""
        print(f"  {row['param_value']:>10.2f}  {_lcoe_str(row)}{flag}")

    # 8. Q_plasma vs η_NBI viability grid (corrected η_NBI range)
    print(f"\n--- 8. Q_plasma × η_NBI viability boundary grid ---")
    print(f"  η_NBI range updated to 0.20–0.45 (corrected two-step efficiency chain).")
    print(f"  (Branch A = no net electricity; B = viable steam; C = ICC)")
    q_grid = [10, 15, 20, 25, 30, 50]
    nbi_grid = [0.20, 0.25, 0.30, 0.35, 0.40, 0.45]
    ...
    # 9. Q_plasma × f_rad viability grid
    print(f"\n--- 9. Q_plasma × f_rad viability boundary grid ---")
    print(f"  Converts the qualitative bremsstrahlung challenge (Section 2)")
    print(f"  into a quantitative viability map. Identifies the f_rad threshold")
    print(f"  above which Branch A is forced at each Q, at baseline η_NBI=0.26.")
    print(f"  (Branch A = no net electricity; B = viable steam)")
    q_grid2 = [10, 15, 20, 25, 30, 50]
    frad_grid = [0.05, 0.15, 0.30, 0.50, 0.70, 0.90]
    header2 = f"  {'Q\\f_rad':>10}" + "".join(f"  {f:.2f}" for f in frad_grid)
    print(header2)
    for q in q_grid2:
        row_str = f"  {'Q='+str(q):>10}"
        for f in frad_grid:
            kw = {**p_b_baseline.__dict__, "q_plasma": float(q), "f_rad": f}
            pw3 = PB11FRCPlantParams(**kw).compute()["power"]
            if pw3["branch"] == "A":
                row_str += "     A  "
            else:
                lc3 = PB11FRCPlantParams(**kw).compute()["economics"]["lcoe_cents_per_kWh"]
                row_str += f" {lc3:5.0f}¢ "
        print(row_str)
    print(f"  (values in ¢/kWh; A = Branch A, no LCOE)")
    ...
    # η_NBI values corrected to two-step efficiency chain (OSTI 2441289):
    # 0.22 ≈ conservative (low source × high losses), 0.26 ≈ midpoint,
    # 0.40 ≈ optimistic (high source + favorable coupling).
    # Q_plasma thresholds updated: commercial min ≥ 25–35 at η_NBI=0.26.
    scenarios = [
        ("Branch A — Physics fails (Q=3)",
         PB11FRCPlantParams(q_plasma=3.0)),
        ("B Conservative (Q=20, η=0.22, CF=70%)",
         PB11FRCPlantParams(q_plasma=20.0, eta_nbi=0.22, plant_availability=0.70)),
        ("B Baseline (Q=30, η=0.26, CF=85%)",
         PB11FRCPlantParams()),
        ("B Optimistic (Q=50, η=0.40, CF=90%)",
         PB11FRCPlantParams(q_plasma=50.0, eta_nbi=0.40, plant_availability=0.90)),
        ("C Baseline — ICC (Q=30, η=0.26, CF=85%)",
         PB11FRCPlantParams(use_icc=True)),
        ("C Optimistic — ICC (Q=50, η=0.40, CF=90%)",
         PB11FRCPlantParams(q_plasma=50.0, eta_nbi=0.40,
  ...
  1. Q_PLASMA — The fundamental physics unknown (binding constraint)
     ...
     C-2W/Norman achieved ~1 keV total temperature (OSTI 2441289); Da Vinci
     requires ~150 keV ion temperature — a ~150× extrapolation with no validated
     scaling law. Q_plasma is not just the top sensitivity parameter: it determines
     whether the concept produces ANY net electricity. Below Q ≈ 14.1 (steam,
     corrected η_NBI=0.26), there is no LCOE — Branch A has no meaningful cost
     model output. The prior breakeven of Q≈6.7 assumed η_NBI=0.55 (source only).
  ...
  2. NBI CAPITAL COST & RECIRCULATING POWER FRACTION (jointly binding)
     ...
     It is also the dominant recirculating load (~100 MW wall-plug at baseline,
     from 26 MW plasma / η_NBI=0.26 — more than doubling the prior ~49 MW estimate
     that used the uncorrected η_NBI=0.55). These two effects reinforce: high NBI
     cost → high capital LCOE component; high NBI recirculating load → low net
     output → high LCOE denominator. At $10M/MW_NBI, NBI accounts for >50% of
     CAS22. No commercial supply chain exists for fusion-scale high-energy NBI.
  ...
  3. ENERGY CONVERSION ARCHITECTURE — ICC vs. Steam (structural LCOE gap)
     ...
     difference in gross electricity from identical fusion power. At the corrected
     Q=30 baseline, Branch B LCOE ≈ $119/MWh vs. Branch C ≈ $44/MWh — ICC
     delivers roughly 2.7× lower LCOE. ICC is patent-stage only — zero
