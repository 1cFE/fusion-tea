"""MagLIF (D-T) — 1costingfe costing model setup (iter-6).
...
ITER-6 UPDATES vs. ITER-5:
  - Source integration finding F-1: Pacific Fusion's commercial design point is 250 MWe
    (≤25 acres footprint), not the 1000 MWe Z-IFE reference. The Z-IFE study shows COE
    scales significantly with plant size; a 250 MWe plant sits in a materially worse
    economy-of-scale position than the 1000 MWe reference.
    Source: ans-news-2025-04-24-article-6980-pacific-fusion-fusing.md (Keith LeChien, PF CTO)
  - Added dual-scenario comparison: Scenario A (1000 MWe, Z-IFE reference for cross-concept
    benchmarking) and Scenario B (250 MWe, Pacific Fusion commercial design point).
  - Source integration finding F-2: GA partnership (April 2025) noted as organizational
    step toward target fabrication capability — does not resolve gap but narrows uncertainty.
  - Analysis.md §2 Scale note now explicit: "All COE figures cited in this analysis should
    be treated as lower bounds for Pacific Fusion's commercial design point."
...
KEY DEVIATIONS FROM 1costingfe MAG_TARGET DEFAULTS:
  ...
  Unchanged from defaults:
    ...
    - C220108 (Target/RTL Factory): $244M at 1 GWe default — Z-IFE costed separately
      but gave no sub-account breakdown; UNCERTAIN: cryo target production may be more.
    ...
    - CAS70 (O&M): DT default (~$52M/yr at 1 GWe) — does NOT capture per-shot consumable
      O&M (~$15.75M/yr additional at $1/shot × 0.5 Hz; cryo targets could be far more).
...
SHARED_KWARGS = dict(
    ...
    eta_pin=0.60,    # Pulsed power wall-plug efficiency — LTD driver architecture
                     ...
                     # NOTE: IMG claims ~90% wall-plug efficiency [PMF §3.2] — not verified
                     # at plant scale. Using LTD figure as conservative anchor.
    ...
    #   consumable O&M is NOT captured — a material omission. Rough estimate:
    #   $1/shot × 0.5 Hz × 3.15×10^7 s/yr ≈ $15.75M/yr additional variable O&M.
...
# NEW IN ITER-6: Pacific Fusion CTO LeChien (April 2025) stated 250 MWe target
# with ≤25 acres footprint. This is 4× smaller than the Z-IFE reference.
# Z-IFE economy-of-scale data: 1000 MWe 0.5 Hz → 7.0 ¢/kWeh; 2000 MWe → 5.7 ¢/kWeh.
# A 250 MWe plant sits below the Z-IFE 1000 MWe reference in economy-of-scale.
...
print()
print("SCENARIO B: 250 MWe — Pacific Fusion commercial design point (iter-6)")
...
print("  6. ITER-6 new: 250 MWe commercial target (Scenario B) faces economy-of-scale")
print("     penalty vs. 1000 MWe Z-IFE reference — all published COE figures are lower")
...
print("  2. Target $/shot       — variable O&M floor (not in framework)")
