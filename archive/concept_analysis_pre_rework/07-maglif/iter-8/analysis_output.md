...
- **Hypothesis 1 (Rep rate break-even)**: The rep rate sweep must be model-computed, not a citation of Z-IFE tabulated results. Parameterize output power as fusion_power ∝ rep_rate × yield_per_shot at fixed driver capital, and compute LCOE at 0.1, 0.25, 0.5, 1.0, and 1.8 Hz. The Z-IFE lookup table in Section 2 above provides cited reference values for calibration, but the model must reproduce these algebraically before being extended to IMG scenarios.
- **Hypothesis 2 (Target cost tolerance)**: Consumable O&M ($/shot × shots/year) must enter the LCOE numerator directly, not as a separate addendum table. Sweep $/shot = 0, 1, 2, 5, 10 and report the $/shot at which LCOE crosses 100 and 150 $/MWh.
- **Hypothesis 3 (Driver cost cliff)**: Driver capital must be swept as a free parameter. Compute Scenario A LCOE at C220104 = $372M (LTD reference), $200M, $100M, $75M (2×, 4×, 5× reductions) with paired efficiency adjustments if IMG wall-plug efficiency differs from LTD baseline. A zeroed driver capital sensitivity in the model cannot answer this hypothesis.
