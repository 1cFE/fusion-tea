...
**Key data gaps**:
- No system-code outputs with MagLIF-specific parameters exist; pyFECONs (Woodruff et al., arXiv:2602.19389, Feb 2026) now includes a MIFE cost-driver track with Account 22.1.3 as a controlled swap-point for structurally different drivers, but MagLIF-specific cost parameters have not been published from this framework[2b]
...
**6. TEA tools for pulsed fusion are underdeveloped.** The arxiv paper explicitly flags this: "cost optimization models and tools have been developed for tokamaks; investment in new reduced models is required so those tools may be applied to pulsed fusion systems." No equivalent of ARIES or PROCESS exists for MagLIF. The Z-IFE systems code is the only published plant-level model, and it is two decades old and based on a different driver architecture. The 2026 pyFECONs extension (Woodruff et al., arXiv:2602.19389) introduces MIFE as an explicit cost-driver track alongside MFE and IFE, treating the driver account as a swap-point with full cost-account development per architecture class — the first published costing framework that covers MagLIF's cost category.[2b] However, MagLIF-specific account parameters (driver cost priors, per-shot consumable accounts) have not been published; the full paper should be acquired to determine whether it provides usable parameter priors for the cost model.

**Modeling recommendation.** Reference-class scaling approaches (1costingfe, ARIES-analogous tools, PROCESS) are not applicable to MagLIF because the dominant cost categories — pulsed power driver capital, per-shot consumables, rep-rated chamber clearing — have no analogues in the databases those tools are built on. The emerging pyFECONs MIFE track (arXiv:2602.19389) provides the first standards-aligned framework that structurally accommodates MagLIF's cost categories, but its MagLIF-specific account parameters are unknown until the full paper is reviewed. In the interim, a free-form parametric model is required, treating rep rate (Hz), yield per shot (GJ), driver capital cost ($/J), and target cost ($/shot) as the four primary free variables and deriving COE algebraically from their interactions. The Z-IFE systems code provides the structural template; it should be updated with IMG architecture driver cost estimates when available.

**Availability (central case)**: The model uses a canonical availability of **0.75** per `scoring_framework.md §Plant availability` (Pulsed MIF, D-T). Z-IFE assumed 85% without explicit attribution; because no Tier-A externally-published availability basis exists for this concept, the canonical policy value is used to ensure apples-to-apples LCOE comparisons across pulsed-MIF and pulsed-IFE concepts in this study. Availability sensitivity scenarios (0.65–0.92) remain informative and are retained in the sweep.

**Model implementation requirements for the three hypotheses:**
- **Hypothesis 1 (Rep rate break-even)**: Model-computed — see REP RATE SCENARIO SWEEP in model_setup.py. Net power parameterized as yield_per_shot_GJ × rep_rate_hz × eta_th × mn × 1000 MWe at fixed single-chamber capital (CAS90 annualized). LCOE computed algebraically at 0.1, 0.25, 0.5, 1.0, and 1.8 Hz. **Energy capture classification**: z-ife-sand2006-7148-thermal-cycles.md §3.2 specifies a Combined Brayton-Rankine cycle — a specific cycle architecture, not an unspecified thermal system. The matching canonical in scoring_framework.md is 0.50 ("Thermal (combined cycle, Brayton-Rankine)"). The model uses η_th = 0.42 as a justified deviation, citing the steel-chamber near-term regime; the 0.50 canonical requires C-C composite chambers not yet commercially available. Inline model comment reads: `# DEVIATION (justified): Energy capture = Thermal (combined cycle, Brayton-Rankine); canonical 0.50. Using 0.42 per z-ife-sand2006-7148-thermal-cycles.md §3.2 (steel-chamber near-term regime; 0.50 requires C-C composite chambers not yet commercial).` A second scenario branch with η_th = 0.50 (advanced material assumption) is included in the rep-rate sweep so downstream comparisons can isolate the material-readiness assumption from the architectural one. **ETA_PIN / ETA_TH interaction**: the combined sensitivity of driver efficiency (ETA_PIN = 0.60 for LTD; IMG claims ~0.90, unverified) and thermal efficiency (0.42 vs. 0.50) matters more for Z-IFE-class LCOE than either parameter alone — the product (η_pin × η_th) determines effective wall-plug-to-electricity conversion and the required fusion yield per shot; a simultaneous IMG driver efficiency improvement (0.60 → 0.90) and advanced thermal cycle (0.42 → 0.50) would reduce the required yield-per-shot by ~27% at fixed net output, reducing the gain requirement that determines whether ignition is commercially accessible. Calibration: model LCOE at 0.5 Hz (see model_output.txt). Break-even with advanced fission (~40–60 $/MWh) requires rep rate ≥ ~1 Hz at this capital base.
- **Hypothesis 2 (Target cost tolerance)**: Model-computed — see PER-SHOT CONSUMABLE COST SWEEP in model_setup.py. Consumable O&M (cost_per_shot × shots/yr) enters LCOE numerator directly. At 0.5 Hz baseline (15.8M shots/yr, 75% availability): break-even at 100 $/MWh ≈ $9.3/shot; break-even at 150 $/MWh ≈ $30.1/shot. Current cryo target cost ($thousands/shot) is orders of magnitude above the commercial viability threshold (~$1–2/shot).
- **Hypothesis 3 (Driver cost cliff)**: Model-computed — see DRIVER COST SWEEP in model_setup.py. Driver capital (C220104) swept from $372M (LTD ref) to $37M (10× reduction). Note: with CAS21 buildings corrected to ~$200M (vs. default $919M), driver capital is a larger share of total capital and driver cost sensitivity is higher than the uncorrected model implied.
...
|-------------|----------------------|--------|
| Buildings / site infrastructure (CAS21) | Reactor hall, turbine hall; MFE/fission scaling formula | **Probable scaling artifact** — default MFE/fission formula gives $919M, exceeding the driver ($372M). Pacific Fusion DS footprint (73m × 80m) anchors a physical estimate at ~$130–200M. Double-count risk: the Z-IFE $372M driver figure may already include the capacitor hall. Model uses $200M override (±100%); $919M is the high-end bound. |
...
[2b] arxiv-2602-19389.md §Abstract: "The IWG development reorganizes and deepens the framework around three architecture-defining cost-driver tracks for Magnetic Fusion Energy (MFE), Inertial Fusion Energy (IFE), and Magneto-Inertial Fusion Energy (MIFE). In particular, the generic driver placeholder in Account 22.1.3 is treated as a controlled swap-point and replaced by a full cost-account development for the dominant driver in each class" — abstract only; MagLIF-specific account parameters not available without the full paper
...
| Thermal efficiency — steel chamber (model baseline) | 42% | z-ife-sand2006-7148-thermal-cycles.md §3.2 | medium | Combined Brayton-Rankine, steel chamber, near-term achievable. Category: "Thermal (combined cycle, Brayton-Rankine)"; scoring_framework.md canonical = 0.50. Model uses 0.42 as justified deviation (steel-chamber regime). |
| Thermal efficiency — C-C composite (advanced scenario) | 50% | z-ife-sand2006-7148-thermal-cycles.md §3.2 | low | Combined Brayton-Rankine, carbon-composite chamber; scoring_framework.md canonical. Requires high-temperature materials (ODS steels, SiC composites) not yet commercially available; model includes as η_th sensitivity branch. |
...
| Plant availability (model baseline) | 0.75 | scoring_framework.md §Plant availability | high | Canonical per scoring_framework.md §Plant availability (Pulsed MIF, D-T); previously 0.85. Z-IFE assumed 0.85 without explicit attribution; no Tier-A basis exists to override the canonical value. Cross-concept LCOE comparisons within the pulsed-MIF/IFE family are now apples-to-apples on this dimension. |
| Capacity factor — thick liquid wall success scenario | 85–90% | z-ife-sand2006-7148-thermal-cycles.md §3.1.1.5 | low | Z-IFE 85% assumption stated without attribution to liquid wall; implicitly presupposes periodic first-wall replacement is eliminated; upper range reflects potential upside if solid maintenance cycles are fully removed. Note: the model uses the canonical 0.75, not this source-cited range. |
...
| Buildings / site (CAS21) — physical estimate | ~$130–200M | [inferred from pacific-fusion-interview-fusion-report.md §DS Architecture; scaled to commercial plant] | low | Physical footprint basis: DS = 73m × 80m = 5,840 m²; commercial plant 20,000–25,000 m² at $6,000–8,000/m² nuclear-grade + industrial areas. Model uses $200M override. Default MFE/fission scaling gives $919M — probable double-count of capacitor hall already in Z-IFE $372M driver figure. |
...
| Capacity factor (demonstrated availability) | truly-unknown | blocking | Model uses canonical 0.75 per scoring_framework.md; Z-IFE assumed 0.85 without explicit attribution; neither figure is based on demonstrated rep-rated operation — no such system exists |
...
|---|-----------------|---------|----------|-------------|-------------------------|-----------------------|
| 1 | No plant study for IMG architecture: all TEA data from Z-IFE LTD-era (2006) | S1, S5 | not-yet-sourced | blocking | All COE estimates carry ±50%+ uncertainty; driver capital (96% of CapEx) may be wrong by a factor of 2–5× in either direction | **Acquire full pyFECONs paper** (Woodruff et al., arXiv:2602.19389, Feb 2026) — this is the first published framework with an explicit MIFE cost-driver track; if it provides account-level priors for the driver swap-point it may partially address this gap. Also: ARPA-E award documents, company technical blogs, future Pacific Fusion publications |
...
11. **Woodruff et al. (2026)** — "Extension of the fusion power plant costing standard," arXiv:2602.19389 (February 2026). CATF International Working Group on Fusion Cost Analysis. Documents pyFECONs framework extensions introducing MIFE as an explicit cost-driver track alongside MFE and IFE. Account 22.1.3 is treated as a controlled swap-point replaced by architecture-specific driver cost accounts. Adds probabilistic costing, safety-informed costing, and expanded finance/value metrics. **Abstract only** — MagLIF-specific account parameters not yet extracted. Full paper should be acquired: if it provides account-level cost priors for MIFE drivers, it may partially address Gap #1.
    - Path: `iter-03/sources/arxiv-2602-19389.md`
