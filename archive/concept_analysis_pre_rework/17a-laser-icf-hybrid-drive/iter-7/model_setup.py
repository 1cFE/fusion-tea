...
# Canonical value: 0.75 per scoring_framework.md §Plant availability (Pulsed IFE, D-T).
# Previously 0.85 (author judgment matching HYLIFE-II optimistic analogue [HY2]).
# Tier-A override (externally-published availability target with stated basis) would be
# required to deviate; no such citation exists for this concept. Central case is now
# policy-driven, enabling apples-to-apples cross-concept LCOE comparisons within the
# pulsed-IFE family.
...
#   85% (optimistic) for the non-driver plant systems. The canonical 0.75 aligns
#   with the HYLIFE-II conservative anchor. Laser driver service interval unknown.
# Source: scoring_framework.md §Plant availability; [i6si] §F-1; [HY2] §Plant Availability
AVAILABILITY = 0.75  # canonical per scoring_framework.md §Plant availability (Pulsed IFE, D-T)
...
print()
print(f"  Availability:     {AVAILABILITY:.0%} (canonical per scoring_framework.md §Plant availability, Pulsed IFE D-T)")
