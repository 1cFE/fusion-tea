...
The TEA model, calibrated to the ARIES stellarator framework rather than the published HESTIA cost table, produces a lower-bound LCOE of approximately $1,145/MWh (at ARIES-framework capital cost structure, P_net = 70 MWe, CF = 85% — canonical per scoring_framework.md §Plant availability). This is a structural lower bound because the ARIES framework cannot reproduce the published $10B inflation-adjusted overnight cost. To bound the range from above, applying the published cost directly: at $10B overnight cost, 70 MWe net output, 85% capacity factor, an 8.6% fixed charge rate (8% discount, 40-year life), and a nominal O&M of $60/kWe-yr (ARIES-CS analogue), the LCOE resolves to approximately **$1,764/MWh** (~$1.76/kWh). This upper-bound figure is driven almost entirely by the inflation-adjusted capital cost. For cross-concept comparison, both bounds must be reported: the framework lower bound (~$1,145/MWh) and the published-cost-anchor upper bound (~$1,764/MWh), with the caveat that the FOAK design point improves to approximately $700–1,000/MWh if the $6B inflation-adjusted cost and 103 MWe output are used. Any single LCOE figure for HESTIA without this range stated is misleading. Note: the central-case availability of 0.85 is policy-driven (scoring_framework.md canonical value for MCF steady-state D-T), not a concept-specific commitment — cross-concept LCOE comparisons within this family are now apples-to-apples on this dimension.
...
| Availability target (FPP) | >80–85% (model: 0.85) | aip-2023-paper-abstract.md §Table I | high | Published target >85% FPP, >90% FOAK. Model uses **0.85** — canonical per scoring_framework.md §Plant availability (MCF steady-state, D-T); previously 0.83. Cross-concept LCOE comparisons within the MCF family are apples-to-apples on this dimension. |
