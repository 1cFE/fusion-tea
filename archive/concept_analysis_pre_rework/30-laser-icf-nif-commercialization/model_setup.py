...
_SHARED_KWARGS = dict(
    ...
    # Canonical value per scoring_framework.md §Plant availability (Pulsed IFE, D-T);
    # policy-driven for cross-concept comparability. Previously 0.80 (unprincipled
    # choice with no published commercial target). No 10 Hz IFE plant has operated;
    # sensitivity sweeps (60–92% range) test concept-specific uncertainty.
    availability=0.75,           # canonical per scoring_framework.md §Plant availability
...
print(f"  avail = 0.75     canonical per scoring_framework.md §Plant availability (Pulsed IFE, D-T); previously 0.80")
...
_annual_energy_gwh = NATIVE_MW * 8760.0 * 0.75 / 1000.0  # GWh/yr at 75% avail (canonical)
...
print(f"  Annual energy at {NATIVE_MW:.0f} MWe / 75% avail: {_annual_energy_gwh:.0f} GWh/yr")
