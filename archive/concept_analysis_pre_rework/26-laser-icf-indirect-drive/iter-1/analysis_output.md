...
**Dominant LCOE uncertainty 4 — Liquid first-wall cost and recirculating power.** For Xcimer's HYLIFE-III design, the FLiBe waterfall simultaneously breeds tritium, absorbs neutrons, protects structural walls, and transfers heat. The FLiBe inventory, pump capital cost, and pump power draw are coupled to chamber volume and rep rate but are not published. Note from cross-concept memory: FLiBe cost data is consistently sparse across IFE analyses; the only baseline is the 1994 Moir HYLIFE-II study in 1994 dollars.
...
**O&M placeholder (per cross-concept memory):** O&M cost breakdown (fixed vs. variable, scheduled maintenance, unplanned outage) is absent from all available sources. This is a consistent gap across IFE analyses. At a minimum, Xcimer's claim of "no structural wall replacement" and Inertia's "3–5 year chamber replacement" span a factor of ~10× difference in a major O&M line item.
...
- **Cost**: No current production at industrial scale. Per cross-concept memory, FLiBe cost data is consistently sparse across IFE analyses; the HYLIFE-II report (Moir 1994) is the only source with cost estimates, and those are in 1994 dollars.
...
|-----------|----------|-------------|-------|
| FLiBe inventory volume and cost | truly-unknown | blocking | No IFE source has this; Moir 1994 is only baseline in 1994$ |
| FLiBe pump capital cost and power draw | truly-unknown | blocking | Recirculating power from pumps not quantified |
| Target factory capital cost | not-yet-sourced | blocking | GEM model exists; Xcimer/GA collaboration may have data |
| Capacity factor | not-yet-sourced | blocking | No rep-rate-coupled availability model in any source |
| O&M fixed and variable costs | not-yet-sourced | important | Consistent gap across all IFE analyses; no published IFE O&M breakdown |
| Tritium processing system capital cost | not-yet-sourced | important | Tritium extraction from FLiBe or liquid Li not costed in any source |
...
| 4 | FLiBe inventory and pump cost — no cost data in any available source | S4, S5 | truly-unknown | blocking | HYLIFE-II (Moir 1994) for 1994$ baseline; HYLIFE-III paper for mass flow rates |
| 5 | Target factory capital cost — GEM bottom-up model exists but not available | S4, S5 | not-yet-sourced | blocking | Ingest LLNL GEM tool documentation; Goodin et al. 2004 |
...
| 9 | O&M breakdown (fixed, variable, scheduled, unplanned) | S5 | not-yet-sourced | important | No published IFE O&M model in available sources |
| 10 | Tritium processing system design and cost | S4, S5 | not-yet-sourced | important | HYLIFE-III design study likely covers FLiBe tritium extraction |
...
10. **Goodin et al. (2004)** — Target cost economic limit: targets must cost <10% of the electricity they produce.
    - Referenced in handwritten exemplar; provides the primary quantitative constraint on target factory economics.
