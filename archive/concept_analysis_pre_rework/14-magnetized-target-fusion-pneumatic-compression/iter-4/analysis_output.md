...
| Capacity factor | 0.75 | scoring_framework.md §Plant availability (canonical, Pulsed MIF D-T); previously 0.8 midpoint of 75–90% analogue band | medium | Canonical project policy value; no published plant-specific availability target exists for this concept. Cross-concept LCOE comparisons within the Pulsed MIF / IFE family use identical central-case availability, making comparisons apples-to-apples on this dimension. Sensitivity sweep tests 0.65–0.92 range. |
...
2. **Capacity factor** (canonical: 0.75 per scoring_framework.md §Plant availability, Pulsed MIF D-T category). The central-case value is policy-driven — no published plant-specific availability target exists for this concept — ensuring apples-to-apples cross-concept LCOE comparisons. The second multiplicative term in the LCOE denominator, and the one most sensitive to the mechanical compression system's unknown maintenance schedule. For a novel piston-and-vortex architecture operating at 1 Hz with no industrial precedent, unplanned downtime scenarios could easily push capacity factor to 50–60%, which would be the dominant LCOE penalty relative to MFE concepts. The model sensitivity sweep (0.65–0.92) characterizes this risk quantitatively.
