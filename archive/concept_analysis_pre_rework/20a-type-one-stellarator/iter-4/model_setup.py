...
This script:
  ...
  2. Derives thermal efficiency from the published power balance (800 MW fusion
     ...
     estimated recirculating → ~415 MWe gross; η_th ≈ 45%).  Central estimate
     of 0.45 is used, consistent with the published operating point.  The
     published bound "> 30%" is a lower floor, not the operating design point.
...
ETA_TH = 0.45               # MEDIUM: thermal conversion efficiency.  Derived from published
                            ...
                            # the published 800 MW / 350 MWe design point.  Published lower
                            # bound "> 30%" is a floor, not the design operating point.
                            # NOTE: prior iterations used 0.40 (steam Rankine conservative);
                            # that implied fusion power ~955 MW, inconsistent with the
                            # published 800 MW — updated per assessment F-1.
...
_SHARED_KWARGS = dict(
    ...
    eta_th=ETA_TH,                  # 0.45 — MEDIUM (consistent with published 800 MW/350 MWe point)
...
print(f"  Thermal eff.: η_th={ETA_TH} (MEDIUM — consistent with published 800 MW/350 MWe operating point)")
...
print(f"  Thermal efficiency η_th:                   {ETA_TH}   [MEDIUM — derived from published 800 MW/350 MWe; consistent operating point]")
