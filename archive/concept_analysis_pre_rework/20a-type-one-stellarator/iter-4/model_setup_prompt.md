...
An existing model from a prior iteration has been copied to `/home/reid/1cfe/fusion-tea/exploration/concept_analysis/analyses/20a-type-one-stellarator/iter-4/model_setup.py`.

**Your task**: Read the existing model at `/home/reid/1cfe/fusion-tea/exploration/concept_analysis/analyses/20a-type-one-stellarator/iter-4/model_setup.py` and apply targeted edits based on the assessment findings below. Use the Edit tool to make changes — do NOT rewrite the file from scratch.
...
- **Recommendation:**
  1. Update the analysis.md Section 5 parameter table availability row to **0.85** with a brief note: "canonical per scoring_framework.md §Plant availability (MCF steady-state, D-T); previously 0.87." Remove or rewrite any "75–90% Araiinejad & Shirvan range" / "midpoint of band" justifications — those are no longer the basis.
...
## Output
Write changes to: `/home/reid/1cfe/fusion-tea/exploration/concept_analysis/analyses/20a-type-one-stellarator/iter-4/model_setup.py`
