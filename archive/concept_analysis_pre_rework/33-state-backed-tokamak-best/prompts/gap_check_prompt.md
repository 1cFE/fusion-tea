...
You are performing a data gap assessment for the fusion concept **State-Backed Tokamak - BEST (D-T)** (Neo Fusion) as preparation for a comprehensive qualitative and quantitative concept analysis.
...
- **Dossier**: `/home/reid/1cfe/fusion-tea/knowledge/concept_research/33-state-backed-tokamak-best/dossier.md`
...
1. **Skim the index below first** to identify which fleet-wide sources are plausibly applicable to **State-Backed Tokamak - BEST (D-T)** (match on confinement family, fuel cycle, plant type, or cost-modeling scope).
