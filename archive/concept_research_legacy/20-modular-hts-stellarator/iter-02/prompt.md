# Fusion Concept Research: Modular HTS Stellarator (D-T)
...
- **Name**: Modular HTS Stellarator (D-T)
- **Company**: Type One Energy, Renaissance Fusion
- **Confinement approach**: Magnetic Confinement
- **Description**: Stellarator using modular HTS magnet assemblies with distinct manufacturing philosophy from Proxima (QI-optimized), Gauss (large-scale W7-X heritage), and Thea (planar coils). Focus on manufacturability and factory production of magnet modules. Steady-state, disruption-free.
...
1. **Start broad**: Search the web for "Type One Energy, Renaissance Fusion fusion technology" and similar queries. Look for the company's website, Wikipedia page, Fusion Industry Association profile, press releases, and investor presentations.
...
Stellarator concepts using modular HTS magnet assemblies with D-T fuel, steady-state operation, and disruption-free confinement. Type One Energy's Infinity Two is a 4-field-period quasi-isodynamic (QI) stellarator at 9 T with published physics basis (6 peer-reviewed papers), targeting 800 MW fusion / 350 MWe with HCPB blanket and Rankine cycle. Renaissance Fusion takes a distinct approach: laser-patterned HTS film deposited on cylindrical surfaces to create 3D stellarator fields, with a liquid Li-LiH metal wall serving as integrated blanket/shield/coolant, targeting 1 GWe at 10 T. Both build on W7-X and HSX heritage but differ fundamentally in magnet manufacturing philosophy and blanket architecture.
...
- **Value**: MFE
- **Confidence**: high
- **Citation**: Baseline CSV; confirmed by Type One and Renaissance Fusion descriptions
- **Notes**: Both are magnetic confinement stellarators with steady-state plasma.

### Confinement Concept
- **Value**: Stellarator (modular)
- **Confidence**: medium
- **Citation**: Baseline CSV; Type One website; Renaissance Fusion website
- **Notes**: Both use modular coil assemblies rather than continuous helical windings. Type One's Infinity Two is specifically quasi-isodynamic (QI), maximum-J optimized. Renaissance Fusion is also QI but with low aspect ratio ("more like cored apples than bicycle tires"). The schema lists `Stellarator (QI)` and `Stellarator (modular)` separately — these concepts are QI-optimized but their primary differentiator from Proxima (concept #09) is the modular manufacturing philosophy. `Stellarator (modular)` is the best fit per schema intent, with QI optimization noted here.

### Fuel
- **Value**: D-T
- **Confidence**: high
- **Citation**: Type One Infinity Two design basis; Renaissance Fusion technology page
- **Notes**: Both explicitly target D-T fuel cycle.

### Primary Heating
- **Value**: RF (ECRH)
- **Confidence**: high
- **Citation**: Type One Infinity Two design basis: "The only envisioned external sources required for Infinity Two operation are pellet injection and electron cyclotron resonance heating (ECRH)"
- **Notes**: ECRH confirmed for Type One. Renaissance Fusion has not specified heating method; ECRH is the universal stellarator heating method per schema guidance. Pellet injection used for fueling (not heating) on Type One.

### Energy Capture
- **Value**: Thermal (steam)
- **Confidence**: high
- **Citation**: Type One: Rankine cycle with reheat, thermal efficiency >30%. Renaissance Fusion: liquid metal heat transfer to steam turbines.
- **Notes**: Type One explicitly uses Rankine cycle. Renaissance Fusion envisions combined Brayton-Rankine cycle with 51% cycle efficiency / 34% net plant efficiency, using liquid metal as working fluid — still thermal conversion to steam turbines. Both fit `Thermal (steam)`.

### Plasma State
- **Value**: Burning
- **Confidence**: high
- **Citation**: Type One: Q = 40, 800 MW fusion power
- **Notes**: Q = 40 is well above the burning plasma threshold (Q >> 5). Renaissance Fusion targets 1 GWe which implies burning plasma, but has not published Q values.

### Magnet Type
- **Value**: HTS (3D stellarator)
- **Confidence**: high
- **Citation**: Type One: modular HTS REBCO coils. Renaissance Fusion: laser-patterned HTS film on cylindrical surfaces.
- **Notes**: Per schema guidance, Renaissance Fusion's laser-patterned HTS film on cylinders is classified as `HTS (3D stellarator)` — the manufacturing method is novel but the functional result is a 3D stellarator field. Type One uses more conventional HTS REBCO wound coils in modular assemblies. Manufacturing distinction is the key differentiator between these two companies and from other stellarator concepts. Type One: 9 T. Renaissance Fusion: 10 T.

### Tritium Breeding
- **Value**: Li blanket (unspecified)
- **Confidence**: medium
- **Citation**: Type One: HCPB (Helium Cooled Pebble Bed), TBR = 1.30. Renaissance Fusion: liquid Li-LiH wall, TBR ~1.60.
- **Notes**: The two companies use very different blanket approaches. Type One uses HCPB — a gas-cooled solid breeder with helium coolant. Renaissance Fusion uses a liquid Li-LiH metal wall (~35 cm thick) that also serves as first wall, shield, and coolant. Neither fits existing schema values perfectly: HCPB is closest to a solid ceramic breeder (not listed), and liquid Li-LiH is closest to `Liquid Li blanket` but with LiH addition. Using `Li blanket (unspecified)` as the umbrella value since the two approaches differ. Schema review may be warranted to add `HCPB` or `Solid ceramic breeder` as a value.

### Neutron Management
- **Value**: Integrated blanket/shield
- **Confidence**: medium
- **Citation**: Type One: HCPB blanket with FLiBe considered where "shielding efficiency is a primary consideration". Renaissance Fusion: liquid metal wall stops 99.99% of neutron energy (company claim); full radial build includes 32 cm Pb + Li-LiH, 54 cm VH2, 1.3 m concrete bioshield.
- **Notes**: Renaissance Fusion's liquid metal wall is explicitly an integrated blanket/shield/first-wall/coolant system. Type One's HCPB is primarily a breeder blanket with separate shielding considerations. `Integrated blanket/shield` is the best fit for the combined concept entry, driven by Renaissance Fusion's approach. Type One may warrant `Heavy shielding (14 MeV)` if treated separately.

### Operation Mode
- **Value**: Steady-state
- **Confidence**: high
- **Citation**: Baseline CSV; Type One: "Steady-state, continuous" with 2-year operating cycle and 30-day planned maintenance outages.
- **Notes**: Inherent stellarator advantage — no plasma current to sustain, no disruptions.
...
- **Confidence**: high
- **Citation**: Type One: modular HTS REBCO coils, 9 T. Renaissance Fusion: laser-patterned HTS film on ~1 m diameter cylinders, 10 T.
- **Notes**: The two companies represent fundamentally different manufacturing approaches to the same functional goal. Type One: conventional HTS tape wound onto modular 3D coil forms, optimized via exascale computing (70,000+ simulations on DOE Frontier). Renaissance Fusion: HTS film deposited on cylindrical surfaces, then laser-patterned to create current paths — no traditional tape winding, simplified to 1D/2D geometry on cylinders.

## Remaining Gaps

- **Tritium Breeding**: Schema vocabulary doesn't perfectly fit either company's approach. HCPB (Type One) and liquid Li-LiH (Renaissance) are distinct technologies lumped under `Li blanket (unspecified)`. A future iteration could search for more detailed blanket engineering papers to refine, or flag for schema review.
- **Neutron Management**: Type One's specific shielding architecture beyond the blanket is not well characterized. The HCPB papers may contain more detail.
- **Energy Capture**: Renaissance Fusion's combined Brayton-Rankine cycle could potentially be a distinct schema value. Current classification as `Thermal (steam)` is adequate but doesn't capture the cycle innovation.
- **Confinement Concept**: The boundary between `Stellarator (modular)` and `Stellarator (QI)` is fuzzy for these concepts — they are both. A second iteration is unlikely to resolve this; it's a schema categorization question.
...
3. Journal of Plasma Physics — Infinity Two physics basis (6 papers, 2025): https://www.cambridge.org/core/journals/journal-of-plasma-physics/collections/physics-basis-of-the-infinity-two-fusion-power-plant
4. ANS Nuclear Newswire coverage: https://www.ans.org/news/2025-04-01/article-6903/
5. Renaissance Fusion — technology page: https://renfusion.eu/technology
6. Renaissance Fusion — blanket paper (J. Nuclear Materials): https://www.sciencedirect.com/science/article/pii/S0022311524003416
...
8. Innovation News Network — Renaissance Fusion profile: https://www.innovationnewsnetwork.com/simplifying-stellarator-technology-to-achieve-fusion-energy/52555/
9. Saved research files:
