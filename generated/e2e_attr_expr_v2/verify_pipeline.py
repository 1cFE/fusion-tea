...
V2: ALL 16 values read from ExitPoint JSON files.
No direct _impl calls needed — Bug 4 (ExitPoint float handler) is fixed.

Validates all 16 ground truth values from the research document:
- 6 FORMULA synthetic module outputs (RootModel[float] via ExitPoint)
- 5 ComponentCostCalc multi-output values (float via ExitPoint)
- 2 AnnualizedCostCalc multi-output values (float via ExitPoint)
- 2 single-output CalcUsage values (RootModel[float] via ExitPoint)
- 1 EXPOSE_PURE (total_capex) verified transitively

Usage:
    PYTHONPATH=generated uv run python generated/e2e_attr_expr_v2/verify_pipeline.py
"""
import json
import sys
...
EXPECTED_VALUES = {
    ...
    # ComponentCostCalc multi-output (float via ExitPoint — Bug 4 fixed)
    ...
    # AnnualizedCostCalc multi-output (float via ExitPoint — Bug 4 fixed)
