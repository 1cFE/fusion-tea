...
### run-study Item 5 — Cold-Pickup Administrator Exercise — COMPLETE 2026-08-20 (audit pending)

`.project/active/run-study-cold-pickup/` (epic RUN-STUDY, Item 5). Design skipped by owner. A fresh `general-purpose` agent read only the git-tracked proof-of-life files (scratch copy; brief at `brief.md`) and wrote `exploration/stellarator_e2e/study/synthesis.md` — 20 unrecoverable facts, citations clean. `gaps.md`: 18 limitations the contract already covers, 1 not-an-absence, 2 gaps. G1 (the record pins every piece of code the verification leans on except the oracle's own source) applied to `record-template.md` at `316fc3a0` by owner decision though not load-bearing; G2 (SKILL.md has no pre-capability allowance) not applied. **Owner question opened, for the Item 6 Align:** the oracle is a dev-process mechanism, not part of what a "study" is in general — revisit whether runbook step 10 (verification, fails closed without an oracle) should be a mandatory study step at all (`plan.md § Revisit`). Next: `/_my_audit`, then Item 6.

### RUN-STUDY epic — Items 1–5 COMPLETE (Items 1–4 orchestrated run 2026-08-19/20; Item 5 2026-08-20)
...
`.project/active/run-study-indicators/` — epic RUN-STUDY, Item 3. Delivered: `scripts/study/indicators.py` and `scripts/study/manifest.py` (both package-agnostic), the three `scripts/study/schemas/*.v1.schema.json` seam files, `exploration/stellarator_e2e/studies/manifest.json` (data-only package catalog), and ten test files under `tests/study/` (134 tests). All seven plan phases committed; the implement session's wall clock expired after its final commit, so only its wrap-up message was lost — `plan.md` Phase 7 notes are complete, including the twelve-invariant sweep. Orchestrator then applied three Item 4 coordination fixes (`620c752d`).

**Audit PASS** (`audit.md`, 2026-08-20): suite green, ruff clean, all twelve design invariants have tests that assert what they claim, the six known-answer expectations reproduce the Item 1 spike field-for-field and were never patched, the manifest is data-only with a live pin, and the mechanical failure messages locate their fault. No should-fix findings. Two notes for later, neither blocking: `design.md:162` still says the pin uses repo-relative paths where the code (correctly, per `plan.md` Phase 1) uses package-relative; and `test_schemas_are_closed` only inspects nodes carrying `"type": "object"` (the closure claim is currently true — verified independently — the test just does not fully guarantee it).

Next: `/_my_close`, then `/_my_pre_pr` once the epic's items ship together.
...
**Epic**: `.project/backlog/epic_run_study_capability.md` (Ready, 6 items, ~6–9d; product-lens CLEAR after F1/F2 fixes). Path: reachability spike → skill/record and indicators in parallel → quality tools + legacy cold pickup → first A/B consumer + new-record cold pickup and policy cutover. **[OWNER-VERBATIM] 2026-08-19:** "I'm fine with this epic owning the A/B proof -- the demo epic is on hold." The demo backlog and Item 5 are amended accordingly.

**Process trail**: input concept `.project/concepts/run-study-skill.md` (amended for coherence, pointers at each open question) · review `.project/concepts/run-study-skill-design-review.md` (two rounds, Closed — C1–C5/M1–M8/m1–m4 all dispositioned; owner accepted directly, waiving a third pass) · epic `.project/backlog/epic_run_study_capability.md` (scope + decomposition owner-approved 2026-08-19; product-lens F1/F2 fixed and gate clear). ADR filing blocked: `.project/adr/` + `adr.sh` absent — decisions and ratification grades recorded in the design's ADR Candidates section instead.
...
3. **WI-026 (pytest baseline re-record, trivial P3) → slotted after WI-029, before Item 5 — EXECUTED 2026-07-25 (fix, not retire).** All 11 stale tests rewritten intent-preserving against the current layout (power balance in `library/analyses/`, alpha inlined, `fuel_type` gone, foundation counts → named-membership asserts). Suite now **30 passed / 13 skipped / 0 failed**, stable twice; one skip un-skipped by the shared path fix; no model files touched. **Standing per-item pytest bar is now green (0 failed / 0 errors)** — the "tally unchanged vs 11/18/14/0" bar retires. Record: `work/completed/20260725_WI-026_pytest-baseline-re-record/`.
...
**Demo board**: criteria 1–2 met · 3 partial (bars ratified; Items 3–4 next) · 4–7 not started · 8 stretch. **Next**: Items 3–4 (account scopes — note: lifecycle Item 10 re-baselined the handshake to the single-pass route; brief against the current harness) ∥ Item 5 (studies — newly unblocked).

### Stellarator MBSE Demo — Item 1 anchor acceptance spec RATIFIED, complete (2026-07-19)

**Status**: **Item 1 COMPLETE** — spec reviewed, revised, and ratified by owner 2026-07-19 ("ratify all, G-8 amend as proposed"). All bars in force; the G-8 successor-bar amendment is recorded at the bar's home (`work/orchestration/stale-basis-recompute.md`, ruling-3 bullet), so Items 3–4 (account scopes) are unblocked alongside Item 2 (constraint execution). Original draft summary: `.project/active/demo-anchor-acceptance-spec/spec.md` drafted — pass/fail bars for both demo anchors (STELLARATOR-DEMO Item 1). Anchor A: per-account handshake bar rel ≤1e-6 (above the ~1e-7 float32 floor), itemized-signed-remainder LCOE form, three test tiers kept distinct, mapping-trap discipline, pin-bump procedure. Anchor B: pre-reveal per-axis bands — structural-similarity checklist, derived quantities same-order-of-magnitude, per-component costs −50%/+100% (AACE Class 5); C220107 and sizing-axis exclusions; prior-leak bar. G-8 premise conflict surfaced (successor-bar amendment, [OWNER DECISION PENDING]). Every number [AGENT-PROPOSED]; consolidated ratification table at the spec's end. Independent spec review returned Revise (3 must-fix: |rel dev| magnitude bar, single [1/3,3] ratio test for B-3, sizing-reallocation de-graded to pending/B-5a); all fixes applied and diff-verified by the orchestrator (recorded per the minor-fix verification rule — no reviewer re-run). Next: owner ratifies the package (11 lines: A-2/A-4/A-6, B-2/B-3/B-4/B-5a/B-6/B-7/B-8, G-8).
