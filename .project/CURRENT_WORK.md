...
1. **Stellarator model migration** — `.project/active/stellarator-model-migration/` (spec → design → plan done; **plan Phases 1–5 complete 2026-08-21** (Phases 1–3 `89f78130`, Phase 4 `8f3b510c`): model repaired and ledgered (`models/stellarator_migration_ledger.md`; a fifth refusal class, codegen unit-scrape byte/char drift, ledger F3, handled as Class B and flagged for owner review), package regenerated and sealed at 2.0.0 on stock teax, baseline/948-grid/19-sweep byte-identical to the before-record, preflight 6/6 and verify with CAS27 compared (`AFTER_MIGRATION_RECORD.md`), MFE models promoted into `models/` with the family spine (`tests/model_families.py`, `tests/models/test_model_family_spines.py`), era adapter/glue/tests deleted whole with a retired-identifier sweep; **owner call pending**: `handshake_1costingfe.py` carries its own glue and cannot run on the 2.0.0 package (see plan Phase 5 notes); next is Phase 6, close the branch gates). D-5 rename (mechanized via sysml-codegen `scripts/make_d5_variant.py --root`), the six scalar-function rewrites (`[OWNER]` rewrite now / file upstream / revert later), unit-comment harmonization, the four positional-binding fixes; iterate `sysml-codegen generate` until it seals at `runtime_contract_version 2.0.0`. Then regenerate, delete `era_adapter.py` and the glue, re-pin `studies/manifest.json`, promote the MFE models from the staged twin into `models/`, reshape the spine test per design family, and prove the "after" bars against `exploration/stellarator_e2e/studies/BEFORE_MIGRATION_RECORD.md`. Unknown: what refusals lie past the three found so far. Background: `.project/research/20260820-221835_stellarator-demo-reconciliation-plan.md` § 3–5.
