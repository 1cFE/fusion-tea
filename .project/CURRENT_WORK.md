...
- **WI-035 closed** 2026-09-01 (`pm close-item`), archived to `work/completed/20260901_WI-035_magnet-closure/`; backlog row `completed: 2026-09-01`. `tests/models` re-run after the move: 48 passed / 13 skipped. Trail citations to the old `work/active/` path are redirected by a trail amendment, not edited.
- **Flagged, deliberately unfixed:** `'Magnet Capital'`'s doc comment cites the now-moved `work/active/WI-035_magnet-closure/design.md` (`models/library/analyses/mfe_magnet_cost.sysml:146`, its twin, and the generated echoes). An MR-4 defect, but correcting it moves the semantic fingerprint and invalidates pin `287479a3…`. It rides the next regeneration done for a real reason — WI-036 is the obvious carrier.
- Merge/push owner-held; `magnet-ab#4` stays behind the Rung C gate. Baseline: LCOE 304.482, total capital $14.574B, seven verdicts satisfied.
