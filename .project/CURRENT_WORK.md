...
1. **Stellarator model migration — implementation complete 2026-08-21**, awaiting `/_my_audit` then `/_my_pre_pr` (`.project/active/stellarator-model-migration/`; commits `89f78130`, `8f3b510c`, `1050743b`, + Phase 6). The MFE model generates clean on the pinned codegen and seals at `runtime_contract_version 2.0.0`; every edit ledgered (`models/stellarator_migration_ledger.md`, A 365 / B 141 / C 0); baseline, 948-grid and 19-sweep byte-identical to the before-record; preflight 6/6 and `verify.py` with CAS27 compared and nothing undisclosed (`exploration/stellarator_e2e/studies/AFTER_MIGRATION_RECORD.md`); MFE models promoted into `models/` with the family spine; era adapter, glue and tests deleted whole. **Owner to-dos:** commit the three upstream rows written in sysml-codegen's `BACKLOG.md`; decide the `handshake_1costingfe.py` BACKLOG row when the demo epic resumes.
2. **RUN-STUDY Item 6** (first A/B consumer + policy cutover) on the stock route — unblocked by 1 (route: `exploration/stellarator_e2e/studies/study_route.py`). Owner Align first (policy disposition; whether oracle verification is mandatory).
3. ~~Two upstream filings to sysml-codegen~~ — written 2026-08-21 by item 1 (plus a third, the unit-scrape defect); they sit uncommitted in `/home/reid/1cfe/sysml-codegen/.project/backlog/BACKLOG.md` for the owner's commit.
...
- **Package state**: regenerated 2026-08-21 on the pinned codegen `8a758e92`, `runtime_contract_version 2.0.0`, runs on stock teax (`744745f`) with no adapter; the era worktree at `/home/reid/1cfe/teax-v1-era` is no longer used by anything (deleting it is a tidy-pass item).
- **MFE models live in `models/`** since 2026-08-21, with `exploration/stellarator_e2e/models/` as the byte-identical twin (same convention as the IFE family); `tests/models/test_model_family_spines.py` generates each family from its own canonical subset.
