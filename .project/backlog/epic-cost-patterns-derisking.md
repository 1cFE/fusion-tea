...
### Stage 2b: Cost Evaluation Script

**Goal**: Prove the pattern is not just parseable but **evaluable** by generating actual cost values from the model.

**Rationale**: Stage 2's `expected_output.csv` is hand-calculated. Without automated evaluation, we can't catch calculation errors or validate that the model structure actually supports cost computation. This script serves as a prototype for what sysml-codegen needs to do.

**Deliverables**:
1. `models/tests/coffee_maker/generate_costs.py` that:
   - Extracts parameter bindings from PartUsages (`:>> power_rating = 1000.0`)
   - Extracts calc def formulas and default values from library
   - Evaluates the calculations with bound parameters
   - Handles multiplicity (heater[2] → 2 × unit cost)
   - Aggregates assembly rollups (children + allocation)
   - Outputs `actual_output.csv` in same schema as `expected_output.csv`
2. Comparison logic that diffs `actual_output.csv` vs `expected_output.csv`

**Key capabilities validated**:
| Capability | What It Proves |
|------------|----------------|
| Parameter extraction | Can resolve `:>>` bindings through redefinition chain |
| Formula evaluation | Can interpret calc def expressions with bound inputs |
| Multiplicity handling | Can multiply unit costs by array size |
| Assembly aggregation | Can sum child costs + allocation at each level |

**Exit criteria**: `generate_costs.py` produces output matching `expected_output.csv` (within floating-point tolerance).
