...
### Item 7: Shared `model_setup` Helpers + Validator Rework — ✅ COMPLETE (2026-05-31, Item 7 scope)
...
- [x] Validators enforce design-point coherence: `P_native` agrees three ways — the design-point table row, the `analysis.md` Design Point block, and `model_setup.py` — and each shared override's `provenance` label matches between `analysis.md` and `model_setup.py`. (Phase 0 self-review surfaced both as real, AST-checkable drift modes — operator picked `P_native=400` against an analysis that said 233, and mislabeled `derived` overrides as `direct`.) *(`validate_design_point_coherence`; two legs live, the `analysis.md` leg is `None`-gated against a provisional scraper until Item 8.)*
- [ ] Loop runs cleanly on a dry-run without the dropped validators. *Deferred to Item 8 (design Decision 1 / FR-9): Item 7 removes nothing and makes zero control-flow changes, so this dry-run cannot be discharged here.*

**Implementation summary (2026-05-31):** Helper module + four structural validators (two output-gate, two coherence) shipped as **pure library code — zero live-loop wiring** (verified: no consumers outside defs/tests), per the sharpened-hybrid scope. Test suites green (`test_model_setup_helpers.py` 13 + `test_validators.py` additions; 111 passed total; retained validators untouched, 0 deletions). Re-pinned oracle reproduced: native 174.5 / 1 GWe bare 137.2 / all-on 584.5 $/MWh. Audited against the plan 2026-05-31 — no critical/major findings; two deferred criteria (regex removal, FR-9 dry-run) carried to Item 8. Handoff: `signal_contract.md`. Spec: `.project/active/concept-rework-helpers-validators/`.
...
**Success Criteria**:
- [x] Dry-run on one concept produces an `analysis.md` containing the Design Point block and a `model_setup.py` matching the four-step shape.
- [x] `analysis_v2.md` prompt takes the design-point *selection* (named plant, maturity tier, `P_native`, primary sources) as a fixed input from the design-point table / frontmatter — it does **not** choose or re-derive the plant. Its job is the quantitative *extraction* for that fixed plant.
- [x] `analysis.md`'s Design Point block carries the *full* specification of the target design point — every field downstream needs (name, maturity tier, `P_native`, geometry/physics/performance values, override provenance per account) is explicit and source-cited. The selection fields match the design-point table row; the geometry/physics/performance values are the extracted quantitative description. Nothing required downstream is left implicit.
- [x] `model_setup_costingfe.md` prompt instructs the agent to **start by identifying the target design point from `analysis.md`** — reading `P_native`, the spec kwargs, and override provenance labels directly from the Design Point block rather than re-deriving them from the dossier.
- [x] `analysis_v2.md` prompt pins override `account` identifiers to the canonical 1costingFE namespace (`C220101`, `C220103`, `CAS27`, etc.) by injecting the account-code list as a schema. (Phase 0 finding: LLM defaulted to `CAS22.1.3`-style codes that would silently miss in the `cost_overrides` dict.)
...
- [x] `model_setup_costingfe.md` prompt forbids re-passing **financial / operating-economics** defaults that apply uniformly across all fusion concepts — `availability`, `lifetime_yr`, `interest_rate`, `inflation_rate`, and the like. These are not per-concept analyst choices; the library defaults are authoritative and MUST NOT appear in `spec`.
- [x] For **physics-characteristic** parameters that 1costingFE defaults *per `ConfinementConcept` archetype* (`eta_th`, `eta_de`, …), the prompt allows a `spec` override only when the concept genuinely deviates from *its own archetype's* default on a physics basis (e.g. a direct-conversion concept whose archetype default mis-represents its conversion path). A company-published "optimistic" value MUST NOT be passed as a spec override. Discipline test: "does this concept differ from its archetype's library default on the physics?" — if yes, override with provenance; if it's just "we expect to do better," do not. (This refines the earlier blanket rule that listed `eta_th`/`eta_de` as uniformly forbidden — they are archetype-keyed physics, not uniform economics; see Item 8 spec FR-13.)
- [x] Sweep / what-if text output remains allowed in `model_output.txt`; `result` and `result_1gw` remain the standardized baseline consumed by the explorer.
- [x] Assess/review output is parse-robust under the new validators (no reliance on the dropped regex paths).
- [x] Quality-standards doc reflects the new discipline (no re-passing of library defaults; every parameter describes the design point).

**Implementation summary (2026-05-31):** Shipped as the FR-29 atomic swap. Phase 1 — `lib/canonical_accounts.py` (16-enum per-archetype account schema + `validate_against_library()` + `FIT_GRADE_OVERRIDE_BAND`). Phase 2 — five parser internals rewritten to line-anchored scanning, signal_contract shapes preserved. Phase 3 — `lib/prompt_blocks.py` + loop wiring (strict helper-only contract gate on all three model-setup branches, `coherence_flags` into assess). Phase 4 — all 10 in-scope templates rewritten + new `config/account_walkthrough.md`; four FR-28 regex constants deleted and their last users rewired onto the line-anchored helpers (discharges Item 7's **FR-9**); `defaults_path`/`mapping_notes` dropped; `fit_grade_band` wired into assess. Phase 5 — ARC (concept 01) end-to-end pilot on **Opus** passed **every** spec AC: conforming Design Point block + §5b six-field override YAML with canonical codes, four-step helper-form `model_setup.py` accepted by the strict contract validator, `eta_th` correctly left to the library (FR-13), `result_1gw` at exactly 1000 MWe, override-toggle 160.7↔543.7 $/MWh, all four parsers signal_contract-correct, explorer extracts at 1 GWe with no fallback. One fold-back fix: `claude.py::_check_interface` regex now recognizes the helper tuple-unpack. Full scripts suite 405 passed / 5 skipped / 4 pre-existing unrelated `test_concepts_v2.py` data-state failures. Spec: `.project/active/concept-rework-prompt-templates/` (spec/design/plan/pilot_report). One tuning candidate noted for the Item 10 pilot: `assessment.md`/`quality_standards.md` could signal more strongly that an aspirational efficiency is not a finding. **[The four-step helper-form prompts shipped here were revised to the three-forward contract (mandatory `generic`; `native, result_1gw = run_native_and_1gw(...)`) by `.project/active/concept-rework-three-forward-contract/`.]**
