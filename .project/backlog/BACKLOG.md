...
| Structural refactor of 08 + 31 to use `pulsed_conversion=INDUCTIVE_DEC` | P1 | Concepts 08 (Helion FRC) and 31 (laser ICF OEC) currently encode overall direct/blended conversion efficiency in `eta_th` as a workaround — exactly the bug shape issue #30 targets. The `eta_th-double-count-fix` work item marked their `eta_th` lines `# DEVIATION:` to keep them out of the value-swap, but the underlying structure remains: 08 stuffs ~85% EM recovery into `eta_th` with `f_dec=0`; 31 defines `ETA_TH_COMBINED = F_NEUTRON*ETA_TH + F_CHARGED*ETA_DEC` and feeds the blend into `eta_th`. Proper structural fix: set `pulsed_conversion=INDUCTIVE_DEC` (08) or wire `f_dec` + `eta_de` correctly (31), drop the `eta_th` hacks. Needs domain analysis per concept (verify costingfe's `pulsed_dec_forward` produces sensible LCOE for each). After refactor: rerun cost models, regenerate synthesis, remove the temporary DEVIATIONs added in PR for issue #30. Likely 1-2 days per concept including domain re-verification. Motivated by `.project/active/eta_th-double-count-fix/` Phase 4 audit. |
