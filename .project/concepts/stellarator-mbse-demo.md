...
ARIES-CS full papers (systems-optimization and overview papers; OSTI IDs already captured in-repo) are ingested into a quarantined location before modeling starts. Every model-development and research prompt blocklists ARIES-CS until stages 2–3 conclude. At comparison time, the model is evaluated at the ARIES-CS design point and compared per criterion 4. The blind is clean-room: demo model-development sessions may read neither the held-out papers nor the existing repo artifacts that carry ARIES-CS-derived content (the 09 pipeline analysis/synthesis/review, the abstract stubs, the Helios comparison extraction) — the spec review of the first work item found the existing exposure was materially larger than this concept first recorded (detailed comparisons and a superseded calibration, not one quote), and the owner resolved it clean-room (2026-07-12; `spec-review.md` L1-1). The protocol, including the full contamination inventory, is documented so the public claim is defensible. One limit is inherent and stated up front: the blocklist controls prompts and sources, not model priors — the agents' training data includes ARIES-CS publications. The public claim is therefore scoped to "no ARIES-CS material in context or sources during model development," and the write-up says so explicitly.
