...
Every fusion concept must navigate this journey. But different concepts face their material risks at different stages. A compact HTS tokamak's biggest risks live at Stage 2 and Stage 3: can a $12B+ first plant get financed, can REBCO and tritium supply chains scale, can factory modularity deliver real learning at Stage 4? A beam-driven aneutronic FRC carries a long Stage-1 timeline note (the commercial plasma regime has never been measured) and a Stage-2 supply-class gap (no commercial 250 keV NBI driver exists at fusion scale). A large stellarator might pass Stages 1–2 but die at Stage 3: can anyone replicate a $10B one-of-a-kind facility?
...
Stage 1 — "does the physics produce net energy?" — is not treated as a selection gate, a discount, or a failure mode. We lack the expertise to adjudicate competing physics claims, and we are not in the business of picking concepts on physics. What we record per concept is a **timeline note**: current maturity (Q achieved, integrated-system demonstrations), distance to a Stage-2-ready commercial regime, and the supporting context (physics-paradigm co-development, scientific heritage, workforce depth). A compact HTS tokamak whose paradigm draws on ITER, JET, JT-60, and multiple private programs is on a shorter timeline than a beam-driven FRC whose commercial regime has never been measured. The timeline note rides alongside the Stage 2–4 assessment as descriptive context — it does not feed any tag, score, or selection rule.
...
- **Component supply**: Do critical components have an existing external supply, or must the concept build its own? CFS vertically integrated REBCO tape manufacturing; concepts using commodity copper coils inherit an existing supply chain. [Wang et al. 2022] A second supply shape matters here: some concepts need a *supply class that doesn't exist at commercial scale at all* — TAE's 250 keV proton NBI driver, Zap's pulsed-power supply at fleet scale. This *capability-class gap* is structurally distinct from the quantity-scaling gap of REBCO (where the supply class exists and external markets are pulling cost down). Capability-class gaps cannot be addressed by riding someone else's learning curve, because no one else's curve runs through the right operating regime.
