...
- **[OWNER 2026-09-01] The confinement bullet is deleted — confinement was never ruled out of scope.** The owner, in session: "I'm not sure when I ever ruled anything out of scope"; "nothing is sacred here." Provenance of the error, recorded once so it is not re-derived: the phrase began as an agent's scope note in a model doc comment (`models/library/analyses/mfe_plasma_scaling.sysml:163` — "the machine -> profile closure (ISS04 confinement solve) is out of scope (Rung C)"), meaning only that the work item then being built did not cover it. It was carried here as `[AGENT]`, then hardened into an owner-grade reserved gate in `work/orchestration/goals/magnet-closure/goal.md` § Reserved gates, where two rounds recorded work as "blocked on the standing Rung C owner gate" and a discovery finding sat undischarged behind it. **Confinement is open work, groundable as a goal like any other.** No agent may cite a Rung C gate as authority.
- **[OWNER 2026-09-01] The WI-033 `p_pump` ruling is scoped to what it decided, and is not a standing bar on modelling the coolant loop.** The owner, in session: the ruling was made in a context where "we had a hard-coded WRONG value, and I approved changing it to a hard-coded BETTER value." What it rejected was expressing `p_pump` as a fixed fraction of thermal power — reason of record: that asserts a linearity across swept (R, a) that no source establishes, and retires a study lever `[OWNER-VERBATIM 2026-08-28, work/completed/20260828_WI-033_p-pump-rebase/spec.md:22]`. A goal that computes pumping power from a real loop — flow, pressure drop, parasitics — is not the fraction form and needs no reopening.
- **Two homes carry the stale wording and are deliberately not edited here.** (1) The depth rubric's Row 1 gate note (`.project/active/demo-depth-rubric/rubric.md@dc0f0b6d`) repeats the Rung C framing; the rubric is the frozen yardstick both gradings cite, and its own versioning policy says a change produces a new version — owner-gated, open question 6. It is corrected when the rubric is next versioned, not before. (2) The originating doc comment at `mfe_plasma_scaling.sysml:163` sits under `models/`, so editing it moves the semantic fingerprint and invalidates pin `287479a3...`; it rides the next regeneration done for a real reason. Until both land, this correction is the authority and they are stale text.
