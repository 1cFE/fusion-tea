...
**`override_reference_mw` is not used.** Because each module runs at native power, a company-quoted component cost is already valid per module; the library multiplies per-module overrides by `n_mod` automatically. This removes the `override_reference_mw` scaling path (and the three-way invocation inconsistency it produced across the current files) entirely.
...
- `model_setup_costingfe.md` prompt — flip the discipline: omit library defaults, pass only deliberate overrides, use the toggleable override registry, always produce `result_1gw` via the `net_electric_mw=1000` + `n_mod=1000/P_native` replication pattern (no `override_reference_mw`)
...
- 1 GWe NOAK is the standardized cost projection basis; achieved via `net_electric_mw=1000` + `n_mod=1000/P_native` (shared plant scaled by output power; reactor island replicated by module count, each module held at native operating point). `override_reference_mw` is not used. Depends on relaxing `1costingFE` `n_mod` to non-integer (one-line library change)
