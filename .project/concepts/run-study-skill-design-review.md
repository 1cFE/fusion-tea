...
- `.project/completed/20260821_demo-proof-of-life/plan.md` and `exploration/stellarator_e2e/study/run_design_search.py` prove one disciplined route. They also show its exact temporary dependencies: a loader exception, two modified sealed artifacts, injected fields, explicit fan-out groups, and `PreparedListStrategy`.
- `modeling_project/STUDY_POLICY.md` (at review time `.project/active/demo-study-parameterization-policy/policy.md`; moved 2026-08-21) requires causal axes, complete fan-out, stock study execution, and honest model-gap reporting. It remains a draft under a temporary `.project/active/` path.
