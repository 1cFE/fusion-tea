...
| `010` | [The independent oracle mirrors the model's audited held bindings — and is demo-scoped, not permanent](010-oracle-mirrors-audited-bindings.md) | active | `[OWNER 2026-08-29]` carry ruling; `[OWNER 2026-08-21]` demo scope; `[AGENT]` rationale | 2026-08-30 |
...
`ADR-00X` is also used loosely in at least one committed modeling artifact — `work/completed/20260303_WI-008_hif-concept-instantiation/design.md:49-107` writes "ADR-002" for what is a modeling calc-placement decision, i.e. an `AD-XXX`. Read an `ADR-NNN` citation against the directory it came from: inside `work/` and `modeling_project/` it may mean an `AD-XXX`; only a citation resolving to a file in this directory is a register id.
