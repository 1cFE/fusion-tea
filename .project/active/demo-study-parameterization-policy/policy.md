# Study Parameterization & Constraint Policy — Stellarator MBSE Demo

**Status**: Draft — for owner ratification at the Item 5 Align
**Created**: 2026-08-02
**Governing frame**: `.project/concepts/stellarator-mbse-demo.md` (criteria 5–6); tracking home `.project/backlog/epic_stellarator_mbse_demo.md` Item 5.
...
## 3. Constraint triage — [AGENT] (shape approved in conversation 2026-08-02; ratify at Align)
...
## 4. Distributed cycles: the resolution ladder — [AGENT] (pressure-test at Align)
...
## 6. teax machinery in scope for Item 5

Item 5's studies are written against exactly this list (the constructs proven in the IFE acceptance harness, `exploration/ife_e2e/study/run_viability_study.py`, main checkout), and nothing else:
...
- **H1 (parameterization holds)**: with levers-only axes (§2) and the current inequality-only constraint set, verdict columns are informative — feasible fraction strictly within 5–95% per study, and every violated point attributes to at least one named constraint. *Falsified* by a near-empty or near-full feasible region despite compliant axes — itself a first-order demo finding, reported per the concept's honesty bar.
