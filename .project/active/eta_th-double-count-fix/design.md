...
- **Score Explorer regression check.** No code change is expected; the Energy Capture column is unchanged. Sanity check: open `docs/index.html` (or https://scoring.1cf.energy/) in a browser after the runs, confirm Energy Capture renders, confirm the 8 affected concepts show shifted LCOE / composite scores.
