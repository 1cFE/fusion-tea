...
- Automation pipeline complete and proven (`scripts/zotero_ingest.py`)
- Zotero write access confirmed (API key updated 2026-02-09)
- Zotero group library (5428393) has items but most lack `new` tag for pipeline pickup
...
1. **Source selection and Zotero curation** — Add 5 specific fusion documents to Zotero, attach PDFs, tag `new`
2. **Batch ingestion** — Run `scripts/zotero_ingest.py` to process all `new`-tagged items
...
6. **Zotero tag management** — Ensure all processed items are tagged `extracted`; researched items tagged `researched`
...
- **Duplicate detection**: If a selected document is already in Zotero (e.g., the Helios paper at key `7E42ICWG`), tag it `new` rather than re-adding.
...
2. PDF attached as a child item and synced to Zotero Storage
3. Tagged `new` to enter the ingestion pipeline queue

**FR-3: Batch Ingestion**

Run `uv run python scripts/zotero_ingest.py` to process all `new`-tagged items. The script MUST:
...
- Tag each successfully processed item as `extracted` in Zotero
...
**FR-6: Commit and Size Check**

All extracted sources, SOURCE_INDEX.md updates, and KNOWLEDGE.md updates MUST be committed to git. After committing, run `git count-objects -vH` and verify total repo size remains under 100MB.
...
- [ ] All 5 new Zotero items are tagged `extracted`
...
- **Automation Script:** `scripts/zotero_ingest.py`
