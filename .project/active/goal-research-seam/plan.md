...
**Completed:** all eight audit findings addressed. F1: per-attempt staging dirs removed in `finally`; stale sweep lock-held and age-thresholded (4x CAPTURE_TIMEOUT_S); design D7 amended with the dated correction; `test_staging_concurrency.py`. F2: `close` reads `queued[]` from triage-logged failures (`log --failure`, disposition `queued|closed`, default queued) as well as receipts; no negative written when queue-worthy candidates exist; design D13 table corrected with the R-B9 argument; `test_triage_queue.py` reproduces the guide's paywall sequence -> OPERATOR_QUEUE. F3: all seven path constants pointed at the temp tree; `test_hermetic_paths.py`. F4 [orchestrator ruling]: `knowledge/research/requests/` contents are committed evidence; policy stated in the guide and command close step. F5 [orchestrator ruling]: guide corrected to required `--title` (settled D10); code unchanged. F6: repo root derived from module location; test covers a differently-named checkout. F7: `raw_artifact_sha256` computed once. F8: `--triage` values listed; offline-invocation section added.
**Verification:** fix work was authored by the implement session (its task was killed before it committed; the kill's source is not recorded — orchestrator initially misattributed it to the owner, corrected 2026-08-26); the orchestrator verified the dirty tree — `tests/research/` + `tests/orchestration/` 150 passed (slow included), `verify` 0 faults / 3 legacy — reviewed every fix diff against the dispositions, and committed. SC7 marked on `test_triage_queue.py` + `test_return_contract.py`; SC9 marked on the auditor's walk plus orchestrator-verified fixes to the three guide defects that caused its refusal (F2 example, F5 title line, F8 triage values/offline note).
