...
- **D7. Stage, then commit in a fixed order, under a lock, with a sweep.** Detail and the failure ladder are in Architecture. *Rejected: compensating-delete only* (the `add-source` referent protects one directory and has no index, manifest or raw copy to undo). *Rejected: a write-ahead journal with crash recovery* (no evidence a mid-commit hard kill has happened; the epic's hardening rule bars building it) — and that rejection is paid for by `verify`, which is therefore first-build scope, not deferred (D14).
...
**Vocabulary mapping** (D13). `register` returns one of `registered`, `duplicate`, `holdout_hit`, `capture_failed`, `precondition_failed`, `limit_reached`. `close` aggregates the run's receipts into one seam class, in this precedence:

| Run's receipts | Seam class | Why |
|---|---|---|
...
| otherwise, ≥1 `holdout_hit` / `capture_failed` / candidate-scoped `precondition_failed` | `OPERATOR_QUEUE` | a named candidate is blocked on something a human must resolve |
...
A run-scoped fault after something already registered stays `REGISTERED` and carries the fault in the return, because what is on disk is established fact (R-B9). The keeper/rejected disposition in the second row is the triage decision R-C5 already requires to be recorded; it is not a new judgment.
...
- Staging is swept at the start of every `register`, so unscanned captured content never persists across invocations.
