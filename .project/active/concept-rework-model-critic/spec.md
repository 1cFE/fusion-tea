...
- **Non-goal:** freeform concepts (`fit_grade=None` or no-`P_native` route) — those don't have the four-step `model_setup.py` shape the reasoning spine assumes.
...
- Freeform concepts — the reasoning spine assumes the costingfe four-step `model_setup.py` shape; freeform concepts don't have that.
