...
**2026-08-25 — orchestrator dispositions, incorporated into `spec.md` by the spec-agent session.**

- **L3-1 (homes list) — FIXED.** Authority: owner for the homes list; `[AGENT]` for the reconciliation. The append-vs-update ambiguity is settled as an execution detail rather than deferred: **a disposition update is delivered as an appended row keyed `<study-id>#<n>`**, which satisfies the design's "appends disposition rows" (`design.md:170`) and the epic criterion's "receives a joined disposition update" (`epic:53`) with one mechanism, and keeps the log append-only with first-sighting rows never edited. Recorded in the spec as `[AGENT]` reconciliation of two owner texts, challengeable by re-deriving against both. The home list grows to five: `runbook.md:221`, `runbook.md:270`, `DISCOVERY_LOG.md:3` (sole-writer), `DISCOVERY_LOG.md:3` (one row per finding), and `runbook.md:290-292` plus the schema table's `Disposition`/`Home` cells. The one-row-per-finding wording is amended in scope — one row per finding *sighting*, plus joined disposition rows; "never a second copy of the finding's account" is untouched.
- **L1-1 (digest provenance) — FIXED.** Reviewer is right: "authority digests" is `design.md:186`'s wording, not the owner's P2/M4 words. The reconciliation is regraded `[INFERRED]` at its home in § Architecture records, and the § Non-Goals echoes now say "digests" flat with the narrowing marked as the inference it is. The digest requirement itself stays `[OWNER]`. The surface-don't-resolve-silently duty is kept and restated in both places.
- **L3-2 (live test) — FIXED.** `tests/study/test_records.py:41` re-verified: `in_log` is a set comprehension, so a second row under an existing id collapses and passes. Recorded in the spec as a `[HARD]` existing-system constraint with both consequences named, plus the requirement that the shipped consistency tests cover the joined-row shape deliberately so the accident becomes a stated guarantee. Test shape and home stay design's.
- **L3-3 (SC1 gap) — FIXED.** New `[INHERITED]` requirement in § Architecture records: `GOAL_RUNBOOK.md` and the affected project guidance cite the filed records.
- **L1-2 (`[REFERENT]` force) — DISPOSITIONED, no force change.** `[REFERENT]` stands — it is the bar to match. Its provenance is regraded to `[INHERITED: epic Item 1 Required Reading — "proven prose referent"]`: owner-ratified epic text (`epic:120`), not an owner-verbatim statement. The product-lens ledger's "authority: owner" on this marking was overstated; the spec now carries the accurate grade.
- **L2-1 (sizing) — NO SPEC CHANGE.** The 1.5-day estimate is epic-level `[AGENT]`, ratified by the owner with the rest of the decomposition. Overrun is acceptable; the estimate is not load-bearing on scope. The reviewer's suggested cut (records-and-amendments first, runbook-plus-templates second) is available to `/_my_plan` as a sequencing option, not a spec requirement.
- **L2-3 (worked checkpoint example) — NO SPEC CHANGE.** That the checkpoint is contracted here and first exercised in Item 5 is the epic's own owner-dispositioned placement (`epic-plan-F2`, `[OWNER 2026-08-25]`). Not re-opened. The existing requirement that this item ship the smallest templates needed already lets design include a checkpoint template if it judges one necessary.
- **L3-4 — FIXED.** The requirement now says this item edits the shared runbook first and must leave room for Phase 4's three pending sentences, rather than describing them as already present.
- **L3-5 — FIXED.** The contract-test line now marks its example as fixing *altitude* only and points to § Open Questions for shape.
- **L4-1 — FIXED.** Added as an `[INFERRED]` same-pass fix: `DISCOVERY_LOG.md:3` attributes its writer rule to `runbook.md § DISCOVERY_LOG.md`, which carries no writer rule; the sentence is at `runbook.md:221`.
- **L5-1 — FIXED.** § Lean artifact contract now opens with a short frame — what `goal.md` fixes, what `trail.md` accumulates, what `learnings.md` carries — and one sentence on the round, before the enumeration. The bullets are unchanged.
- **L2-2 — PARTIALLY TAKEN (not on the must-fix list; cheap and clearly right).** The ADR-home open question now names the existing prior art (`modeling_project/ARCHITECTURE.md`'s `AD-XXX` records; the stray `exploration/phase_1a/ADR-001_*`) and asks design to weigh extending it and to say whether orchestration decisions are a deliberately separate register. The Item 2 filing dependency is promoted from a coordination note to its own open question, flagged as live scheduling.
