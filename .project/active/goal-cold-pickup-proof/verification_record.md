...
|---|---|---|---|---|
| 1 | Cold grounding | 01, 02 | `work/orchestration/goals/cryo-volume-basis/goal.md` at `Status: grounded` (commit `c3e47e11`, C-GROUND): all template headings, 22 sha-pinned citations, four limits restated with numbers, operator content `[AGENT]`. The session found the question's premise gap itself (turn 1's final message, `sessions/01-grounding/transcript.jsonl`). | **MET** |
...
| 6 | Judgment replays via five-field decisions | 09–11 | Every recorded goal-level decision carries all five fields (resumer ×2, continuation ×5, incl. the SV-mint refusal and the rejected fudge factor). The killed session's in-flight decisions are absent by construction — the predicted thin T-001 record, recorded in § Failures below, not backfilled. | **MET**, with the predicted shortfall recorded |
...
6. **No first-sighting edit, no minted id** — `git diff 31f9eb0b..HEAD -- exploration/stellarator_e2e/studies/DISCOVERY_LOG.md` is three pure row additions under the existing id; sighting byte-untouched; join test 7 passed. ✓
