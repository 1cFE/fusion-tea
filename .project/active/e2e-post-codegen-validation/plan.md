...
- [x] Native hierarchy: created simple pipeline runner (like e2e_attr_expr_v3 pattern). No ComponentCostEvaluator hybrid merge needed.
- [x] ComponentCostEvaluator fully replaced by 20 native aggregation modules + 1 computed attribute module
...
```
- [x] All `:>>` literal values present (wattage=400, efficiency=0.21, etc.) — 13 values in system_design.json
- [x] All multiplicity counts present (module_count=20, inverter_count=4, pack_count=8)
...
```
- [x] Pipeline executes without errors — all 36 modules executed
- [x] All modules execute in correct topological order
...
- [x] Output directory prefix: `solar-battery-v3-results-*`
...
- [x] **PASS**: 7/7 values pass (total_capex=41205.0, annual_energy_mwh=11.14272, annual_om=160.0, fuel=0.0, crf=0.0709525, annualized_capital=2923.596, lcoe=288.676)
...
```
- [x] All `_impl.py` files remain `AUTO_IMPLEMENTED` (not downgraded to stubs) — 36/36 AUTO_IMPLEMENTED after smart-regen
- [x] **REGRESSION**: Smart-regen overwrites Bug 9/10/11 workarounds in pipeline.yaml, system_design.py, and permittingcostcalc_output.py. Only `__init__.py` (Bug 8) is preserved by `--preserve-handwritten`. Pipeline will NOT execute after smart-regen without re-applying workarounds.
...
```
- [x] Core tests: 48 passed, 1 skipped
...
**Completed:** 2026-02-16
**Actual Changes:**
- Created `generated/solar_battery_v3/run_pipeline.py` — clean pipeline runner using native hierarchy modules, no ComponentCostEvaluator hybrid merge needed
- Created `generated/solar_battery_v3/verify_pipeline.py` — 7 ground truth value checker
- Modified `generated/solar_battery_v3/__init__.py` — rewrote with aliased imports + module_type_override map (Bug 8 workaround)
- Modified `generated/solar_battery_v3/pipelines/pipeline.yaml` — added `system_design.` prefix to 28 input channels (Bug 9) + changed 12 `int` → `float` types (Bug 10)
- Modified `generated/solar_battery_v3/schemas/permittingcostcalc_output.py` — removed `default=0.0` from 4 output fields (Bug 11)
- Modified `generated/solar_battery_v3/schemas/system_design.py` — changed 3 multiplicity count fields from `int` → `float`

**Results:**
- [x] 5.1 Hybrid merge: NOT NEEDED — 20 native aggregation modules + 1 computed attribute fully replace ComponentCostEvaluator
- [x] 5.2 run_pipeline.py: created, zero application-level workarounds
- [x] 5.3 Input JSON: all values present (13 `:>>` literals, 3 multiplicity counts, 3 permitting defaults, CalcDef defaults in design_params/library_params)
- [x] 5.4 Pipeline execution: all 36 modules executed successfully
- [x] 5.5 verify_pipeline.py: created with 7 ground truth values
- [x] 5.6 Numerical verification: **7/7 PASS**
  - total_capex = 41205.0 (exact)
  - annual_energy_mwh = 11.14272 (within 1%)
  - annual_om_cost = 160.0 (within 1%)
  - annual_fuel_cost = 0.0 (exact)
  - capital_recovery_factor = 0.0709525 (within 1%)
  - annualized_capital_cost = 2923.596 (within 1%)
  - lcoe_per_mwh = 288.676 (within 1%)
- [ ] 5.7 Smart-regen: NOT YET RUN
- [x] 5.8 Regression: core 48 passed, 1 skipped; generated 15/15 passed

**4 New Codegen Bugs Discovered (infrastructure-level, not application-level):**
1. **Bug 8** — `__init__.py` wrong import paths + name collisions: aggregation modules use `solarbatterylibrary__solar_array` paths (nonexistent) instead of `solarbatterydesign.solar_battery_plant.solar_array`; 20 aggregation modules share 5 class names (`capital_costModule`, etc.) causing import collisions. **Workaround**: aliased imports + `module_type_override` map in `create_registry()`
2. **Bug 9** — Missing `system_design.` prefix on entry point channels: pipeline.yaml references raw channel names (`SolarBatteryDesign__...wattage`) instead of `system_design.SolarBatteryDesign__...wattage`. **Workaround**: Python script added prefix to 28 input references
3. **Bug 10** — `int` type for multiplicity counts: pipeline.yaml uses `int` for `module_count`/`inverter_count`/`pack_count` but TEAx modules expect `float`. **Workaround**: Changed `int` → `float` in pipeline.yaml and SystemDesign schema
4. **Bug 11** — `default=0.0` on MultiOutput fields: `PermittingCostCalcOutput` has defaults on 4 of 5 fields, causing `create_registry()` to detect only 1 output. **Workaround**: Removed defaults from output schema fields

**Issues:**
- All 4 workarounds are in generated infrastructure code (registry, pipeline YAML, schemas), NOT in business logic (calc implementations, aggregation formulas)
- The pipeline produces correct numerical results with these workarounds applied
- These should be filed as codegen bugs for the sysml-codegen backlog

**Phase 5.7 Smart-Regen Result:**
- `--smart-regen --preserve-handwritten` preserves `__init__.py` (Bug 8 workaround) and all 36 `_impl.py` files (AUTO_IMPLEMENTED)
- BUT it re-generates pipeline.yaml, system_design.py, permittingcostcalc_output.py — overwriting Bug 9/10/11 workarounds
- **Conclusion**: Smart-regen is NOT safe to run after workarounds are applied. Bugs 9-11 must be fixed in codegen before smart-regen is usable.

**Deviations:**
- Plan anticipated possible ComponentCostEvaluator hybrid merge — not needed at all
- 4 unexpected codegen bugs required workarounds to generated files (plan assumed codegen output would be directly executable)
- Smart-regen overwrites 3 of 4 workarounds (all except __init__.py)
