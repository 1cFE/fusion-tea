...
- **R-D4** `[AGENT]` **The determinism assumption, stated as an assumption.** R-B4 skips no gate, so a second invocation re-runs regeneration (R-B1.2) in place; R-C1 makes a non-git-clean package tree a `BLOCKER`. So R-D1's "same identity" holds only if regeneration under the pinned toolchain is byte-stable onto an already-regenerated tree. The contract does not depend on that being true:
  ...
  - **De-risk this in design before building on it** `[AGENT]`: run the pinned generate twice in place on an already-sealed package and check that the tree stays git-clean and the executable and semantic fingerprints do not move. If it does not hold, R-B4's no-skip rule needs an explicit exemption for regeneration and design says so rather than discovering it at implement.
...
- **Regeneration determinism de-risk** (R-D4) — confirm before building on it that the pinned generate, run twice in place on an already-sealed package, leaves the tree git-clean and both fingerprints unmoved. If it does not, R-B4 needs a stated exemption for regeneration.
