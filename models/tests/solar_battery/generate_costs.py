#!/usr/bin/env python3
"""Cost evaluation script for the solar+battery SysML model.

Evaluates cost calculations from the model and produces actual_output.csv
to validate against expected_output.csv.

Adapted from the coffee maker evaluation script. Uses the same evaluation
engine (extract -> resolve -> evaluate -> aggregate) with ROOT_PART_NAME
changed to target the solar battery plant hierarchy.
...
class PartInstance:
    ...
    path: str  # "solar_battery_plant.solar_array.pv_module"
...
def extract_design_hierarchy(
    ...
    """Extract the design hierarchy starting from the root part.
...
def compute_costs(model_path: str, verbose: bool = False) -> dict[str, dict[str, float]]:
    ...
    Returns:
        Dict mapping qualified paths (e.g., "solar_battery_plant.solar_array.pv_module") to
...
def extract_design_params(
    ...
    EXPECTED_PARAMS = {
        ...
        "yearly_inflation", "discount_rate", "om_rate_per_kw_year",
        "fuel_unit_cost", "fuel_consumption",
    }

    params: dict[str, float] = {}

    if hasattr(root_usage, "owned_members"):
        for member in root_usage.owned_members:
            type_name = type(member).__name__
            if "AttributeUsage" not in type_name:
                continue
            ...
            name = getattr(member, "name", None)
            if not name or name not in EXPECTED_PARAMS:
                continue
    ...
    if "p_net_mw" in params:
        params["p_net_kw"] = params["p_net_mw"] * 1000.0

    # Add total_capex from cost rollup
    params["total_capex"] = total_capex

    # Validate all 11 keys present
    expected_keys = EXPECTED_PARAMS | {"p_net_kw", "total_capex"}
    missing = expected_keys - set(params.keys())
    if missing:
        raise ValueError(
            f"Missing design parameters: {sorted(missing)}. "
            f"Found: {sorted(params.keys())}"
        )
...
def write_component_costs_json(
    ...
    """
    # Find root and subsystem results by path
    root_result = None
    subsystems: dict[str, CostResult] = {}
    subsystem_names = ["solar_array", "battery_system", "site_infra"]

    for result in results:
        if result.path == ROOT_PART_NAME:
            root_result = result
        for sub_name in subsystem_names:
            if result.path == f"{ROOT_PART_NAME}.{sub_name}":
                subsystems[sub_name] = result

    if root_result is None:
        raise ValueError(f"Root result '{ROOT_PART_NAME}' not found in results")

    for sub_name in subsystem_names:
        if sub_name not in subsystems:
            raise ValueError(f"Subsystem result '{ROOT_PART_NAME}.{sub_name}' not found")
    ...
    for sub_name in subsystem_names:
        sub = subsystems[sub_name]
        data[sub_name] = {
            "total_cost": sub.total_cost,
            "material_cost": sub.total_material_cost,
            "fab_cost": sub.total_fab_cost,
            "install_cost": sub.total_install_cost,
        }
...
def verify_system_outputs(
    ...
    Returns:
        (passed, list of diff messages)
    """
    p_net_mw = params["p_net_mw"]
    p_net_kw = params["p_net_kw"]
    n_mod = params["n_mod"]
    plant_availability = params["plant_availability"]
    plant_lifetime = params["plant_lifetime"]
    yearly_inflation = params["yearly_inflation"]
    discount_rate = params["discount_rate"]
    om_rate_per_kw_year = params["om_rate_per_kw_year"]
    fuel_unit_cost = params["fuel_unit_cost"]
    fuel_consumption = params["fuel_consumption"]
    total_capex = params["total_capex"]

    # Compute system-level outputs
    # Source: library.sysml EnergyProductionCalc (line 280)
    annual_energy_mwh = 8760.0 * p_net_mw * n_mod * plant_availability

    # Source: library.sysml AnnualizedOMCalc (line 295)
    annual_om_cost = om_rate_per_kw_year * p_net_kw

    # Source: library.sysml AnnualizedFuelCalc (line 311)
    annual_fuel_cost = fuel_unit_cost * fuel_consumption

    # Source: library.sysml AnnualizedFinancialCalc (lines 329-332)
    r = discount_rate
    n = plant_lifetime
    capital_recovery_factor = r * (1.0 + r) ** n / ((1.0 + r) ** n - 1.0)
    annualized_capital_cost = capital_recovery_factor * total_capex

    # Source: library.sysml LCOECalc (lines 352-355)
    lcoe_per_mwh = (
        annualized_capital_cost
        + (annual_om_cost + annual_fuel_cost) * (1.0 + yearly_inflation) ** plant_lifetime
    ) / annual_energy_mwh
    ...
    with open(expected_path, "r") as f:
        ...
        for row in reader:
            ...
            expected_value = float(row["output_value"])

            if output_name not in computed:
                diffs.append(f"Unknown output: {output_name}")
                continue

            actual_value = computed[output_name]

            # Relative tolerance check (1% default)
            if expected_value == 0.0:
                if abs(actual_value) > tolerance:
                    diffs.append(
                        f"{output_name}: actual={actual_value:.6f}, expected={expected_value:.6f}"
                    )
            else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate cost calculations from solar+battery SysML model"
    )
    parser.add_argument(
    ...
    print("Extracting design parameters...")
    root_usage = None
    for part_usage in SysideAdapter.elements_of_type(model, "PartUsage"):
        if getattr(part_usage, "name", None) == ROOT_PART_NAME:
            ...
            break
    if root_usage is None:
        print(f"Error: Could not find root part usage '{ROOT_PART_NAME}'", file=sys.stderr)
        return 1

    # Get total_capex from root result
    root_result = next((r for r in results if r.path == ROOT_PART_NAME), None)
    if root_result is None:
        print(f"Error: No result for root '{ROOT_PART_NAME}'", file=sys.stderr)
        return 1

    params = extract_design_params(root_usage, total_capex=root_result.total_cost)
    print(f"  Extracted {len(params)} design parameters")

    # Phase 7: Write component costs JSON
    component_json_path = model_dir / "component_costs.json"
    print(f"Writing {component_json_path}...")
    write_component_costs_json(results, component_json_path)

    # Phase 8: Write design params JSON
    design_json_path = model_dir / "design_params.json"
    print(f"Writing {design_json_path}...")
    write_design_params_json(params, design_json_path)

    # Phase 9: Verify system-level outputs
    expected_system_path = model_dir / "expected_system_outputs.csv"
    if expected_system_path.exists():
        print(f"Verifying system outputs against {expected_system_path}...")
        sys_passed, sys_diffs = verify_system_outputs(params, expected_system_path)

        if sys_passed:
            print("PASS: System-level outputs match within tolerance")
        else:
            print(f"FAIL: System output verification failed:")
            for diff in sys_diffs:
                print(f"  - {diff}")
            return 1
    else:
        print(f"Note: {expected_system_path} not found, skipping system verification")

    # Phase 10: Compare CSV with expected
